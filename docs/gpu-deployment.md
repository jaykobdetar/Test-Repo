...
The stop watchdog shares the runner's absolute startup cutoff: the original approval deadline minus the full job runtime and 120 seconds for collection/deletion. Only a bound, never-dispatched, single-job calibration on a disposable research Pod qualifies. The approved batch, job specification, deployment and allowance must match; no attempt may ever have existed for that job or approval. Startup ends at this fixed cutoff even if the runner crashes or the watchdog restarts. After any execution attempt, normal five-minute idle shutdown resumes from the recorded stop time. Infrastructure and other requests keep their existing idle policy; ledger outage, uncertainty, a latched stop decision and the original absolute deadline retain priority. This resolves the observed conflict where provider RUNNING meant image pulling while the old watchdog stopped the still-starting Pod after five minutes.
