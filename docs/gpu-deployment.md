...
`gpu_acceptance_runner` provides a narrow installed path for one public fixed case: backend parity, capture retention, hard deadline, output limit or VRAM limit. A `probe-research` one-shot submits the fixed job and compute request through the existing facade; a separate trusted process waits for the existing human approval, binds the observed Pod and SSH host key, stages the configuration, dispatches the exact job, seals its evidence and deletes the Pod with independent absence readback. Neither runner mode approves compute.

Prepare each additional wait case by selecting exactly one case from the generated plan, preserving its label and complete case contents. Pin the resulting plan bytes in the runner configuration before submission. Capture and limit cases must match the generated recipe exactly, including operation, deterministic inputs, limits and idempotency key. Each run uses its own existing approval and at most 900 seconds, with startup and collection/deletion sharing that allowance. The runner still refuses cancellation, supervisor restart, multi-case batches and replacement requests; those need their separate lifecycle orchestration.
