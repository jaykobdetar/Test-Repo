...
Lab's deployment branch. It provides pinned assets, image recipes and fixed
calibration paths through the approved-job pipeline. The immutable worker images
have their own source identities. Commands below are reviewed procedures, not
evidence that their live cases have passed.

This guide covers the **managed-worker** profile. Its 18 prepared case plans
have not passed live; that work is deferred and not required by the current
roadmap. The separate supervised profile has passed numerical calibration for
both models. [STATUS.md](../STATUS.md) is the only place that records current
standing. Every assigned host must pass its resource gates before model
execution.
...
Prepare each additional runtime case by selecting exactly one case from the generated plan, preserving its label and complete case contents. Pin the resulting plan bytes in the runner configuration before submission. Capture, limit and lifecycle cases must match the generated recipe exactly, including action, operation, deterministic inputs, limits and idempotency key. Each run uses its own existing approval and at most 900 seconds, with startup and collection/deletion sharing that allowance. The runner refuses multi-case batches. Its replacement path is restricted to the successful predecessor and fresh parity sequence described below.

The independent watchdog uses the same fixed startup cutoff as the runner: the original approval deadline minus the full job runtime and 120 seconds for collection and deletion. This exception to idle shutdown applies only to a newly created or replaced disposable worker with exactly one approved, pending calibration job and no attempt history. A 900-second approval with a 240-second job therefore permits at most 540 seconds of startup, including image download. Neither process restart nor readiness polling renews this window. The first attempt permanently ends the exception; its recorded stop time anchors the ordinary five-minute idle limit, even if execution starts and stops between watchdog polls. Infrastructure probes and other job batches retain their existing idle policy. Absolute deadlines, uncertain actions, database failure and previously recorded stop decisions still take precedence.
...
The runner's optional `replacement` binding pins the predecessor's request, logical worker, physical Pod, approval, completed job, attempt, canonical manifest and canonical runner-result hashes, plus the raw SHA256 and path of its original administrator-owned runner configuration. That original configuration anchors the same deployment and worker-file bytes, source, region, price, identities and paths. The new plan changes only its label and job idempotency key; use fresh submission and trusted-state directories. A startup failure, failed parity, missing artifact bundle, open allowance or an unknown physical resource cannot serve as this baseline.

Before requesting a new approval, preparation runs `/opt/probe-core/venv/bin/python -I -m probe_core.gpu_acceptance_runner verify-replacement --config <administrator-owned-config>` as the trusted service identity. This mode has no compute authority: it reads the prior provider state and writes a private `replacement-preflight.json` receipt. The research submission mode then uses the existing `request_gpu_provision` method with the exact predecessor worker ID. The human still approves the new request through the existing admin socket. Running acceptance without the saved, matching preflight fails and deletes any newly approved Pod.

After approval the runner independently rechecks the predecessor's stopped attempt, closed allowance, exact deleted Pod and accepted artifact seal/bytes. On the newly authenticated SSH endpoint it runs only the existing model inventory code, after clearing the environment and dropping supplementary groups and UID/GID to 10001. The fixed command has a wall alarm, CPU ceiling and bounded output; its observed model, tokenizer and weight hashes must match the unchanged worker configuration and canonical model lock. This read shares the original startup cutoff and cannot extend the approved deadline. Fresh parity still runs through the normal dispatcher and collector. The old controller bundle is rehashed after worker readiness and after fresh parity, and the new Pod must be positively absent before `replacement.complete` becomes true. Private before/model/ready/after receipts retain the exact evidence. A restart cannot repeat a completed result or dispatch an already-started replacement attempt. This replacement-specific result does not by itself complete cancellation, tunnel or other lifecycle cases.
...
This section keeps the startup contracts that later managed attempts
established. The dated attempt-by-attempt record and the incident-specific
recovery helpers are in
[the startup attempt history](history/2026-09-20-gpu-startup-attempts.md);
current standing is in [STATUS.md](../STATUS.md).
...
Startup uses the existing approval deadline minus the full job runtime and a
120-second allowance for result collection and deletion; it never extends the
paid allowance. The launcher re-executes its fixed interpreter in isolated mode
with an allowlisted environment before any cgroup, SSH or model work, because
[RunPod automatically supplies a Pod-scoped API key](https://docs.runpod.io/pods/templates/environment-variables).
SSH and every worker process also receive explicitly constructed environments.
A retry preserves the completed failure and stopped attempt and uses fresh
job, request and approval identities; a consumed approval is never replayed.
