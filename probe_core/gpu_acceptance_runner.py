...
def safe_provider_diagnostic(value):
    if (
        ...
        or value["content_type"] not in {"json", "html", "other"}
        or (
    ...
    return {
        key: value.get(key)
        for key in ("phase", "http_status", "content_type", "retry_after_seconds", "cf_mitigated_challenge")
    }
...
def bootstrap_record(raw):
    ...
    try:
        ...
        if (
            ...
            or set(record) != {"status", "code", "error_type", "bootstrap_line"}
            or record["status"] != "failed"
            or record["code"] != "WORKER_BOOTSTRAP_FAILED"
            or record["error_type"] not in BOOTSTRAP_EXCEPTION_TYPES
            or type(record["bootstrap_line"]) is not int
            or not 1 <= record["bootstrap_line"] <= 1000000
        ):
            return None
        return record
...
def safe_command_diagnostic(value, *, phase=None):
    ...
    if (
        ...
        or phase not in {"verification", "upload", "configure", "tunnel"}
        or (status is not None and (type(status) is not int or not -255 <= status <= 255))
        or type(timeout) is not bool
        or type(classification) is not str
        or classification not in COMMAND_CLASSIFICATIONS
    ):
        return None
...
def read_file(path, *, owner, private=False, bound=BOUND):
    ...
    for parent in path.parents:
        ...
        require(
            info.st_uid in {root_owner, owner} and (not info.st_mode & 0o022 or temporary_root),
            "UNTRUSTED_INPUT_PARENT",
        )
...
class RunnerConfig(FrozenModel):
    ...
    def fixed_profile(self):
        ...
        require(
            self.deployment.storage_mode == "disposable_research"
            and self.deployment.volume_id is None
            and self.deployment.volume_gb == 0
            and self.deployment.image_repository is not None
            and self.deployment.launch_config_hash is not None,
            "DISPOSABLE_RESEARCH_DEPLOYMENT_REQUIRED",
        )
        for name in (
            "plan_path",
            "worker_config_path",
            "submission_state_directory",
            ...
            require(
                value.startswith("/") and all(p not in ("", ".", "..") for p in value.split("/")[1:]),
...
def validate_plan(config, plan):
    ...
    require(
        case.spec.experiment_stage.value == "calibration" and case.spec.model == plan.model,
        "FIXED_CALIBRATION_CASE_REQUIRED",
    )
    if case.name == "backend-parity":
        ...
        require(
            case.action == "wait"
            and case.expected_state == "COMPLETED"
            and case.expected_failure_kind is None
            and case.spec.operation.kind == "backend_parity",
            "FIXED_BACKEND_PARITY_REQUIRED",
        )
    elif case.name == "public-direction-transfer":
        inputs = case.spec.inputs
        expected = fixed_direction_plan(
            plan.model, plan.label, inputs.dataset_revision, inputs.prompt_set_hash, inputs.prompt_ids
        ).cases[0]
    ...
    require(
        any(
            (item["repo"], item["revision"]) == (plan.model.repo, plan.model.revision_sha)
            for item in canonical["models"]
        )
        and plan.model.dtype == "bfloat16"
        and plan.model.quantized is False,
        "CANONICAL_MODEL_REQUIRED",
    )
    require(
        len(case.spec.inputs.prompt_ids) == 2 and case.spec.limits.max_runtime_seconds <= config.max_runtime_seconds,
...
def load_worker_config(config, plan, *, owner=0):
    ...
    require(
        worker.model == plan.model
        and worker.device == "cuda:0"
        and worker.backend == "nnsight"
        and worker.provider_backend == "runpod"
        ...
        and worker.output_directory == "/workspace/probe/attempts",
        "WORKER_CONFIG_PROVENANCE_MISMATCH",
    )
    require(
        worker.model_directory.startswith("/opt/probe-assets/models/")
        and len(worker.datasets) == 1
...
class State:
    ...
    def __init__(self, directory):
        ...
        require(
            stat.S_ISDIR(info.st_mode) and info.st_uid == os.geteuid() and not info.st_mode & 0o077, "STATE_NOT_PRIVATE"
        )
...
def matches_request(row, config, job_id):
    ...
    return (
        row.get("action") == ("REPLACE" if predecessor else "CREATE")
        and row.get("job_ids") == [job_id]
        ...
        and row.get("replaces_worker_id") == predecessor
    )
...
def submit(config, plan, rpc, state):
    ...
    while True:
        try:
            ...
            require(
                status.get("gpu_start_authority") is False and isinstance(status.get("jobs"), list),
                "RESEARCH_SURFACE_INVALID",
            )
            break
        except RunnerError:
            raise
...
def validate_authority(ledger, config, plan, request, *, now):
    ...
    with ledger.read_connection() as conn:
        ...
        jobs = [
            v[0]
            for v in conn.execute("SELECT job_id FROM approval_jobs WHERE approval_id=?", (request["approval_id"],))
        ]
    ...
    require(
        ...
        and jobs == request["job_ids"]
        and public.get("purpose", "research") == "research"
        and public["pod_id"] == request["worker_id"]
        and public["batch_hash"] == request["batch_hash"]
        ...
        and row["deadline"] == request["deadline"]
        and math.isfinite(row["deadline"])
        and row["consumed_at"] <= now < row["deadline"]
        and row["deadline"] - row["consumed_at"] <= config.max_runtime_seconds,
...
def read_provider_logs(provider_config, pod_id, *, seconds=3, bootstrap_only=False):
    ...
    url = (
        ...
        + quote(pod_id, safe="")
        + "/logs?"
        + urlencode({"source": "container", "tail": 1000})
    )
    request = Request(
        url,
        headers={
    ...
    try:
        ...
        with error:
            diagnostic = safe_provider_diagnostic(
                {"phase": "provider_logs", **provider_http_metadata(error.code, error.headers)}
            )
...
def verified_endpoint(config, request, backend, state, *, logs=read_provider_logs, command=command_bytes):
    ...
    require(
        intent is not None
        and intent["provider_id"] == request["observed_provider_id"]
        and intent["request_key"] == request["request_id"]
        and intent["configuration_hash"] == config.deployment.digest
        and intent["deadline"] == request["deadline"],
    ...
    require(
        observed.state == WorkerState.RUNNING and observed.provider_id == request["observed_provider_id"],
        "PROVIDER_NOT_RUNNING",
    )
    require(
        pod.get("dataCenterId") == config.deployment.region
        and pod.get("image") == config.deployment.image_repository + "@" + config.deployment.image_digest
        and Decimal(str(pod.get("cost"))) == Decimal(str(config.expected_worker_price_usd_per_hour)),
    ...
    require(
        all(
            type(value.get("private")) is int and 1 <= value["private"] <= 65535 and type(value.get("type")) is str
            for value in direct
        ),
        "PROVIDER_RUNTIME_INVALID",
    )
    endpoints = [
        (v.get("ip"), v.get("public"))
        for v in direct
    ...
    return dict(
        ...
        ssh_port=port,
        remote_port=8080,
        identity_file=Path(config.ssh_identity_file),
        known_hosts_file=hosts,
    )


def configure_worker(config, plan, settings, state, *, deadline, clock=time.time, command=command_bytes, owner=0):
    ...
    expected = {
        "schema_version": 1,
        "configured": True,
        "worker_config_sha256": digest(worker),
...
def wait_for_worker(client, *, deadline, clock=time.time, sleep=time.sleep):
    ...
    while clock() < deadline:
        request = Request(
            client.base_url + "/v1/acceptance-readiness",
...
class RunningAction:
    ...
    def __init__(
        self, ledger, plan, request, client, lifecycle, directory, *, deadline, clock=time.time, action=run_action
    ):
        ...
        helper = SimpleNamespace(
            config_sha256=lifecycle.config_sha256,
            endpoint_identity=lifecycle.endpoint_identity,
            inspect=guarded(lifecycle.inspect),
            restart=guarded(lifecycle.restart),
            reconnect=guarded(self.request_reconnect),
        )
    ...
    def request_reconnect(self, request, action_id, *, expected_before):
        ...
        while not self.reconnect_finished.wait(0.1):
            self.check()
        self.check()
...
def lifecycle_settings(settings, configuration):
    require(
        configuration.get("configured") is True
        and isinstance(configuration.get("worker_config_sha256"), str)
    ...
    return {
        **{key: str(settings[key]) for key in ("host", "user", "identity_file", "known_hosts_file")},
...
def read_worker_direction(settings, phase, *, deadline, clock=time.time, command=command_bytes):
    ...
    argv = [
        ...
        "-I",
        "-c",
        shlex.quote(READBACK_PROGRAM),
        "/workspace/probe/tensors",
...
def reconnect_owned_tunnel(
    ...
    *,
    ...
    require(
        dispatcher._request(job) == request and _authority(ledger, request, now), "TUNNEL_RECONNECT_AUTHORITY_CHANGED"
    )
    ...
    require(
        dispatcher._request(renewed) == request
        and renewed.lease_expires_at.timestamp() <= request.deadline.timestamp(),
    ...
    require(
        lifecycle_factory(lifecycle_settings(settings, configuration)).endpoint_identity == pinned.endpoint_identity,
    ...
    require(
        ...
        and replacement.process.poll() is None
        and replacement.local_port == previous["local_port"]
        and replacement.process.pid != previous["pid"]
    ...
    ack = {
        "schema_version": 1,
        "operation": "reconnect_tunnel",
        "action_id": action_id,
        "replayed": False,
        "before": before,
        "endpoint_identity": pinned.endpoint_identity,
        "previous_transport": previous,
        "replacement_transport": {"pid": replacement.process.pid, "local_port": replacement.local_port},
        "renewed_at": now.isoformat(),
        "previous_lease": job.lease_expires_at.timestamp(),
        "renewed_lease": renewed.lease_expires_at.timestamp(),
        "execution_deadline": request.deadline.timestamp(),
    }
    ...
    *,
    ...
    try:
        ...
        if config.replacement is not None:
            ...
            require(
                state.read("replacement-preflight.json") == preflight_record(config, predecessor),
        ...
        state.publish(
            ...
            {
                key: request[key]
                for key in ("request_id", "worker_id", "approval_id", "observed_provider_id", "deadline", "batch_hash")
            },
        )
        ...
        window = {
            ...
            "dispatch_cutoff": startup_deadline,
        ...
        with ExitStack() as resources:
            ...
            if config.replacement is not None:
                ...
                state.publish(
                    "replacement-ready.json",
                    {
                        ...
                        "observed_at": clock(),
                        "bundle": retained,
                        "model_receipt_sha256": assets["receipt_sha256"],
                    },
                )
                require(clock() < startup_deadline, "WORKER_STARTUP_DEADLINE")
            ...
            direction_options = (
                {
                    "readback": lambda phase: direction_readback(settings, phase, deadline=deadline, clock=clock),
                    "publish": lambda evidence: state.publish("direction-transfer.json", evidence),
                }
                if direction_case
                else {}
            )
            dispatcher = (DirectionDispatcher if direction_case else Dispatcher)(
                ledger,
                client,
                worker_id=request["worker_id"],
            ...
            lifecycle = (
                lifecycle_factory(lifecycle_settings(settings, result["configuration"])) if action_case else None
            )
            while clock() < deadline:
                ...
                if (
                    ...
                    and action_task.reconnect_pending.is_set()
                    and not action_task.reconnect_finished.is_set()
                ):
                    tunnel = reconnect_owned_tunnel(
                        action_task,
                        ledger,
                        dispatcher,
                        service,
            ...
            observation = collect(
                ...
                plan,
                client,
                action_directory=action_directory if action_case else None,
            ...
            if plan.cases[0].expected_state == "COMPLETED":
                ...
                require(
                    manifest.software.container_image_digest == config.deployment.image_digest
                    and manifest.software.probe_mcp_git_commit == config.source_commit
                    and manifest.hardware.region == config.deployment.region
                    and Decimal(str(manifest.hardware.live_price_usd_per_hour))
                    == Decimal(str(config.expected_worker_price_usd_per_hour)),
            ...
            if config.replacement is not None:
                require(
                    job.attempt_id != config.replacement.attempt_id
                    and job.attempt_count == 1
                    and job.retry_count == 0
                    and job.worker_id == request["worker_id"]
                    and job.approval_id == request["approval_id"],
                    "REPLACEMENT_FRESH_ATTEMPT_INVALID",
                )
                after = retained_bundle(ledger, config.replacement)
                require(after == predecessor["bundle"], "REPLACEMENT_RETAINED_ARTIFACTS_CHANGED")
                state.publish(
                    "replacement-after.json",
                    {
                        "observed_at": clock(),
                        "bundle": after,
                        "job_id": job.job_id,
                ...
                result["replacement"] = {
                    "complete": False,
                    "predecessor": config.replacement.model_dump(mode="json"),
                    "before_sha256": digest(predecessor),
                    "model_readback_sha256": digest(assets),
...
def main():
    ...
    try:
        ...
        reason = (
            str(error)
            if isinstance(error, (RunnerError, ReplacementError)) and re.fullmatch(r"[A-Z0-9_]{1,100}", str(error))
            else "ACCEPTANCE_RUNNER_REFUSED"
        )
