...
from .gpu_acceptance_runner import (
    CONTROLLER_SOCKET, LEDGER, RESEARCH_SOCKET, RunnerConfig, RunnerError, State,
    digest, find_request, read_file, require, stop_and_observe, submit,
    validate_authority, validate_plan, verified_endpoint,
)
...
PUBLIC_FIELDS = frozenset({'model', 'assets', 'datasets', 'code_git_commit',
    'container_image_digest', 'region', 'live_price_usd_per_hour'})
...
class SupervisedRunnerConfig(RunnerConfig):
    ...
    def supervised_scope(self):
        require(Path(self.helper_path).is_absolute() and '..' not in Path(self.helper_path).parts,
...
def load_inputs(path, *, mode, owner=0):
    ...
    require(os.geteuid() == (config.research_uid if mode == 'submit' else config.service_uid),
            'WRONG_PROCESS_IDENTITY')
    ...
    case = plan.cases[0]
    require(case.action in {'wait', 'cancel_after_running'}
            and case.name != 'public-direction-transfer', 'SUPERVISED_ACTION_NOT_IMPLEMENTED')
    raw = read_file(config.worker_config_path, owner=owner)
    require('sha256:' + hashlib.sha256(raw).hexdigest() == config.worker_config_sha256,
            'WORKER_CONFIG_HASH_MISMATCH')
    worker = decode(raw)
    require(type(worker) is dict and set(worker) == PUBLIC_FIELDS, 'PUBLIC_CONFIGURATION_REQUIRED')
    require(worker['model'] == plan.model.model_dump(mode='json')
    ...
    require('sha256:' + hashlib.sha256(helper).hexdigest() == config.helper_sha256,
...
def wait_for_approval(config, plan, ledger, *, clock=time.time, sleep=time.sleep):
    ...
    while time.monotonic() < until:
        ...
        if request is not None and request['state'] == 'RUNNING':
            return request
        require(request is None or request['state'] in {'PENDING', 'PREPARING', 'STARTING'},
...
def cancellation_step(dispatcher, client, job, state, worker, deadline):
    ...
    require(current['receipt']['state'] == 'CANCELLED'
            ...
            and all(proof.get(key) == before.get(key) == current.get(key)
            ...
            and proof.get('process_stopped') is True and proof.get('descendants_stopped') is True
            and proof.get('result_present') is False and proof.get('signal') == 'SIGKILL'
...
def run(config, plan, worker, ledger, backend, cloud, state, *, clock=time.time,
        sleep=time.sleep, endpoint=verified_endpoint, client_factory=SSHJobClient,
        progress=lambda stage: None):
    """Resume the persisted attempt; never manufacture a new approval or deadline."""
    validate_plan(config, plan)
    state.publish('binding.json', {'config_sha256': digest(config.model_dump(mode='json')),
    ...
    report = {'schema_version': 1, 'kind': 'supervised_ledger_acceptance', 'status': 'failed',
        'profile': config.profile, 'request_id': request['request_id'], 'worker_id': request['worker_id'],
    ...
    try:
        ...
        state.publish('bound-request.json', {k: request[k] for k in
            ('request_id', 'worker_id', 'approval_id', 'observed_provider_id', 'deadline', 'batch_hash')})
        ...
        startup_cutoff = deadline - 120 - (plan.cases[0].spec.limits.max_runtime_seconds
                                         if job.attempt_id is None else 0)
        ...
        client = client_factory(settings, worker, datetime.fromtimestamp(deadline, timezone.utc),
                                timeout_seconds=10)
        ...
        dispatcher = Dispatcher(ledger, client, worker_id=request['worker_id'],
            transfer_directory=state.directory / 'transfers', input_artifact_root=LEDGER.parent/'input-artifacts',
            lease_seconds=30)
        ...
        if plan.cases[0].action == 'cancel_after_running':
            require(state.read('cancellation-before.json') is not None
                    and state.read('cancellation-after.json') is not None, 'CANCELLATION_EVIDENCE_MISSING')
            report['cancellation_proven'] = True
        job = ledger.get_job(job.job_id)
        if job.state == JobState.COMPLETED:
            manifest = ledger.get_manifest(job.job_id)
            require(manifest.model == plan.model
        ...
        report.update(status='passed', case=plan.cases[0].name, job_id=job.job_id,
            attempt_id=job.attempt_id, observed_state=job.state.value,
            observed_failure_kind=job.failure_kind, attempt_count=job.attempt_count)
...
def configuration_paths(config_path, suite_path):
    ...
    require(type(body) is dict and set(body) == {'schema_version', 'configurations'}
            and body['schema_version'] == 1 and type(body['configurations']) is list
            and 1 <= len(body['configurations']) <= 16, 'SUITE_INVALID')
    paths = []
    for value in body['configurations']:
        require(type(value) is str and '/' not in value and value.endswith('-acceptance.json')
                and value not in {'.', '..'} and len(value) <= 128, 'SUITE_PATH_INVALID')
        ...
        with State(directory) as state:
            if args.mode == 'submit':
                ...
                with Ledger(LEDGER) as ledger:
                    result = run(config, plan, worker, ledger, provider,
