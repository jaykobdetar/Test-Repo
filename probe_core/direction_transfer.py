...
def direction_reference():
    return {
        "path": DIRECTION_SHA256.removeprefix("sha256:") + "/tensor.safetensors",
        "sha256": DIRECTION_SHA256,
        "tensor_name": DIRECTION_NAME,
    }


def expected_upload_receipt():
    return {
        "path": direction_reference()["path"],
        "sha256": DIRECTION_SHA256,
...
def controller_direction(root):
    ...
    expected = {
        "artifact_id": DIRECTION_SHA256.removeprefix("sha256:"),
        "path": direction_reference()["path"],
        "bytes": DIRECTION_BYTES,
        "sha256": DIRECTION_SHA256,
        "tensor_refs": [direction_reference()],
    }
    ...
    def _stage_inputs(self, job):
        try:
            ...
            self.publish(
                {
                    "schema_version": 1,
                    "kind": "public_direction_transfer",
                    "request": self._request(job).model_dump(mode="json"),
                    "controller": controller,
                    "worker_before": before,
                    "upload_receipt": uploaded[DIRECTION_SHA256],
                    "worker_after": after,
...
def verify_direction_evidence(ledger, job, evidence, root, receipt, manifest):
    ...
    expected = {
        ...
        "controller": controller_direction(root),
        "worker_before": {"state": "absent"},
        "upload_receipt": expected_upload_receipt(),
        "worker_after": expected_present_readback(),
        "scientific_evidence": False,
    }
    if (
        not _same(evidence, expected)
        or receipt is None
