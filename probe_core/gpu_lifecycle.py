...
def _directory(path: Path, policy: _Policy, *, private=False, root_only=False):
    ...
    try:
        for component in (None, *parts):
            ...
            _require(
                info.st_uid in {policy.root_uid, policy.worker_uid} and not info.st_mode & 0o022, "UNSAFE_DIRECTORY"
            )
        info = os.fstat(descriptor)
        if private:
...
def _worker_identity(process, policy):
    _require(
        process is not None
        and process["uids"] == [policy.worker_uid] * 4
...
def _load_config(config_path, expected, config_sha256, policy):
    _require(
        isinstance(config_sha256, str) and re.fullmatch(r"sha256:[0-9a-f]{64}", config_sha256), "CONFIG_HASH_REQUIRED"
    )
...
def _snapshot(config_path, token_path, expected, config_sha256, policy):
    ...
    with _directory(directory_path, policy, private=True) as directory:
        raw = {
            name: _read_at(directory, name, uid=policy.worker_uid)
    ...
    _require(
        type(child) is dict
        and set(child) == {"pid", "identity", "boot_id", "deadline", "monotonic_deadline", "cgroup"},
        "INVALID_CHILD_METADATA",
    )
    _require(
        type(child["pid"]) is int
        and child["pid"] > 1
        and isinstance(child["identity"], str)
        and child["identity"].isdigit()
        and child["boot_id"] == _boot_id(),
        "INVALID_CHILD_IDENTITY",
    )
    for key in ("deadline", "monotonic_deadline"):
        _require(
            type(child[key]) in (float, int) and math.isfinite(child[key]) and child[key] > 0,
            "INVALID_EXECUTION_DEADLINE",
        )
    expected_cgroup = (
        None
        if config.cgroup_directory is None
        else str(Path(config.cgroup_directory) / ("probe-" + request.attempt_id))
    )
    ...
    expected_deadline = min(
        expected.deadline.timestamp(), receipt.started_at.timestamp() + expected.spec.limits.max_runtime_seconds
    )
    ...
    _require(
        type(marker) is dict
        and set(marker) == marker_fields
        and type(marker["schema_version"]) is int
        and marker["schema_version"] == 1,
        "INVALID_STARTED_MARKER",
    )
    _require(
        all(marker[key] == getattr(expected, key) for key in ("job_id", "attempt_id", "worker_id", "approval_id"))
    ...
    try:
        ...
        _require(
            started.tzinfo is not None and receipt.started_at <= started and started.timestamp() <= child["deadline"],
            "INVALID_STARTED_TIME",
        )
    ...
    _require(
        supervisor["executable"] == interpreter
        and supervisor["args"]
        in ([policy.interpreter, *worker_arguments], [policy.interpreter, "-I", *worker_arguments]),
        "WRONG_SUPERVISOR_COMMAND",
    )
    bootstrap = inspector(supervisor["ppid"])
    _require(
        bootstrap is not None
        and bootstrap["uids"] == [policy.root_uid] * 4
        and bootstrap["executable"] == interpreter
        and bootstrap["args"]
        in (
            ["python", "-m", "probe_core.gpu_launch"],
            [policy.interpreter, "-m", "probe_core.gpu_launch"],
            [policy.interpreter, "-I", "-m", "probe_core.gpu_launch"],
        )
        and bootstrap["boot_id"] == supervisor["boot_id"] == child["boot_id"],
        "WRONG_BOOTSTRAP_IDENTITY",
    )
    _require(child["pid"] not in {supervisor["pid"], bootstrap["pid"]}, "OVERLAPPING_PROCESS_IDENTITIES")
    _require(inspector(supervisor["pid"]) == supervisor and inspector(bootstrap["pid"]) == bootstrap, "PROCESS_CHANGED")
    ...
    if cancellation_bytes is not None:
        ...
        _require(
            ...
            and set(cancellation) == fields
            and type(cancellation["schema_version"]) is int
            and cancellation["schema_version"] == 1,
            "INVALID_CANCELLATION_PROOF",
        )
        _require(
            all(
                cancellation[key] == getattr(expected, key)
                for key in ("job_id", "attempt_id", "worker_id", "approval_id")
            )
            and all(
                cancellation[key] == child[key]
        ...
        _require(
            ...
            and cancellation["process_stopped"] is True
            and cancellation["job_scope_stopped"] is True
            and cancellation["result_present"] is False
            and not result_present
            and not child_alive
            and receipt.state == WorkerState.CANCELLED
            and receipt.failure_kind == "cancelled"
            and receipt.process_stopped is True,
            "CANCELLATION_STOP_NOT_PROVEN",
        )
        try:
            ...
            _require(
                ...
                and stopped == receipt.finished_at,
    ...
    return {
        ...
        "request_sha256": _sha(raw["request.json"]),
        "process_sha256": _sha(raw["process.json"]),
        "execution_started_sha256": _sha(raw["execution-started.json"]),
        ...
        "cancellation_sha256": None if cancellation_bytes is None else _sha(cancellation_bytes),
    }
...
def _running(snapshot):
    _require(
        ...
        and snapshot["receipt"]["state"] == WorkerState.RUNNING
        and snapshot["receipt"]["process_stopped"] is False,
        "ATTEMPT_NOT_RUNNING",
    )
    child = snapshot["child"]
    _require(
        time.time() < child["deadline"] and time.monotonic() < child["monotonic_deadline"], "EXECUTION_DEADLINE_EXPIRED"
    )
    _require(
        child["monotonic_deadline"] - time.monotonic()
        <= snapshot["request"]["spec"]["limits"]["max_runtime_seconds"] + 1,
        "INVALID_EXECUTION_DEADLINE",
    )
    ...
    with _directory(policy.intent_root, policy, private=True, root_only=True) as directory:
        ...
        try:
            ...
            _require(
                ...
                and info.st_nlink == 1
                and info.st_uid == policy.root_uid
            ...
            if previous is not None:
                ...
                try:
                    result = _json(
                        _read_at(directory, result_name, uid=policy.root_uid, private=True, limit=MAX_OUTPUT)
                    )
                    _require(
                        result["action_id"] == action_id and type(result["signal_sent"]) is bool,
            ...
            try:
                ...
                _running(fresh)
                _require(
                    _stable(before) == _stable(fresh) and not select.select([descriptor], [], [], 0)[0],
                    "TARGET_CHANGED_BEFORE_SIGNAL",
                )
                ...
                _running(final)
                _require(
                    _stable(fresh) == _stable(final) and not select.select([descriptor], [], [], 0)[0],
                    "TARGET_CHANGED_BEFORE_SIGNAL",
                )
...
def main():
    ...
    try:
        ...
        _require(
            operation in {"inspect", "restart"}
            and set(command) == fields | ({"action_id", "expected_before"} if operation == "restart" else set()),
            "INVALID_COMMAND",
        )
        ...
        code = (
            str(error)
            if isinstance(error, LifecycleError) and re.fullmatch(r"[A-Z][A-Z0-9_]{1,80}", str(error))
            else "LIFECYCLE_INSPECTION_FAILED"
        )
