...
def run_parity(engine, request):
    ...
    if engine.config.model.repo != "probe/testing-tiny-qwen3" and (engine.config.device != "cuda:0" or engine.config.model.dtype != "bfloat16"):
    ...
    def compare(name, actual, expected):
        ...
        checks.append({"name": name, "passed": True, "max_absolute_error": float((actual.float()-expected.float()).abs().max()), "rtol": 0, "atol": 0})
    ...
    try:
        ...
        modules = [{"layer": layer, "component": name, **({"head": 0} if name == "attention_head" else {})}
        ...
        with tempfile.TemporaryDirectory(prefix="parity-", dir=root) as directory_name:
            ...
            def artifact(name, tensor):
                ...
                return {"path": str(path.relative_to(root)), "sha256": "sha256:" + hashlib.sha256(path.read_bytes()).hexdigest(), "tensor_name": "value"}
            for index, target in enumerate(modules):
                ...
                for strength in (0.0, 0.5):
                    steer = Steer(kind="steer", target=target, positions=("last",), direction=reference, strength=strength)
        ...
        for row, length in enumerate(lengths):
            ...
            settings = GenerationConfig(max_new_tokens=request.spec.inputs.generation.max_new_tokens,
                                        do_sample=False, use_cache=False,
                                        eos_token_id=model.config.eos_token_id,
                                        pad_token_id=model.config.pad_token_id or model.config.eos_token_id,
                                        bos_token_id=model.config.bos_token_id)
            with torch.inference_mode():
                native_tokens = model.generate(input_ids=prefix, attention_mask=torch.ones_like(prefix), generation_config=settings)[0, length:]
            produced = []
            current = prefix
            for _ in range(request.spec.inputs.generation.max_new_tokens):
                deadline()
                mask = torch.ones_like(current)
                logits, _ = engine.forward({"input_ids": current, "attention_mask": mask, "position_ids": mask.cumsum(-1)-1, "use_cache": False}, (current.shape[1],), backend="nnsight")
        ...
        return tensors, {"suite": "backend_parity_v1", "passed": True, "scientific_evidence": False,
                         "device": engine.config.device, "dtype": engine.config.model.dtype,
                         "input_lengths": list(lengths), "chat_template_hash": engine.config.model.chat_template_hash,
                         "thinking_mode": engine.config.model.thinking_mode, "checks": checks,
                         "generations": generations, "generated_tokens_including_reference": generated}, generated
    finally:
