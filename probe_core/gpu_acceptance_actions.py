...
class SSHActionClient:
    ...
    def __init__(self, settings):
        ...
        self.command = [
            ...
            "-o",
            "UserKnownHostsFile=" + str(tunnel.known_hosts_file),
            "-i",
            str(tunnel.identity_file),
            "-p",
            str(tunnel.ssh_port),
...
def _snapshot(value, request):
    ...
    for identity, minimum_pid in ((child, 2), (value["supervisor"], 2), (value["bootstrap"], 1)):
        if (
            ...
            or identity["pid"] < minimum_pid
            or not isinstance(identity.get("identity"), str)
            or not identity["identity"].isdigit()
    ...
    if (
        ...
        or type(value.get("observed_monotonic")) not in (float, int)
        or not math.isfinite(value["observed_monotonic"])
        or value["observed_monotonic"] < 0
    ):
...
def _running(snapshot, request, now):
    ...
    return (
        snapshot["child_alive"]
        and receipt.state == WorkerState.RUNNING
        and not receipt.process_stopped
        and now.timestamp() < snapshot["child"]["deadline"] <= request.deadline.timestamp()
        and datetime.fromisoformat(snapshot["observed_at"]).timestamp() < snapshot["child"]["deadline"]
        and snapshot["observed_monotonic"] < snapshot["child"]["monotonic_deadline"]
    )
...
def _reconnect_ack(ack, before, request, action_id, endpoint_identity):
    ...
    try:
        if (
            ...
            or ack.get("operation") != "reconnect_tunnel"
            or ack.get("action_id") != action_id
            or ack.get("replayed") is not False
            or ack.get("endpoint_identity") != endpoint_identity
        ...
        if (
            set(old) != {"pid", "exit_status", "local_port"}
            or set(new) != {"pid", "local_port"}
            or any(type(value["pid"]) is not int or value["pid"] <= 1 for value in (old, new))
            or old["pid"] == new["pid"]
            or type(old["exit_status"]) is not int
            or not -255 <= old["exit_status"] <= 255
            or any(
                type(value["local_port"]) is not int or not 1 <= value["local_port"] <= 65535 for value in (old, new)
            )
...
def _cancel_proof(snapshot, before, request):
    ...
    if (
        ...
        or not re.fullmatch(r"sha256:[0-9a-f]{64}", proof_hash)
    ):
        return False
    child = before["child"]
    if (
        ...
        or any(proof.get(key) != getattr(request, key) for key in ("job_id", "attempt_id", "worker_id", "approval_id"))
        or proof.get("request_sha256") != digest(request.model_dump(mode="json"))
        or proof.get("config_sha256") != before["loaded_config_sha256"]
        or any(
            proof.get(key) != child[key]
            for key in ("pid", "identity", "boot_id", "deadline", "monotonic_deadline", "cgroup")
        )
        or proof.get("signal") != "SIGKILL"
        or proof.get("signal_scope") != ("process_group" if child["cgroup"] is None else "process_and_cgroup")
        or proof.get("process_stopped") is not True
        or proof.get("job_scope_stopped") is not True
        or proof.get("result_present") is not False
        or snapshot["child_alive"]
    ):
        return False
    try:
        sent, stopped = (datetime.fromisoformat(proof[key]) for key in ("signal_sent_at", "stopped_at"))
        return (
            sent.tzinfo is not None
            and stopped.tzinfo is not None
            and datetime.fromisoformat(before["observed_at"])
            <= sent
            <= stopped
            <= datetime.fromisoformat(snapshot["observed_at"])
            and sent.timestamp() < child["deadline"]
        )
...
def _authority(ledger, request, now):
    ...
    with ledger.read_connection() as connection:
        row = connection.execute(
            """SELECT j.state, j.attempt_id AS current_attempt,
    ...
    return (
        row["state"] == "RUNNING"
        and row["current_attempt"] == request.attempt_id
        and row["current_worker"] == row["worker_id"] == request.worker_id
        and row["current_approval"] == row["approval_id"] == request.approval_id
        and row["job_id"] == request.job_id
        and row["stopped_at"] is None
        and row["batch_member"] == 1
        and row["consumed_at"] is not None
        and row["consumed_at"] <= timestamp
        and row["ended_at"] is None
        and row["execution_deadline"] == request.deadline.timestamp()
        and timestamp < row["job_lease"]
        and timestamp < row["attempt_lease"]
        and timestamp < row["execution_deadline"] <= row["approval_deadline"]
    )
...
def run_action(
    ...
    *,
    ...
    request = Dispatcher(ledger, client, worker_id=job.worker_id, transfer_directory=Path(action_directory))._request(
        job
    )
    binding = {
        ...
        "action": case.action,
        "request": request.model_dump(mode="json"),
        "worker_config_sha256": lifecycle.config_sha256,
    ...
    try:
        ...
        if intent is None:
            ...
            if (
                not _running(before, request, clock())
                or not _running(checked, request, clock())
                or not _stable(before, checked)
                or before["supervisor"] != checked["supervisor"]
                or remote.state != WorkerState.RUNNING
                or remote.process_stopped
            ):
            ...
            if first:
                ...
                try:
                    if case.action == "cancel_after_running":
                        ack = {
                            "operation": "cancel",
                            "receipt": _receipt(client.cancel(attempt_id), request).model_dump(mode="json"),
                        }
                        ...
                        if (
                            ack.get("action_id") != action_id
                            or ack.get("operation") != "restart"
                            or ack.get("signal_sent") is not True
                            or ack.get("replayed") is not False
                            or not _stable(checked, _snapshot(ack["before"], request))
                            or ack["before"]["supervisor"] != checked["supervisor"]
                        ):
        ...
        while True:
            try:
                ...
                if not _stable(before, after) or not _stable(before, checked):
                    result = _report(
                        binding, action_id, "inconclusive", "EXECUTION_AUTHORITY_CHANGED", intent, ack, checked, receipt
                    )
                    break
                if case.action == "cancel_after_running":
                    ...
                    receipts = [
                        cancelled,
                        receipt,
                        _receipt(after["receipt"], request),
                        _receipt(checked["receipt"], request),
                    ]
                    if any(
                        item.state != WorkerState.CANCELLED
                        or item.failure_kind != "cancelled"
                        or not item.process_stopped
                        ...
                        break
                    if (
                        not _cancel_proof(after, before, request)
                        or not _cancel_proof(checked, before, request)
                        or after["cancellation"] != checked["cancellation"]
                        or after["cancellation_sha256"] != checked["cancellation_sha256"]
                    ):
                        result = _report(
                            binding,
                            action_id,
                            "inconclusive",
                            "CANCELLATION_SIGNAL_EVIDENCE_MISSING",
                            intent,
                            ack,
                            checked,
                            receipt,
                    ...
                    if not after["child_alive"] and not checked["child_alive"]:
                        result = _report(
                            ...
                            action_id,
                            "passed",
                            "EXACT_RUNNING_ATTEMPT_CANCELLED",
                            intent,
                            ack,
                            checked,
                            receipt,
                    ...
                    if (
                        not _running(after, request, clock())
                        or not _running(checked, request, clock())
                        or receipt.state != WorkerState.RUNNING
                        or receipt.process_stopped
                        ...
                        result = _report(
                            ...
                            action_id,
                            "inconclusive",
                            "RECONNECT_DID_NOT_PRESERVE_LIVE_ATTEMPT",
                            intent,
                            ack,
                            checked,
                            receipt,
                    ...
                    if _authority(ledger, request, clock()):
                        result = _report(
                            ...
                            action_id,
                            "passed",
                            "TUNNEL_RECONNECTED_EXACT_ATTEMPT",
                            intent,
                            ack,
                            checked,
                            receipt,
                    ...
                    if (
                        ...
                        result = _report(
                            ...
                            action_id,
                            "inconclusive",
                            "RESTART_DID_NOT_PRESERVE_LIVE_ATTEMPT",
                            intent,
                            ack,
                            checked,
                            receipt,
                        ...
                        break
                    if (
                        after["supervisor"] == checked["supervisor"]
                        and _process_key(after["supervisor"]) != _process_key(before["supervisor"])
                        and _authority(ledger, request, clock())
                    ):
                        result = _report(
                            binding,
                            action_id,
                            "passed",
                            "REPLACEMENT_ADOPTED_EXACT_ATTEMPT",
                            intent,
                            ack,
                            checked,
                            receipt,
...
def _report(binding, action_id, status, reason, intent, ack=None, after=None, receipt=None):
    return {
        "schema_version": 1,
        "kind": "gpu_acceptance_action",
        "action_id": action_id,
        "binding": binding,
        "status": status,
        "reason": reason,
        "before": intent["before"],
        "action_acknowledgment": ack,
        "after": after,
...
def _recorded_proof(report, intent, acknowledgment, request, action):
    ...
    if (
        ...
        or report.get("kind") != "gpu_acceptance_action"
        or report.get("status") != "passed"
        or report.get("compute_started") is not False
        or report.get("approval_consumed") is not False
        or report.get("scientific_evidence") is not False
        or intent.get("binding") != report["binding"]
        or acknowledgment.get("binding") != report["binding"]
        or intent.get("before") != report.get("before")
        or acknowledgment.get("ack") != report.get("action_acknowledgment")
    ):
        return False
    ...
    if action == "cancel_after_running":
        ...
        return (
            ack.get("operation") == "cancel"
            and _cancel_proof(after, before, request)
            and all(
                item.state == WorkerState.CANCELLED and item.failure_kind == "cancelled" and item.process_stopped
                for item in outcomes
            )
        )
    if action == "reconnect_tunnel_after_running":
        return (
            _reconnect_ack(ack, before, request, report["action_id"], report["binding"]["endpoint_identity"])
    ...
    return (
        ack.get("operation") == "restart"
        and ack.get("action_id") == report["action_id"]
        and ack.get("signal_sent") is True
        and ack.get("replayed") is False
        and _stable(before, _snapshot(ack["before"], request))
        and ack["before"]["supervisor"] == before["supervisor"]
        and _process_key(before["supervisor"]) != _process_key(after["supervisor"])
        and _running(after, request, datetime.fromisoformat(after["observed_at"]))
        and receipt.state == WorkerState.RUNNING
        and not receipt.process_stopped
    )
    ...
    try:
        ...
        for name in names:
            ...
            try:
                ...
                if (
                    case is None
                    or binding["plan_sha256"] != digest(plan.model_dump(mode="json"))
                    or binding["action"] != case.action
                    or report["action_id"] != action_id
                    or digest(binding) != "sha256:" + action_id
                ):
                    continue
                request = ExecutionRequest.model_validate(binding["request"])
                job = ledger.get_job(request.job_id)
                actual = Dispatcher(
                    ledger, None, worker_id=job.worker_id, transfer_directory=Path(action_directory)
                )._request(job)
                if (
                    request != actual
                    or request.spec != case.spec
                    or request.spec.experiment_stage.value != "calibration"
                ):
                    continue
                intent, ack = (store.read(action_id, kind) for kind in ("intent", "ack"))
                if intent["action_id"] == ack["action_id"] == action_id and _recorded_proof(
                    report, intent, ack, request, case.action
                ):
    ...
    result["cases"] = [
        {
            "case": case.name,
            "action": case.action,
            "passed": found.get(case.name) is not None,
            "action_id": found.get(case.name),
        }
        for case in required.values()
    ]
...
def main():
    ...
    try:
        ...
        client = WorkerClient(
            settings["worker_url"],
            private_bytes(Path(settings["bearer_secret_file"])).decode().strip(),
            timeout_seconds=5,
        )
