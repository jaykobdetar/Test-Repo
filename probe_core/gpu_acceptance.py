...
class AcceptanceCase(FrozenModel):
    name: Identifier
    action: Literal[
        "wait", "cancel_after_running", "restart_supervisor_after_running", "reconnect_tunnel_after_running"
    ]
...
def fixed_plan(
    model: ModelIdentity, label: str, dataset_revision: str, prompts_sha256: str, prompt_ids: tuple[str, ...]
) -> AcceptancePlan:
    ...
    base = {
        ...
        "inputs": {
            ...
            "prompt_set_hash": prompts_sha256,
            "prompt_ids": prompt_ids,
            "random_seed": 123,
            "generation": {"temperature": 0.0, "max_new_tokens": 4},
        },
    ...
    for name, action, state, kind, operation, limits in definitions:
        values = dict(
            ...
            operation=operation or base["operation"],
            limits=dict(base["limits"], **limits),
        )
        cases.append(
            AcceptanceCase(
                name=name,
...
def fixed_direction_plan(
    ...
    values.update(
        ...
        operation={
            "kind": "steer",
            "target": {"layer": 14, "component": "residual"},
            "positions": ["last"],
    ...
    case = AcceptanceCase(
        name="public-direction-transfer", action="wait", expected_state="COMPLETED", spec=JobSpec.model_validate(values)
    )
...
def collect(
    ...
    *,
    ...
    direction_evidence: dict | None = None,
    ...
    for case in plan.cases:
        ...
        with ledger.read_connection() as connection:
            stopped = connection.execute(
                "SELECT stopped_at FROM attempts WHERE attempt_id=?", (job.attempt_id,)
            ).fetchone()[0]
        ...
        if client is not None:
            ...
            if cancellation_case:
                ...
                receipt.state != (WorkerState.SUCCEEDED if case.expected_state == "COMPLETED" else WorkerState.FAILED)
                or receipt.failure_kind != case.expected_failure_kind
            ):
            ...
            if case.name in LIMIT_FAILURE_CODES:
                ...
                local_deadline = case.name == "hard-deadline" and job.failure_reason == "execution deadline elapsed"
        ...
        if job.state.value == "COMPLETED":
            ...
            if (
                ...
                or manifest.hardware.provider_backend != "runpod"
                or manifest.hardware.gpu_count != 1
                or manifest.software.container_image_digest is None
            ):
                row["reason"] = "canonical GPU/container provenance is absent"
                continue
            directory = ledger.get_artifact_root(job.job_id)
            if case.name == "capture-retention" and "tensors.safetensors" not in {
                item.path for item in manifest.artifacts
            }:
                row["reason"] = "captured tensor artifact is absent"
                continue
            row["retained_artifacts"] = [
                {"path": item.path, "sha256": sha256_file(directory / item.path)} for item in manifest.artifacts
            ]
            if any(
                item["sha256"] != "sha256:" + expected.sha256
                for item, expected in zip(row["retained_artifacts"], manifest.artifacts)
            ):
            ...
            if case.name == "public-direction-transfer":
                try:
                    ...
                    row["direction_transfer"] = verify_direction_evidence(
                        ledger, job, direction_evidence, input_artifact_root, receipt, manifest
                    )
    ...
    return {
        "schema_version": 1,
        "kind": "gpu_runtime_acceptance_observations",
        "model": plan.model.model_dump(mode="json"),
        "case_results_passed": all(row["passed"] for row in reports),
        "cases": reports,
...
def main():
    ...
    if args.command == "plan":
        ...
        result = make_plan(WorkerConfig.model_validate_json(args.worker_config.read_text()), args.label).model_dump(
            mode="json"
        )
    else:
        ...
        with Ledger(args.ledger) as ledger:
            result = (
                ...
                else collect(ledger, plan, client, action_directory=args.action_directory)
            )
