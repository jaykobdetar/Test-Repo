...
class ReplacementBinding(FrozenModel):
    ...
    def fixed_path(self):
        require(self.runner_config_path.startswith('/') and all(
            part not in ('', '.', '..') for part in self.runner_config_path.split('/')[1:]),
...
def submission_predecessor(config, rpc):
    ...
    require(job.get('job_id') == binding.job_id and job.get('state') == 'COMPLETED'
            and job.get('attempt_count') == 1, 'REPLACEMENT_PREDECESSOR_NOT_COMPLETED')
...
def validate_old_request(config, row):
    ...
    require(row.get('worker_id') == b.worker_id and row.get('observed_provider_id') == b.provider_id
            and row.get('approval_id') == b.approval_id and row.get('job_ids') == [b.job_id]
            and row.get('state') == 'STOPPED' and row.get('action') in {'CREATE', 'REPLACE'}
...
def retained_bundle(ledger, binding):
    ...
    require(digest(manifest.model_dump(mode='json')) == binding.manifest_sha256,
            'REPLACEMENT_MANIFEST_CHANGED')
    root = ledger.get_artifact_root(binding.job_id)
    seal = json.loads(read_file(root / '.probe-bundle.json', owner=os.geteuid(), bound=4096))
    require(seal == {'job_id': binding.job_id, 'attempt_id': binding.attempt_id,
                     'manifest_hash': binding.manifest_sha256}, 'REPLACEMENT_BUNDLE_SEAL_INVALID')
    ...
    artifacts = [{'path': item.path, 'sha256': 'sha256:' + item.sha256}
    ...
    require(summary.get('suite') == 'backend_parity_v1' and summary.get('passed') is True
            and summary.get('scientific_evidence') is False, 'REPLACEMENT_PREDECESSOR_PARITY_FAILED')
    return {'manifest_sha256': binding.manifest_sha256, 'artifacts': artifacts,
...
def verify_predecessor(config, plan, ledger, backend, *, owner=0):
    ...
    require('sha256:' + hashlib.sha256(raw).hexdigest() == b.runner_config_sha256,
            'REPLACEMENT_CONFIG_CHANGED')
    previous = RunnerConfig.model_validate_json(raw)
    unchanged = ('service_uid', 'research_uid', 'admin_uid', 'worker_config_path', 'worker_config_sha256',
                 'deployment', 'source_commit', 'expected_worker_price_usd_per_hour', 'max_runtime_seconds',
                 'provider_config_path', 'ssh_identity_file', 'bearer_secret_file')
    require(all(getattr(config, name) == getattr(previous, name) for name in unchanged),
            'REPLACEMENT_SCOPE_CHANGED')
    require(config.trusted_state_directory != previous.trusted_state_directory
            and config.submission_state_directory != previous.submission_state_directory
            and config.plan_path != previous.plan_path, 'REPLACEMENT_STATE_REUSED')
    old_raw = read_file(previous.plan_path, owner=owner)
    require('sha256:' + hashlib.sha256(old_raw).hexdigest() == previous.plan_sha256,
            'REPLACEMENT_PLAN_CHANGED')
    old_plan = AcceptancePlan.model_validate_json(old_raw)
    require(len(old_plan.cases) == 1 and old_plan.cases[0].name == 'backend-parity'
            and old_plan.cases[0].action == 'wait' and old_plan.cases[0].expected_state == 'COMPLETED'
            and old_plan.cases[0].expected_failure_kind is None and old_plan.model == plan.model
    ...
    require(result.get('schema_version') == 1 and result.get('kind') == 'single_public_gpu_calibration'
            and result.get('status') == 'passed' and result.get('stage') == 'completed'
            and result.get('scientific_evidence') is False
            and result.get('approval_consumed_by_runner') is False
            and result.get('case') == 'backend-parity' and result.get('observed_state') == 'COMPLETED'
            and result.get('observed_failure_kind') is None
            and all(result.get(name) == getattr(b, name) for name in
                    ('request_id', 'worker_id', 'provider_id', 'job_id', 'attempt_id', 'manifest_sha256'))
            and result.get('plan_sha256') == previous.plan_sha256,
            'REPLACEMENT_SUCCESSFUL_PREDECESSOR_REQUIRED')
    teardown = result.get('teardown', {})
    require(teardown.get('confirmed') is True and teardown.get('state') == 'ABSENT'
            and teardown.get('provider_id') == b.provider_id, 'REPLACEMENT_PRIOR_DELETION_UNCONFIRMED')
    ...
    require(observations.get('case_results_passed') is True and len(cases) == 1
            and cases[0].get('case') == 'backend-parity' and cases[0].get('passed') is True,
            'REPLACEMENT_PRIOR_RECEIPT_MISSING')
    receipt = ExecutionReceipt.model_validate(cases[0].get('observed_receipt'))
    require(digest(receipt.model_dump(mode='json')) == result.get('worker_receipt_sha256')
            and receipt.job_id == b.job_id and receipt.attempt_id == b.attempt_id
            and receipt.state == ExecutionState.SUCCEEDED and receipt.process_stopped
    ...
    require(job.state == JobState.COMPLETED and job.attempt_id == b.attempt_id and job.worker_id == b.worker_id
            and job.approval_id == b.approval_id and job.attempt_count == 1 and job.retry_count == 0
            and job.spec == old_plan.cases[0].spec, 'REPLACEMENT_PREDECESSOR_NOT_COMPLETED')
    with ledger.read_connection() as connection:
        ...
        jobs = [row[0] for row in connection.execute('SELECT job_id FROM approval_jobs WHERE approval_id=?', (b.approval_id,))]
        count = connection.execute('SELECT count(*) FROM compute_requests WHERE worker_id=?', (b.worker_id,)).fetchone()[0]
    require(old is not None and attempt is not None and approval is not None and count == 1,
    ...
    require(approval['consumed_at'] is not None and approval['ended_at'] is not None
            and approval['ended_at'] >= approval['consumed_at'] and jobs == [b.job_id],
            'REPLACEMENT_PREDECESSOR_APPROVAL_OPEN')
    require(public.get('purpose', 'research') == 'research'
            and public['pod_id'] == b.worker_id and public['batch_hash'] == old['batch_hash']
            ...
            and math.isfinite(approval['deadline']) and math.isfinite(old['deadline'])
    ...
    require(attempt['job_id'] == b.job_id and attempt['worker_id'] == b.worker_id
            and attempt['approval_id'] == b.approval_id and attempt['stopped_at'] is not None
            and approval['ended_at'] >= attempt['stopped_at']
            and attempt['outcome'] == 'completed', 'REPLACEMENT_PREDECESSOR_AUTHORITY_INVALID')
    manifest = ledger.get_manifest(b.job_id)
    require(manifest.model == worker.model and manifest.software.container_image_digest == config.deployment.image_digest
            ...
            and manifest.hardware.provider_backend == 'runpod' and manifest.hardware.gpu_count == 1
            and manifest.run.approval_id == b.approval_id, 'REPLACEMENT_PRIOR_PROVENANCE_CHANGED')
    observed = backend.status(b.worker_id)
    require(observed.state == WorkerState.ABSENT and observed.worker_id == b.worker_id
            and observed.provider_id == b.provider_id and observed.request_key == b.request_id
            and observed.configuration_hash == config.deployment.digest, 'REPLACEMENT_PREDECESSOR_NOT_ABSENT')
    ...
    provider = {'worker_id': observed.worker_id, 'state': observed.state.value,
                'provider_id': observed.provider_id, 'request_key': observed.request_key,
                'configuration_hash': observed.configuration_hash}
    return {'binding': b.model_dump(mode='json'), 'provider': provider,
            'bundle': retained_bundle(ledger, b), 'worker_config_sha256': config.worker_config_sha256,
            'assets_sha256': digest([asset.model_dump(mode='json') for asset in worker.assets])}


def fresh_request(config, request, ledger):
    b = config.replacement
    require(all(request[name] != getattr(b, name) for name in ('request_id', 'worker_id', 'approval_id'))
            and request['observed_provider_id'] != b.provider_id and request['job_ids'] != [b.job_id]
            and request['action'] == 'REPLACE' and request['replaces_worker_id'] == b.worker_id,
            'REPLACEMENT_IDENTITIES_REUSED')
    job = ledger.get_job(request['job_ids'][0])
    require(job.state == JobState.PENDING and job.attempt_id is None and job.attempt_count == 0
            and job.retry_count == 0, 'REPLACEMENT_ALREADY_EXECUTED')


def preflight_record(config, predecessor):
    return {'schema_version': 1, 'kind': 'replacement_preflight',
            'config_sha256': digest(config.model_dump(mode='json')), 'plan_sha256': config.plan_sha256,
            'predecessor': predecessor, 'approval_consumed_by_runner': False}
...
def verify_before_approval(config, plan, ledger, backend, state, *, owner=0):
    ...
    return {'status': 'passed', 'kind': 'replacement_preflight', 'receipt_sha256': digest(record),
...
def read_model_inventory(config, plan, settings, *, deadline, clock=time.time, command=None, owner=0):
    ...
    worker = WorkerConfig.model_validate(load_worker_config(config, plan, owner=owner))
    require(worker.model_directory == '/opt/probe-assets/models/' + worker.model.repo.split('/')[-1]
            + '/' + worker.model.revision_sha, 'REPLACEMENT_MODEL_PATH_INVALID')
    ...
    remote = shlex.join(['/opt/probe-core/venv/bin/python', '-I', '-B', '-c', INVENTORY_SCRIPT,
                         worker.model.repo, thinking, str(seconds)])
    ...
    raw = (command or command_bytes)(['/usr/bin/ssh', *options, '-T', '-p', str(settings['ssh_port']),
                                      'root@' + settings['host'], remote], timeout=seconds)
    require(clock() < deadline and isinstance(raw, str) and len(raw) <= 65536, 'REPLACEMENT_READBACK_DEADLINE')
    observed = decode(raw.encode())
    require(type(observed) is dict and set(observed) == {'schema_version', 'uid', 'gid', 'inventory'}
            and type(observed['schema_version']) is int and observed['schema_version'] == 1
            and type(observed['uid']) is int and observed['uid'] == 10001
            and type(observed['gid']) is int and observed['gid'] == 10001,
    ...
    require(inventory.model == worker.model and inventory.source_lock_hash == lock_hash(lock)
    ...
    return {'schema_version': 1, 'observed_at': clock(), 'receipt': observed,
            'receipt_sha256': digest(observed), 'worker_config_sha256': config.worker_config_sha256}
