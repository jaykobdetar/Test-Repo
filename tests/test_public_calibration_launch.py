...
def test_environment_never_inherits_provider_keys(monkeypatch):
    ...
    for key in ("RUNPOD_API_KEY", "HF_TOKEN", "PYTHONPATH"):
...
class DeletedProvider:
    ...
    def _intent(self, worker):
        return {"provider_id": "owned-pod"}
...
def test_delete_requires_get_404():
    ...
    result = runner.cleanup(provider, "worker")
...
def test_delete_ack_alone_not_success(monkeypatch):
    provider = DeletedProvider()
    provider.request = lambda *args: {"status": "RUNNING"}
    clock = iter([0, 0, 2])
...
def test_creation_failure_still_deletes_owned_intent(tmp_path, monkeypatch):
    ...
    with pytest.raises(FileExistsError):
        runner.run(tmp_path, record, provider)
...
def copied_results(tmp_path):
    ...
    summary = {
        ...
        "input_lengths": [5, 20],
        "checks": [{"passed": True, "rtol": 0, "atol": 0, "max_absolute_error": 0} for _ in range(25)]
        + [{"passed": True} for _ in range(4)],
    }
    (tmp_path / "summary.json").write_text(json.dumps(summary))
    header = json.dumps(
        {str(i): {"dtype": "F32", "shape": [1], "data_offsets": [4 * i, 4 * i + 4]} for i in range(9)}
    ).encode()
    ...
    manifest = {
        ...
        "software": {
            ...
            "probe_mcp_git_commit": config["code_git_commit"],
        },
        ...
        "artifacts": [
            ...
            for name in ("summary.json", "tensors.safetensors")
        ],
    }
...
def test_copied_result_mutation_rejected(copied_results, mutation):
    ...
    if mutation == "artifact":
        with (directory / "tensors.safetensors").open("ab") as stream:
            stream.write(b"changed")
    else:
        p = directory / "standalone-manifest.json"
        m = json.loads(p.read_text())
        if mutation == "image":
            m["software"]["container_image_digest"] = "sha256:" + "d" * 64
        if mutation == "configuration":
            m["config_sha256"] = "sha256:" + "d" * 64
        if mutation == "deadline":
            m["absolute_deadline"] = "2099-01-01T00:00:00+00:00"
