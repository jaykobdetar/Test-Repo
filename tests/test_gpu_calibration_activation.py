...
def gate():
    return dict(
        passed=True,
        checks={f"check-{i}": True for i in range(26)},
        failure_codes=[],
        check_count=26,
        passed_count=26,
...
def package(tmp_path):
    ...
    manifest = dict(
        schema_version=1,
        storage_checked_at=NOW.isoformat(),
        upgrade_directory="/opt/probe-core/upgrades/" + "a" * 64,
        identity_checker_sha256="b" * 64,
        files={name: digest(raw) for name, raw in bodies.items()},
    )
...
@pytest.mark.parametrize(
    "kind",
    ["wrong_pin", "modified_file", "symlink", "hardlink", "extra_file_entry", "missing_worker", "stale", "future"],
)
def test_package_refusals_leave_every_input_unchanged(package, kind):
    ...
    if kind == "wrong_pin":
        ...
        if kind == "extra_file_entry":
            ...
            manifest["storage_checked_at"] = (
                NOW + timedelta(seconds=1) if kind == "future" else NOW - timedelta(days=2)
            ).isoformat()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("normal_research_audit_events", 0),
...
def test_exact_real_units_pass_and_other_executable_is_refused(mode):
    ...
    for changed in (
        raw.replace(b" -I -m ", b" -m "),
        raw.replace(b"NoNewPrivileges=yes", b"NoNewPrivileges=no"),
...
def test_private_key_descriptor_fallback_supports_real_ssh_keygen(tmp_path, monkeypatch):
    ...
    try:
        ...
        result = subprocess.run(
            ["/usr/bin/ssh-keygen", "-y", "-P", "", "-f", f"/proc/self/fd/{fd}"],
            pass_fds=(fd,),
            capture_output=True,
            check=True,
        )
...
def orchestration(tmp_path, monkeypatch):
    for name, path in {
        ...
        "CONFIG": "etc/core",
        "UNITS": "etc/units",
        "PUBLIC": "etc/calibration",
        "SUBMIT": "var/submit",
        "STATE": "var/core/acceptance",
    }.items():
    ...
    put(
        upgrade / "upgrade-release.json",
        a.encoded(dict(wheel_sha256="a" * 64, identity_checker_sha256=digest(checker))),
    )
    ...
    manifest = dict(
        schema_version=1,
        storage_checked_at=NOW.isoformat(),
        upgrade_directory=str(upgrade),
        identity_checker_sha256=digest(checker),
        files={name: digest(raw) for name, raw in bodies.items()},
    )
    ...
    monkeypatch.setattr(
        a.pwd,
        "getpwnam",
        lambda name: SimpleNamespace(pw_uid={"probe-trusted": 994, "probe-research": 993, "human": 1000}[name]),
    )
    monkeypatch.setattr(a.grp, "getgrnam", lambda name: SimpleNamespace(gr_gid=982))
    monkeypatch.setattr(
        a, "configurations", lambda *args: (b'{"new_provider":true}', b'{"new_research":true}', "ssh-ed25519 AAAA")
    )
...
def test_partial_activation_closes_services_durably(orchestration, monkeypatch, failure):
    ...
    assert disabled["sandbox_image"] is None and disabled["policy"] == {
        "discovery_datasets": [],
        "allow_calibration": False,
    }
    assert all(
        (a.UNITS / (name + ".d") / "50-probe-calibration.conf").read_bytes() == a.GUARD
...
def typed_inputs(monkeypatch):
    ...
    model.update(
        ...
        revision_sha="ea980cb0a6c2ae4b936e82123acc929f1cec04c1",
    ...
    inputs = data["inputs"]
    inputs.update(
        dataset_revision=a.DATASET,
        prompt_ids=["public-short", "public-long"],
        generation={"max_new_tokens": 4, "temperature": 0.0},
    )
    spec = dict(
        experiment_stage="calibration",
        idempotency_key="public-test",
        model=model,
        inputs=inputs,
        operation={"kind": "backend_parity"},
        limits={"max_runtime_seconds": 240, "max_output_bytes": 33554432},
    )
    plan = dict(
        label="public-test",
        model=model,
        cases=[dict(name="backend-parity", action="wait", expected_state="COMPLETED", spec=spec)],
    )
    public = "ssh-ed25519 " + "A" * 68
    old_launch = dict(
        image_repository="ghcr.io/jaykobdetar/probe-mcp-diagnostic",
        ports=["22/tcp"],
        environment={"PUBLIC_KEY": public},
    )
    ...
    old_provider = dict(
        state_path="/var/lib/probe-provider/runpod.sqlite",
        api_key_file="/etc/probe-core/runpod-api-key",
        launch=old_launch,
        storage_rates={"checked_at": (NOW - timedelta(days=3)).isoformat()},
    ...
    worker = dict(
        model_directory="/opt/probe-assets/models/Qwen3-1.7B-Base/" + model["revision_sha"],
        model=model,
    ...
    bodies = {
        "plan.json": a.encoded(plan),
        "launch.json": a.encoded(launch),
        "worker.json": a.encoded(worker),
        "worker-token": b"x" * 48,
    }
    runner = dict(
        service_uid=994,
        research_uid=993,
        admin_uid=1000,
        plan_path="/etc/probe-calibration/plan.json",
        plan_sha256="sha256:" + digest(bodies["plan.json"]),
        worker_config_path="/etc/probe-calibration/worker.json",
        worker_config_sha256="sha256:" + digest(bodies["worker.json"]),
        ...
        deployment={
            ...
            "region": "EU-CZ-1",
            "volume_gb": 0,
            "image_repository": a.REPOSITORY,
            "launch_config_hash": typed_launch.digest,
    ...
    return SimpleNamespace(
        ...
        research=old_research,
        manifest={"storage_checked_at": NOW.isoformat()},
        users=(994, 993, 1000),
    )
...
def test_typed_configuration_preserves_all_unrelated_existing_fields(typed_inputs):
    s = typed_inputs
    provider, research, public = a.configurations(
        s.bodies, a.encoded(s.provider), a.encoded(s.research), s.manifest, s.users
    )
    expected_provider = deepcopy(s.provider)
    expected_provider.update(
        launch=json.loads(s.bodies["launch.json"]), max_runtime_seconds=900, ready_timeout_seconds=300
    )
    from probe_core.runpod_provider import RunPodLaunchConfig

    expected_provider["launch"] = RunPodLaunchConfig.model_validate_json(s.bodies["launch.json"]).model_dump(
        mode="json"
    )
...
def test_configuration_bounds_fail_before_mutation(typed_inputs, change):
    ...
    if change == "api_key":
        ...
        field, value = {
            "disk": ("container_disk_gb", 21),
            "cuda": ("min_cuda_version", "12.8"),
            "args": ("args", "bash"),
            "start_ssh": ("start_ssh", True),
            "ssh_key": ("environment", {"PUBLIC_KEY": "ssh-ed25519 " + "B" * 68}),
        }[change]
        launch[field] = value
        ...
        worker["container_image_digest" if change == "worker_image" else "tensor_directory"] = (
            "sha256:" + "f" * 64 if change == "worker_image" else "/etc/escape"
        )
        ...
        if change == "other_gpu":
            ...
            field, value = {
                "worker_path": ("worker_config_path", "/etc/elsewhere.json"),
                "worker_hash": ("worker_config_sha256", "sha256:" + "f" * 64),
                "runtime": ("max_runtime_seconds", 899),
                "plan_hash": ("plan_sha256", "sha256:" + "f" * 64),
                "identity": ("service_uid", 995),
            }[change]
            runner[field] = value
        s.bodies["acceptance.json"] = a.encoded(runner)
