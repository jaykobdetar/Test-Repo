...
def asset(data=b"verified public test data", *, inline=False, path="models/base/model.safetensors"):
    ...
    result.update({"inline_base64":base64.b64encode(data).decode()} if inline else {"url":"https://huggingface.co/public/pinned/model.safetensors"})
    return result
...
class Client:
    ...
    def open(self, request, timeout):
        self.requests.append((request,timeout)); return self.response
...
def test_streamed_hash_size_permissions_and_second_readback(local):
    implementation,root,path=local; payload=b"small streamed fixture"; entry=asset(payload)
    ...
    report=implementation.stage_asset(root,entry,Clock(),SimpleNamespace(open=lambda *_a,**_k:pytest.fail("existing verified file fetched again")))
...
def prepare_partial(implementation,root,path,entry,prefix):
    ...
    partial.write_bytes(prefix); partial.chmod(0o600)
    sidecar=target.parent/("."+name+".partial.json")
    sidecar.write_text(json.dumps({"schema_version":1,"asset_id":implementation.asset_identity(entry)})); sidecar.chmod(0o600)
    return target,partial,sidecar


def test_resume_requires_exact_range_and_rehashes_entire_file(local):
    implementation,root,path=local; payload=b"abcdefghij"; entry=asset(payload)
...
@pytest.mark.parametrize("status,length,content_range", [(200,"10",None),(206,"6","bytes 0-5/10"),
    (206,"6","bytes 4-9/11"),(206,"5","bytes 4-9/10"),(206,"6","bytes 4-8/10"),(206,"6",None)])
...
def test_corrupt_partial_cannot_pass_using_only_downloaded_suffix(local):
    ...
    with pytest.raises(implementation.StageError,match="AssetHashMismatch"):
        implementation.stage_asset(root,entry,Clock(),Client(Response(payload[4:],status=206,headers={"Content-Length":"6","Content-Range":"bytes 4-9/10"})))
    assert not target.exists() and partial.read_bytes()==b"XXXXefghij"
    assert stat.S_IMODE(partial.stat().st_mode)==0o600


@pytest.mark.parametrize("data,code", [(b"abc","DownloadIncomplete"),(b"abcdefghijk","DownloadTooLarge"),(b"XXXXXXXXXX","AssetHashMismatch")])
def test_download_length_and_hash_fail_before_publication(local,data,code):
    implementation,root,path=local; entry=asset(b"abcdefghij")
...
def test_deadline_interruption_keeps_known_resumable_private_prefix(local,monkeypatch):
    ...
    class Interrupted(Response):
        def read(self,count=-1):
            if self.tell()>=4: raise implementation.StageError("TransferDeadlineReached")
...
def test_inline_asset_is_validated_offline_and_published_without_http(local):
    implementation,root,path=local; entry=asset(b"public dataset",inline=True,path="datasets/calibration.json")
    implementation.validate_manifest({"schema_version":1,"assets":[entry]})
    report=implementation.stage_asset(root,entry,Clock(),SimpleNamespace(open=lambda *_a,**_k:pytest.fail("inline asset used network")))
    assert report["bytes"]==14 and (path/entry["path"]).read_bytes()==b"public dataset"


@pytest.mark.parametrize("bad_path", ["/models/a","models/../config/token","models//a","models/.private/a",
                                    "models/a/./b","config/worker.json","datasets","models/a\\b"])
def test_manifest_rejects_escape_and_nonasset_paths(bad_path):
    implementation=module(); entry=asset(path=bad_path)
...
def test_declared_total_and_inline_integrity_are_checked_before_staging():
    implementation=module(); first=asset(path="models/a"); second=asset(path="models/b")
    ...
    entry=asset(b"abc",inline=True); entry["inline_base64"]=base64.b64encode(b"XYZ").decode()
...
def test_symlinked_parent_or_partial_never_writes_outside_workspace(local):
    implementation,root,path=local; outside=path/"outside"; outside.mkdir()
    ...
    (path/"models").unlink()
    entry=asset(b"abcdefghij"); _,partial,_=prepare_partial(implementation,root,path,entry,b"abcd")
    untouched=outside/"untouched"; untouched.write_text("untouched")
    partial.unlink(); partial.symlink_to(untouched)
    with pytest.raises(OSError):
...
@pytest.mark.parametrize("url", ["http://example.com/file","https://user:secret@example.com/file","file:///etc/passwd","https://example.com:8443/file"])
...
def test_error_report_never_includes_secret_url(local,monkeypatch):
    ...
    def fail(*_a,**_k): raise urllib.error.URLError("https://example.com/?secret=do-not-print")
...
@pytest.mark.parametrize("has_mount,different_device,filesystem,passed", [(True,True,"nfs4",True),
    (False,True,"nfs4",False),(True,False,"nfs4",False),(True,True,"tmpfs",False),(True,True,"overlay",False)])
def test_workspace_requires_exact_separate_persistent_mount(local,monkeypatch,has_mount,different_device,filesystem,passed):
    implementation,root,path=local
    monkeypatch.setattr(implementation,"VOLUME",path)
    info=os.fstat(root)
    text=(f"40 20 {os.major(info.st_dev)}:{os.minor(info.st_dev)} / {path} rw - {filesystem} volume rw\n" if has_mount else "")
    original_read=implementation.read_file; original_stat=os.stat
    monkeypatch.setattr(implementation,"read_file",lambda fd,name,limit:text.encode() if name=="mountinfo" else original_read(fd,name,limit))
    def fake_stat(target,*args,**kwargs):
        if target=="/": return SimpleNamespace(st_dev=info.st_dev+1 if different_device else info.st_dev)
        return original_stat(target,*args,**kwargs)
    monkeypatch.setattr(implementation.os,"stat",fake_stat)
    if passed:
        verified=implementation.validate_volume(path); os.close(verified)
    else:
        with pytest.raises(implementation.StageError): implementation.validate_volume(path)


def test_complete_partial_is_rehashed_then_published_without_network(local):
    implementation,root,path=local; data=b"already complete"; entry=asset(data)
    target,partial,_=prepare_partial(implementation,root,path,entry,data)
    report=implementation.stage_asset(root,entry,Clock(),SimpleNamespace(open=lambda *_a,**_k:pytest.fail("complete partial fetched again")))
...
def test_existing_bad_final_is_never_replaced(local):
    implementation,root,path=local; entry=asset(b"expected")
    parent,name=implementation.asset_parent(root,entry["path"]); os.close(parent)
    final=path/entry["path"]; final.write_bytes(b"WRONG"); final.chmod(0o440)
...
def test_normalized_modes_survive_private_launcher_umask(local):
    implementation,root,path=local; entry=asset(b"public",inline=True)
...
def test_http_client_disables_environment_proxies(monkeypatch):
    ...
    monkeypatch.setattr(implementation.urllib.request,"ProxyHandler",lambda options:seen.append(options) or "proxy-disabled")
