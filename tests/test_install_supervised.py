...
def capsule(tmp_path):
    ...
    files = {
        "install-supervised.py": SCRIPT.read_bytes(),
    ...
    for name in installer.SIDE_FILES - {"files.json"}:
        source = (
            ...
            / (
                "probe_core/"
                if name in {"ssh_job_client.py", "supervised_runner.py"}
                else "deploy/gpu/"
                if name == "public-job.py"
                else "deploy/"
            )
            / name
        )
        files["sidecar/" + name] = source.read_bytes()
    files["sidecar/files.json"] = encoded(
        {name: "sha256:" + installer.sha(files["sidecar/" + name]) for name in installer.MODULES}
    )
    ...
    config = {
        "helper_path": installer.SIDECAR + "/public-job.py",
    ...
    manifest.write_bytes(
        encoded({"schema_version": 1, "files": {name: installer.sha(raw) for name, raw in files.items()}})
    )
    return SimpleNamespace(
        folder=folder,
        manifest=manifest,
        checksum=installer.sha(manifest.read_bytes()),
        files=files,
        case=case,
        config=config,
    )
...
@pytest.mark.parametrize(
    "mutation", ["manifest", "file", "extra", "symlink", "hardlink", "directory_symlink", "unknown_path"]
)
def test_capsule_refuses_unpinned_or_unsafe_input(tmp_path, mutation):
    offer = capsule(tmp_path)
    target = offer.folder / "sidecar/public-job.py"
    if mutation == "manifest":
        offer.manifest.write_bytes(offer.manifest.read_bytes() + b" ")
    if mutation == "file":
        target.write_bytes(b"changed")
    if mutation == "extra":
        (offer.folder / "unreviewed-token").write_bytes(b"not a credential, but forbidden")
    if mutation == "symlink":
        target.unlink()
    ...
    if mutation == "unknown_path":
        ...
        offer.manifest.write_bytes(encoded(manifest))
        offer.checksum = installer.sha(offer.manifest.read_bytes())
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("plan_path", "/etc/probe-core/runpod.json"),
        ("worker_config_path", "/etc/probe-core/worker-token"),
        ("helper_sha256", "sha256:" + "a" * 64),
        ("submission_state_directory", "/var/lib/probe-core"),
        ("trusted_state_directory", "/root"),
        ("service_uid", 0),
        ("research_uid", True),
    ],
)
...
class FakeInstaller(installer.Installer):
    ...
    def accounts(self, cases):
        ...
        return {
            name: SimpleNamespace(pw_uid=OWNER, pw_gid=os.getegid())
...
@pytest.mark.parametrize(
    "location", ["sidecar/public-job.py", "configs/suite.json", "units/probe-supervised-run.service"]
)
...
def test_failed_account_preflight_never_reloads_or_starts_units(tmp_path, monkeypatch):
    ...
    def fail(_):
        raise installer.InstallError("fixture-preflight-failed")
...
def test_trusted_preflight_compares_installed_launch_without_provider_call(tmp_path, monkeypatch, capsys, matches):
    ...
    def load(path):
        ...
        return SimpleNamespace(
            launch=SimpleNamespace(
                digest="sha256:expected" if matches else "sha256:changed", image_repository="ghcr.io/worker"
            )
        )
    ...
    source = installer.PREFLIGHT.replace(
        "pathlib.Path('/opt/probe-core/supervised')", f"pathlib.Path({str(tmp_path)!r})"
    )
    if matches:
...
def test_runtime_checks_real_service_groups_and_process_ids(tmp_path):
    ...
    accounts = {
        "probe-trusted": SimpleNamespace(pw_uid=991),
        "probe-research": SimpleNamespace(pw_uid=992),
