...
def test_unreported_exit_uses_observed_deadline_or_signal_evidence(
    tiny_bundle, make_request, tmp_path, expired, exitcode, kind, code
):
    ...
    (directory / "process.json").write_text(
        json.dumps(
            {
                "pid": 999999999,
                "identity": "absent-fixture",
                "boot_id": "absent-fixture",
                "deadline": time.time() + (-1 if expired else 60),
                "monotonic_deadline": time.monotonic() + (-1 if expired else 60),
                "cgroup": None,
            }
        )
    )
