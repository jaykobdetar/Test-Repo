...
from test_gpu_acceptance_runner import (
    setup,
    runpod,
    manifest_data,
    approve_fixture,
...
def reconnect_worker(worker):
    ...
    def reconnect(request, action_id, *, expected_before):
        ...
        return {
            ...
            "action_id": action_id,
            "replayed": False,
            "before": deepcopy(expected_before),
            "endpoint_identity": worker.endpoint_identity,
            "previous_transport": {"pid": 201, "exit_status": -15, "local_port": 45678},
            "replacement_transport": {"pid": 202, "local_port": 45678},
            "renewed_at": worker.clock().isoformat(),
            "previous_lease": worker.clock().timestamp() + 30,
            "renewed_lease": worker.clock().timestamp() + 30,
...
def test_completion_before_reconnect_observation_stays_inconclusive(scenario, manifest_data):
    ...
    def completes(*args, **kwargs):
        ...
        worker.receipt = ExecutionReceipt(
            job_id=worker.request.job_id,
            attempt_id=worker.request.attempt_id,
            state="SUCCEEDED",
            started_at=worker.clock(),
            finished_at=worker.clock(),
...
def test_reconnect_collector_rejects_tampered_durable_proof(scenario, fault):
    ...
    if fault == "missing_ack":
        ...
        if fault == "endpoint":
            ack["endpoint_identity"] = "sha256:" + "f" * 64
        elif fault == "old_alive":
            ack["previous_transport"]["exit_status"] = None
        elif fault == "same_pid":
            ack["replacement_transport"]["pid"] = ack["previous_transport"]["pid"]
        elif fault == "local_port":
            ack["replacement_transport"]["local_port"] += 1
        elif fault == "deadline":
            ack["execution_deadline"] += 1
        elif fault == "supervisor":
            body["after"]["supervisor"]["pid"] += 1
        elif fault == "child":
            body["after"]["child"]["monotonic_deadline"] += 1
        elif fault == "replayed":
            ack["replayed"] = True
        else:
            body["after"]["request"]["attempt_id"] = "different-attempt"
        ...
        ack_body = json.loads(ack_path.read_bytes())
        ack_body["ack"] = ack
