...
def cancellation_execution(tmp_path, clock, job_factory):
    ...
    plan = AcceptancePlan(label="cancellation-acceptance", model=spec.model, cases=(
        AcceptanceCase(name="cancel-running", action="cancel_after_running",
                       expected_state="FAILED", expected_failure_kind="cancelled", spec=spec),
    ))
...
def cancelled_receipt(job, clock, **changes):
    values = dict(job_id=job.job_id, attempt_id=job.attempt_id, state="CANCELLED",
                  started_at=clock(), finished_at=clock(), failure_kind="cancelled", process_stopped=True)
...
def test_completed_before_cancel_receipt_cannot_pass(cancellation_execution, manifest_data, tmp_path):
    ...
    endpoint = ReceiptEndpoint(cancelled_receipt(job, clock, state="SUCCEEDED", failure_kind=None,
