...
def run_case(s, **overrides):
    endpoint = Endpoint(s)
    args = dict(clock=lambda: s.clock().timestamp(), sleep=lambda _: None,
        endpoint=lambda *_: {}, tunnel_factory=Tunnel, client_factory=lambda *_a, **_k: endpoint,
    ...
    with runner.State(s.config.trusted_state_directory) as state:
        result = runner.run(s.config, s.plan, s.ledger, s.backend,
            SimpleNamespace(stop_gpu=s.controller.stop_gpu), state, **args)
...
def predecessor(setup):
    ...
    binding = replacement.ReplacementBinding(
        **{name: result[name] for name in ('request_id', 'worker_id', 'provider_id', 'job_id', 'attempt_id', 'manifest_sha256')},
        approval_id=request['approval_id'], runner_config_path=str(config_path),
    ...
    s.config = s.config.model_copy(update={
        'plan_path': str(plan_path), 'plan_sha256': 'sha256:' + hashlib.sha256(plan_path.read_bytes()).hexdigest(),
        'submission_state_directory': str(s.root / 'replacement-submit'),
        'trusted_state_directory': str(s.root / 'replacement-run'), 'replacement': binding})
    s.old = old
    with runner.State(s.config.trusted_state_directory) as state:
...
def approve_replacement(s):
    ...
    watcher = StopWatchdog(s.ledger.path, StopOnlyBackend(s.backend),
        state_path=s.root / 'watchdog' / 'state.sqlite', health_path=s.root / 'watchdog' / 'health.json', clock=s.clock)
    ...
    return s.controller.admin_dispatch('approve', {'request_id': queued['request_id'], 'price_ceiling_usd_per_hour': .8})
...
def test_real_approved_replacement_rehashes_old_bundle_then_fresh_parity_and_deletes(predecessor):
    ...
    def observe(config, plan, settings, *, deadline, clock):
        ...
        return {'schema_version': 1, 'receipt_sha256': 'sha256:' + 'f' * 64,
                'fixture_readback': True}
    ...
    with runner.State(s.config.trusted_state_directory) as state:
        again = runner.run(s.config, s.plan, s.ledger, s.backend, None, state,
                           replacement_reader=lambda *_: pytest.fail('must not replay'))
    assert again == result and len(s.http.purchases) == 2


@pytest.mark.parametrize('kind', ['raw_config', 'result', 'observation', 'artifact', 'missing_artifact', 'seal', 'symlink', 'hardlink'])
...
@pytest.mark.parametrize('field,value', [('provider_id', 'wrong-pod'), ('approval_id', 'wrong-approval'),
                                       ('worker_id', 'wrong-worker'), ('attempt_id', 'wrong-attempt')])
...
@pytest.mark.parametrize('status_change', [{'state': WorkerState.STOPPED}, {'provider_id': 'wrong'},
                                         {'request_key': None}, {'configuration_hash': None}])
...
@pytest.mark.parametrize('change', [{'source_commit': 'd' * 40}, {'expected_worker_price_usd_per_hour': .75},
...
def test_replacement_preserves_exact_worker_provenance_and_price(predecessor, change):
    ...
    with pytest.raises(replacement.ReplacementError, match='SCOPE_CHANGED'):
        replacement.verify_predecessor(s.config.model_copy(update=change), s.plan, s.ledger, s.backend, owner=os.geteuid())
...
def test_startup_only_failure_cannot_become_successful_predecessor_even_with_new_result_pin(predecessor):
    ...
    config = s.config.model_copy(update={'replacement': s.config.replacement.model_copy(update={'result_sha256': runner.digest(body)})})
...
def test_replacement_failure_after_fresh_parity_cannot_leave_passed_status(predecessor):
    ...
    result, _ = run_case(s, client_factory=lambda *_a, **_k: client,
...
def test_existing_config_digests_stay_unchanged_when_no_replacement(setup):
    ...
    assert runner.matches_request({'action': 'CREATE', 'job_ids': ['j'],
        'configuration_hash': setup.config.deployment.digest,
        'configuration': setup.config.deployment.model_dump(mode='json'),
        'max_runtime_seconds': 900, 'infrastructure': None, 'replaces_worker_id': None}, setup.config, 'j')
...
def asset_readback(setup, monkeypatch):
    ...
    root = s.root / 'model'
    root.mkdir()
    contents = {'config.json': encoded({'model_type': 'qwen3', 'num_hidden_layers': 28,
        'num_attention_heads': 16, 'num_key_value_heads': 8, 'hidden_size': 2048,
        'head_dim': 128, 'vocab_size': 151936}), 'tokenizer.json': b'{}',
        'tokenizer_config.json': b'{}', 'model.safetensors': b'synthetic, never loaded'}
    for name, body in contents.items():
        (root / name).write_bytes(body)
    lock = ModelLock(repo='Qwen/Qwen3-1.7B-Base', revision=s.plan.model.revision_sha,
        source_url='https://example.invalid/fixture', files=tuple(
    ...
    worker.update(model=data['model'], assets=[entry.model_dump(mode='json') for entry in actual.assets],
        model_directory='/opt/probe-assets/models/Qwen3-1.7B-Base/' + lock.revision)
    worker_path.write_bytes(encoded(worker))
    s.config = s.config.model_copy(update={'worker_config_sha256': 'sha256:' + hashlib.sha256(worker_path.read_bytes()).hexdigest()})
    ...
    s.settings = {'host': '213.192.2.71', 'ssh_port': 40125,
                  'identity_file': s.root/'key', 'known_hosts_file': s.root/'known-hosts'}
...
def test_remote_readback_matches_real_file_hashes_fixed_command_identity_and_deadline(asset_readback):
    ...
    result = replacement.read_model_inventory(s.config, s.plan, s.settings,
        deadline=1042, clock=lambda: 1000, command=command, owner=os.geteuid())
...
@pytest.mark.parametrize('fault', ['uid', 'gid', 'extra', 'hash', 'missing', 'duplicate', 'model', 'lock', 'invalid_json', 'too_large'])
def test_inventory_receipt_cannot_substitute_identity_model_or_hashes(asset_readback, fault):
    ...
    with pytest.raises(ValueError):
        replacement.read_model_inventory(s.config, s.plan, s.settings,
            deadline=1042, clock=lambda: 1000, command=lambda *_a, **_k: raw, owner=os.geteuid())
...
def test_inventory_never_starts_or_accepts_after_original_cutoff(asset_readback):
    ...
    with pytest.raises(replacement.ReplacementError, match='READBACK_DEADLINE'):
        replacement.read_model_inventory(s.config, s.plan, s.settings, deadline=1000, clock=lambda: 1000,
            command=lambda *_a, **_k: pytest.fail('deadline already expired'), owner=os.geteuid())
    values = iter([1000, 1042])
    with pytest.raises(replacement.ReplacementError, match='READBACK_DEADLINE'):
        replacement.read_model_inventory(s.config, s.plan, s.settings, deadline=1042, clock=lambda: next(values),
            command=lambda *_a, **_k: canonical_json(s.inventory), owner=os.geteuid())
...
def test_fresh_absence_lookup_timestamps_do_not_invalidate_preflight(predecessor):
    ...
    def status(worker_id):
        ...
        return SimpleNamespace(worker_id=original.worker_id, state=original.state, provider_id=original.provider_id,
            request_key=original.request_key, configuration_hash=original.configuration_hash,
