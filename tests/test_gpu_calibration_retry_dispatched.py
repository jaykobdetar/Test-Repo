...
def stopped_failure(setup, monkeypatch):
    ...
    result = dict(
        ...
        reason="CALIBRATION_DID_NOT_PASS",
    ...
    manifest = dict(
        ...
        expected_failure_stage="artifact_collection",
        expected_failure_reason="CALIBRATION_DID_NOT_PASS",
        failed_result_canonical_sha256=r.digest(r.encoded(result)),
    )
    operation = r.Recovery(
        SimpleNamespace(Activation=lambda: None, write_file=lambda path, raw, **_: put(path, raw)),
        None,
        manifest,
        {},
        "a" * 64,
    )
    operation.work = s.root / "recovery"
    ...
    exec(
        ...
        + checker.STATE_READER,
        namespace,
    )

    def idle():
        value = namespace["idle_history_snapshot"](
            s.root / "research.sqlite", s.backend.path, expected=operation.operation.baseline
        )
        if operation.operation.baseline is None:
            operation.operation.baseline = value
        return value
...
def test_schema6_refuses_unproven_or_mismatched_execution(stopped_failure, fault):
    ...
    if fault == "active_job":
        ...
        snapshot["attempts"].append(dict(attempt, attempt_id="extra"))
    elif fault == "retry":
        job["retry_count"] = 1
    elif fault == "count":
        job["attempt_count"] = 2
    elif fault in {"attempt_id", "job_id"}:
        attempt[fault] = "changed"
    elif fault == "worker":
        attempt["worker_id"] = "changed"
    elif fault == "approval":
        attempt["approval_id"] = "changed"
    elif fault == "outcome":
        attempt["outcome"] = "scientific"
    elif fault == "reason":
        job["failure_reason"] = "different"
    elif fault == "kind":
        job["failure_kind"] = "scientific"
    elif fault == "number":
        attempt["attempt_number"] = 2
    elif fault == "deadline":
        attempt["execution_deadline"] = snapshot["requests"][0]["deadline"] + 1
    elif fault == "nan_stop":
        attempt["stopped_at"] = float("nan")
    elif fault == "early_stop":
        attempt["stopped_at"] = attempt["dispatched_at"] - 1
    elif fault == "open_approval":
        snapshot["approvals"][0]["ended_at"] = None
    elif fault == "pod":
        snapshot["provider_absent"] = False
    elif fault == "other_work":
        snapshot["other_unfinished_jobs"] = 1
    elif fault == "unconfirmed":
        snapshot["unconfirmed_attempts"] = 1
    elif fault == "request":
        snapshot["requests"][0]["state"] = "RUNNING"
    elif fault.startswith("result_"):
        op.result[
            {"result_stage": "stage", "result_reason": "reason", "result_attempt": "attempt_id", "result_pin": "extra"}[
                fault
            ]
        ] = "changed"
        if fault != "result_pin":
            op.m["failed_result_canonical_sha256"] = r.digest(r.encoded(op.result))
    elif fault == "bound_approval":
        op.bound["approval_id"] = "changed"
    else:
        value = json.loads(snapshot["approvals"][0]["document"])
        value["pod_id"] = "changed"
        snapshot["approvals"][0]["document"] = json.dumps(value)
    with pytest.raises(r.RetryError):
        op.verify_failure(snapshot)
...
def test_schema6_detects_history_drift_between_preservation_reads(stopped_failure):
    ...
    def snapshot():
        ...
        value = f.snapshot()
        if count > 1:
            value["jobs"][0]["updated_at"] += 1
        return value

    f.operation.snapshot = snapshot
    with pytest.raises(r.RetryError, match="FAILED_HISTORY_CHANGED"):
        f.operation.close_failed()
...
@pytest.mark.parametrize(
    "fault", [None, "missing_attempt", "extra_attempt_field", "stage", "reason", "missing_worker", "equal_old_schema"]
)
def test_schema6_manifest_exact_fields_and_same_controller_allowed(tmp_path, monkeypatch, fault):
    ...
    manifest = dict(
        ...
        previous_upgrade=baseline,
        target_upgrade=baseline,
        previous_activation_manifest_sha256="d" * 64,
        previous_retry_manifest_sha256="e" * 64,
        previous_retry_record_sha256="f" * 64,
        expected_failure_reason="CALIBRATION_DID_NOT_PASS",
        expected_failure_stage="artifact_collection",
        failed_result_canonical_sha256="1" * 64,
        retry_id="2" * 32,
    ...
    if fault == "missing_attempt":
        del manifest["failed_attempt"]
    elif fault == "extra_attempt_field":
        manifest["failed_attempt"]["retry"] = True
    elif fault == "stage":
        ...
        del manifest["failed_attempt"]
        del manifest["files"]["worker.json"]
        manifest["expected_failure_stage"] = "verified_worker_startup"
        manifest["expected_failure_reason"] = "REQUEST_CHANGED"
...
def sixth_retry(retarget_retry):
    ...
    worker["code_git_commit"] = "8" * 40
    worker["container_image_digest"] = "sha256:" + "9" * 64
    s.new["worker.json"] = r.encoded(worker)
    config = json.loads(s.new["acceptance.json"])
    config["source_commit"] = worker["code_git_commit"]
...
def test_schema6_worker_replacement_cannot_change_other_scope(sixth_retry, field):
    ...
    if field in {"model", "operation"}:
        ...
        if field == "model":
            value["cases"][0]["spec"]["model"]["revision_sha"] = "7" * 40
        else:
            value["cases"][0]["spec"]["operation"] = {"kind": "generate"}
        s.new["plan.json"] = r.encoded(value)
    elif field in {"runtime", "region", "price"}:
        value = json.loads(s.new["acceptance.json"])
        if field == "runtime":
            value["max_runtime_seconds"] = 800
        elif field == "region":
            value["deployment"]["region"] = "EU-CZ-1"
        else:
            value["expected_worker_price_usd_per_hour"] = 0.75
        s.new["acceptance.json"] = r.encoded(value)
    else:
        value = json.loads(s.new["worker.json"])
        if field == "worker_limit":
            value["max_tensor_bytes"] = 123456
        else:
            value["container_image_digest"] = json.loads(old["worker.json"])["container_image_digest"]
        s.new["worker.json"] = r.encoded(value)
    with pytest.raises(r.RetryError):
        r.validate_replacement(old, s.new, s.manifest["retry_id"], schema_version=6)
...
def test_no_reinstall_preparation_requires_local_gates_before_any_publication(prepare_case, gate_failure):
    ...
    def gates():
        ...
        if gate_failure:
            raise r.RetryError("BACKUP_NOT_VERIFIED")
        return {"before.tar": "a" * 64, "backup.json": "b" * 64, "identity-acceptance.json": "c" * 64}

    op.local_gates = gates
    if gate_failure:
        with pytest.raises(r.RetryError, match="BACKUP_NOT_VERIFIED"):
            op.prepare()
...
def identity_report():
    return dict(
        passed=True,
        checks={str(i): True for i in range(26)},
        check_count=26,
        passed_count=26,
        failure_codes=[],
...
def local_gate_case(stopped_failure, monkeypatch):
    ...
    def helper(raw, name):
        ...
        if name == "pinned_retry_identity_checker":
            module.CURRENT = lambda: latest[0]
        return module
    ...
    def command(argv, **_kwargs):
        ...
        if argv[:3] == ["/usr/bin/systemctl", "start", "probe-backup.service"]:
            ...
            if starts[0] == 2 and failure[0] != "stale":
                latest[0] = outbox / "probe-new.tar"
                put(latest[0], b"new verified snapshot")
                put(
                    receipts / "receipt.json",
                    r.encoded(
                        dict(
                            archive_sha256=r.digest(latest[0].read_bytes()),
                            verified=True,
                            readback_verified=True,
                            restore_verified=failure[0] != "restore",
                        )
                    ),
                )
        elif argv[:2] == ["/usr/bin/python3", "-I"]:
            report = identity_report()
            if failure[0] == "identity":
                report["checks"]["0"] = False
            put(op.work / "identity-acceptance.json", r.encoded(report))
        else:
            pytest.fail("unexpected host command")
        return b""

    def ctl(*args):
        events.append(args)
        if args[1] == "probe-backup.service":
            return b"Result=success\nExecMainStatus=0\n"
        return b"failed\n" if failure[0] == "prune" else b"inactive\n"

    op.operation.command = command
    op.operation.ctl = ctl
    op.operation.users = (1000, 1001, 1003)
...
@pytest.mark.parametrize(
    "fault,code",
    [
        ("stale", "FRESH_BACKUP_REQUIRED"),
        ("restore", "BACKUP_NOT_VERIFIED"),
        ("prune", "BACKUP_PRUNE_NOT_FINISHED"),
        ("identity", "IDENTITY_GATE_FAILED"),
    ],
)
...
def test_completed_schema6_can_be_revalidated_only_with_retained_proofs(sixth_retry, stopped_failure, fault):
    ...
    old = s.operation.previous_inputs(s.original)
    snapshot = f.snapshot()
    ids = s.manifest["failed"]
    approval = snapshot["approvals"][0]["approval_id"]
    snapshot["jobs"][0].update(
        job_id=ids["job_id"],
    ...
    snapshot["attempts"][0].update(
        attempt_id=s.manifest["failed_attempt"]["attempt_id"], job_id=ids["job_id"], worker_id=ids["worker_id"]
    )
    request = snapshot["requests"][0]
    request.update(
        request_id=ids["request_id"],
        worker_id=ids["worker_id"],
        observed_provider_id=ids["provider_id"],
        job_ids=json.dumps([ids["job_id"]]),
    )
    snapshot["intents"][0].update(
        worker_id=ids["worker_id"], request_key=ids["request_id"], provider_id=ids["provider_id"]
    )
    document = json.loads(snapshot["approvals"][0]["document"])
    document["pod_id"] = ids["worker_id"]
    snapshot["approvals"][0]["document"] = json.dumps(document)
    snapshot["approval_jobs"] = [{"job_id": ids["job_id"]}]
    result = dict(
        f.operation.result,
        **{key: ids[key] for key in ("request_id", "worker_id", "provider_id")},
        teardown=dict(provider_id=ids["provider_id"], state="ABSENT", confirmed=True),
    )
    evidence = dict(
        result=result,
        bound={key: request[key] for key in f.operation.bound},
        submitted={**ids, "approval_id": approval},
    )
    s.manifest["failed_result_canonical_sha256"] = r.digest(r.encoded(result))
    following = completed_retry_then_next(s, "7" * 32)
    following.manifest.update(
        schema_version=6,
        expected_failure_stage="artifact_collection",
        expected_failure_reason="CALIBRATION_DID_NOT_PASS",
    )
    capsule = following.capsule
    put(capsule / "failed-snapshot.json", r.encoded(snapshot))
    put(capsule / "failed-evidence.json", r.encoded(evidence))
    closed = json.loads((capsule / "close-report.json").read_bytes())
    closed.update(
        original_job_cancelled=False,
        failed_job_preserved=True,
        failed_attempt=s.manifest["failed_attempt"],
        failed_snapshot_sha256=r.digest(r.encoded(snapshot)),
        failed_evidence_sha256=r.digest(r.encoded(evidence)),
    )
    ...
    put(
        ...
        r.encoded(
            dict(
                archive_sha256=r.digest(b"verified archive"),
                verified=True,
                readback_verified=True,
    ...
    report.update(
        ...
        local_gates={
            name: r.digest((capsule / name).read_bytes())
    ...
    if fault == "report":
        report["application_reinstalled"] = True
        put(capsule / "prepare-report.json", r.encoded(report))
    elif fault == "stop_proof":
        snapshot["attempts"][0]["stopped_at"] = None
        put(capsule / "failed-snapshot.json", r.encoded(snapshot))
    elif fault == "evidence_pin":
        put(capsule / "failed-evidence.json", b"{}\n")
    elif fault == "snapshot":
        put(capsule / "failed-snapshot.json", b"{}\n")
    elif fault == "backup":
        put(capsule / "before.tar", b"changed archive")
    elif fault == "identity":
        put(capsule / "identity-acceptance.json", b"{}\n")
    if fault:
        with pytest.raises(r.RetryError):
            following.operation.previous_inputs(s.original)
    else:
        assert following.operation.previous_inputs(s.original) == s.new
