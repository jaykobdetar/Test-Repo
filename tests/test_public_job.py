...
def inputs(monkeypatch):
    ...
    config = public.PublicCalibrationConfig.model_validate({
        'model': {'repo': lock.repo, 'revision_sha': lock.revision, 'tokenizer_revision': lock.revision,
                  'local_weight_hashes': [item.sha256 for item in lock.files if item.path.endswith('.safetensors')],
                  'dtype': 'bfloat16', 'quantized': False, 'thinking_mode': None, 'chat_template_hash': None},
        'assets': [{'path': item['path'][len(prefix):], 'sha256': item['sha256']}
                   for item in baked['assets'] if item['path'].startswith(prefix)],
        'datasets': [{'path': '/opt/probe-assets/datasets/public-calibration-prompts.json', 'sha256': public.DATASET_SHA}],
        'code_git_commit': 'f' * 40, 'container_image_digest': 'sha256:' + 'a' * 64,
        'region': 'EU-RO-1', 'live_price_usd_per_hour': .74})
    ...
    plan = fixed_plan(config.model, 'fixed', public.DATASET_SHA, prompt_set_hash(dataset, public.PROMPTS), public.PROMPTS)
    now = datetime.now(timezone.utc)
    request = ExecutionRequest(job_id='job-actual', attempt_id='attempt-actual', approval_id='approval-actual',
        worker_id='worker-actual', deadline=now + timedelta(seconds=30), spec=plan.cases[1].spec)
    body = {'config': config.model_dump(mode='json'), 'request': request.model_dump(mode='json'),
...
def test_every_existing_fixed_case_and_direction_is_accepted(inputs):
    cases = list(inputs.plan.cases) + list(fixed_direction_plan(inputs.config.model, 'direction', public.DATASET_SHA,
        prompt_set_hash(inputs.dataset, public.PROMPTS), public.PROMPTS).cases)
    for case in cases:
...
def test_only_identity_and_deadline_fields_can_change(inputs, mutation):
    ...
    if mutation == 'prompt': spec['inputs']['prompt_ids'] = ['private-heldout']
    if mutation == 'seed': spec['inputs']['random_seed'] = 124
    if mutation == 'operation': spec['operation']['modules'][0]['layer'] = 15
    if mutation == 'model': spec['model']['revision_sha'] = 'b' * 40
    if mutation == 'limit': spec['limits']['max_runtime_seconds'] = 121
    if mutation == 'science': body['request']['science']['session_id'] = 'claims-science'
    if mutation == 'extra': body['command'] = 'arbitrary.py'
...
def monitor_environment(tmp_path, inputs, monkeypatch):
    ...
    def launch(_):
        with (tmp_path / 'monitor.log').open('ab') as log:
            process = subprocess.Popen([sys.executable, '-I', str(harness), str(state), str(workspace), mode[0]],
                                       stdin=subprocess.DEVNULL, stdout=log, stderr=log, start_new_session=True)
        processes.append(process)
        return process
    monkeypatch.setattr(job, 'launch_monitor', launch)
    env = SimpleNamespace(state=state, workspace=workspace, mode=mode, processes=processes, log=tmp_path / 'monitor.log')
    yield env
    ...
    for process in processes:
        try:
            ...
            try: job.pidfd_signal(fd, signal.SIGKILL)
            finally: os.close(fd)
...
def test_detached_same_attempt_survives_start_transport_exit(inputs, monitor_environment, monkeypatch):
    ...
    for field in ('request_sha256', 'config_sha256', 'child_identity', 'monitor_identity',
                  'original_deadline', 'effective_job_deadline', 'execution_started'):
...
def test_cancellation_stops_actual_child_and_setsid_descendant(inputs, monitor_environment):
    ...
    grandchild = int(wait_for(lambda: (env.workspace / 'grandchild.pid').read_text()
                            if (env.workspace / 'grandchild.pid').exists() else None))
...
def test_completed_before_cancel_preserves_success(inputs, monitor_environment):
    ...
    value = {'job_id': inputs.request.job_id, 'attempt_id': inputs.request.attempt_id,
             'state': 'SUCCEEDED', 'started_at': datetime.now(timezone.utc).isoformat(),
             'finished_at': datetime.now(timezone.utc).isoformat(), 'manifest': manifest}
...
def test_real_child_specific_cuda_exception_is_not_generic_oom(inputs, tmp_path, monkeypatch, specific):
    ...
    class ActualCudaOOM(Exception): pass
    class Engine:
        def __init__(self, config): pass
        def execute(self, request, output):
            raise ActualCudaOOM('synthetic specific CUDA allocator error') if specific else MemoryError('ordinary host RAM')
    ...
    read, write = os.pipe(); os.write(write, b'1'); os.close(write)
    job.run_child(read, state, workspace)
    receipt = job.child_result(workspace, inputs.request)
    assert (receipt.failure_kind, receipt.error_code) == (('oom', 'OutOfMemoryError') if specific else ('policy', 'MemoryError'))
...
def test_tensor_protocol_only_accepts_fixed_digest_and_exact_stream(inputs, tmp_path, monkeypatch):
    ...
    import struct
    header = json.dumps({'public_basis_direction': {'dtype': 'F32', 'shape': [2048], 'data_offsets': [0, 8192]}},
                        separators=(',', ':')).encode()
    header += b' ' * ((8 - len(header) % 8) % 8)
    body = struct.pack('<Q', len(header)) + header + struct.pack('<f', 1.) + b'\0' * (8192 - 4)
...
def test_artifact_download_requires_stopped_success_and_manifest_hash(inputs, tmp_path, monkeypatch):
    ...
    manifest = json.loads((ROOT / 'tests/fixtures/manifest.json').read_text())
    manifest['artifacts'] = [{'path': 'summary.json', 'sha256': hashlib.sha256(raw).hexdigest(),
                              'retention_class': 'derived'}]
    receipt = {'job_id': inputs.request.job_id, 'attempt_id': inputs.request.attempt_id, 'state': 'SUCCEEDED',
               'started_at': datetime.now(timezone.utc).isoformat(), 'finished_at': datetime.now(timezone.utc).isoformat(),
               'manifest': manifest, 'process_stopped': True}
