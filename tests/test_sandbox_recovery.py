...
def installed(tmp_path):
    ...
    groups = {
        name: os.getgid()
        for name in (*recovery.ACCOUNTS, "probe-ipc", "probe-ledger-read", "probe-stop", "probe-backup-read")
    }
    ...
    for path in (PROJECT / "deploy/live").iterdir():
        ...
        if path.name == "probe-research.service":
            ...
            text = text.replace(
                "ExecStart=/bin/sh -ec '/usr/bin/chgrp probe-research /run/probe-research; exec ", "ExecStart="
            )
    ...
    installer = (
        (PROJECT / "deploy/install-controller.sh")
        .read_text()
        .replace(
            "runuser -u probe-trusted -g probe-trusted -- env HOME=",
            "runuser -u probe-trusted -g probe-research -- env HOME=",
        )
    )
    ...
    research = {
        ...
        "controller_uid": u,
        "input_artifact_root": "/var/lib/probe-core/input-artifacts",
        "sandbox_image": None,
    ...
    write(
        fs / "etc/probe-core/runpod.json",
        json.dumps(
            {"state_path": "/var/lib/probe-provider/runpod.sqlite", "api_key_file": "/etc/probe-core/runpod-api-key"}
        ),
    )
    ...
    for relative, tables in (
        ("var/lib/probe-core/research.sqlite", recovery.LEDGER_TABLES),
    ...
    for relative in (
        "var/lib/probe-core/input-artifacts",
        "var/lib/probe-core/worker-transfers",
        "var/lib/probe-watchdog",
        "var/lib/probe-backups/outbox",
        "var/lib/probe-backups/receipts",
        "var/lib/probe-research",
        "var/lib/probe-sandbox/.local",
    ):
...
@pytest.mark.parametrize(
    "kind", ["source", "native", "extra", "escaping_link", "writable_venv", "cache_without_source"]
)
...
def test_service_state_must_be_inactive_disabled_and_exact(installed, kind):
    ...
    def reply(command, **kwargs):
        ...
        if kind == "enabled":
            result.stdout = result.stdout.replace(b"UnitFileState=disabled", b"UnitFileState=enabled")
        if kind == "dropin":
            result.stdout = result.stdout.replace(b"DropInPaths=", b"DropInPaths=/etc/override.conf")
        return result
...
def test_started_work_or_completed_gate_is_preserved_and_refused(installed, kind):
    ...
    if kind in {"job", "provider_intent"}:
        relative, table = (
            ("var/lib/probe-core/research.sqlite", "jobs")
            if kind == "job"
            else ("var/lib/probe-provider/runpod.sqlite", "runpod_intents")
        )
        with closing(sqlite3.connect(fs / relative)) as connection:
            connection.execute(f'INSERT INTO "{table}" VALUES (?)', ("existing work",))
            connection.commit()
    elif kind == "wal":
        write(fs / "var/lib/probe-core/research.sqlite-wal", "uncheckpointed")
    elif kind == "backup":
        write(fs / "var/lib/probe-backups/outbox/retained.tar", "retained data")
    elif kind == "sandbox_acceptance":
        write(fs / "var/lib/probe-sandbox/acceptance-report.json", '{"status":"passed"}')
    else:
