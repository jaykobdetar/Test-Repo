...
def approve(ledger, jobs):
    ...
    nonce = ApprovalNonce(
        ...
        price_ceiling_usd_per_hour=1.2,
        issued_at=now,
...
def test_http_requires_authentication_and_loopback(service, make_request):
    ...
    with pytest.raises(TransportError):
        bad.submit(make_request())
    assert not supervisor._requests
    with pytest.raises(ValueError):
        WorkerClient("http://example.com:8080", "x" * 40)
    with pytest.raises(ValueError):
        WorkerHTTPServer(supervisor, "x" * 40, host="0.0.0.0")
...
def test_cancel_kills_worker_and_acknowledges_stop_to_ledger(service, make_request, tmp_path):
    ...
    with Ledger(tmp_path / "ledger.sqlite") as ledger:
        ...
        with ledger.read_connection() as reader:
            assert (
                reader.execute("SELECT stopped_at FROM attempts WHERE attempt_id=?", (running.attempt_id,)).fetchone()[
                    0
                ]
                is not None
            )
...
def test_single_worker_rejects_parallel_jobs_and_submission_is_idempotent(service, make_request):
    ...
    with pytest.raises(TransportError):
        client.submit(make_request(key="job-2"))
    assert len(supervisor._requests) == 1
    client.cancel(request.attempt_id)


def test_uncertain_submission_preserves_attempt_and_can_reconcile_same_execution(
    service, make_request, tmp_path, monkeypatch
):
    _, _, client = service
    with Ledger(tmp_path / "ledger.sqlite") as ledger:
        job = ledger.submit_job(make_request().spec)
        ...
        with monkeypatch.context() as patch:
            ...
            with pytest.raises(TransportError):
                dispatcher.dispatch_next(approval_id="wake-1")
...
def test_ssh_command_pins_trust_and_never_uses_host_shell(tmp_path):
    ...
    with pytest.raises(ValueError):
        SSHTunnel("host; touch /tmp/invalid", user="root", identity_file=key, known_hosts_file=hosts)
...
def test_controller_direction_is_staged_by_hash_before_steering(service, make_request, tmp_path):
    ...
    operation = {
        ...
        "positions": ["last"],
        "direction": {"path": digest + "/tensor.safetensors", "sha256": "sha256:" + digest, "tensor_name": "direction"},
        "strength": 0.2,
    }
...
def test_capture_output_can_be_uploaded_and_replayed_by_patch(service, make_request, tmp_path):
    ...
    operation = {
        ...
        "target": {"layer": 0, "component": "residual"},
        "positions": ["last"],
        "source": {"path": uploaded["path"], "sha256": uploaded["sha256"], "tensor_name": "layer_0_residual"},
    }
...
def test_non_safetensors_and_wrong_digest_uploads_are_rejected(service, tmp_path):
    ...
    with pytest.raises(TransportError):
        client.upload_tensor(path, expected_sha256=digest)
    with pytest.raises(TransportError):
        client.upload_tensor(path, expected_sha256="sha256:" + "0" * 64)
...
def _independent_watchdog(ledger_path, provider_path, state_path, health_path, stop_event):
    ...
    watcher = StopWatchdog(
        ledger_path, StopOnlyBackend(SimulatedProvider(provider_path)), state_path=state_path, health_path=health_path
    )
...
def test_daemon_runs_controller_approved_job_with_independent_watchdog(
    service, make_request, tmp_path, stop_while_running
):
    ...
    with Ledger(database) as ledger:
        ...
        watcher = context.Process(
            target=_independent_watchdog,
            args=(str(database), str(provider_path), str(tmp_path / "watchdog" / "state.sqlite"), str(health), stop),
        )
        watcher.start()
            ...
            json.dumps(
                {
                    ...
                    "transfer_directory": str(tmp_path / "daemon-transfers"),
                    "input_artifact_root": str(tmp_path / "registry"),
                    "bearer_secret_file": str(secret_file),
        ...
        try:
            ...
            while time.monotonic() < deadline:
                ...
                if record.state.value in (
                    {"RUNNING", "COMPLETED", "FAILED"} if stop_while_running else {"COMPLETED", "FAILED"}
                ):
                    break
            ...
            if stop_while_running:
                ...
                try:
                    with closing(sqlite3.connect(database, isolation_level=None, timeout=5)) as gate:
                        ...
                        try:
                            assert (
                                gate.execute("SELECT state FROM jobs WHERE job_id=?", (job.job_id,)).fetchone()[0]
                                == "RUNNING"
                            )
                    ...
                    with ledger.read_connection() as reader:
                        assert (
                            reader.execute(
                                "SELECT ended_at FROM approvals WHERE approval_id=?", (request["approval_id"],)
                            ).fetchone()[0]
                            is None
                        )
                        assert (
                            reader.execute(
                                "SELECT stopped_at FROM attempts WHERE attempt_id=?", (record.attempt_id,)
                            ).fetchone()[0]
                            is None
                        )
                finally:
