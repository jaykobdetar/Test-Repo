...
def test_real_bootstrap_refusal_producer_is_accepted_without_its_private_message():
    ...
    try:
        exec(compile('raise BootstrapRefused(secret)', gpu_launch.__file__, 'exec'),
...
def test_nonzero_subprocess_retains_only_fixed_bootstrap_record():
    ...
    assert caught.value.diagnostic == {'phase': 'verification', 'exit_status': 19, 'timeout': False,
...
def test_nonzero_subprocess_does_not_extract_receipt_from_stderr():
    ...
    with pytest.raises(runner.RunnerError) as caught:
        runner.command_bytes([sys.executable, '-I', '-c', code])
    assert caught.value.diagnostic == {'phase': 'verification', 'exit_status': 23, 'timeout': False,
...
def test_command_timeout_kills_owned_group_including_child_holding_stdout(tmp_path):
    ...
    started = time.monotonic()
    with pytest.raises(runner.RunnerError) as caught:
        runner.command_bytes([sys.executable, '-I', '-c', code], timeout=.4)
    assert time.monotonic()-started < 3
    assert caught.value.diagnostic == {'phase': 'verification', 'exit_status': 0, 'timeout': True,
...
def test_diagnostic_attribute_cannot_publish_arbitrary_exception_fields():
    raw = {'phase': 'configure', 'exit_status': 7, 'timeout': False, 'classification': 'PROCESS_EXITED',
           'stdout': SECRET, 'stderr': SECRET, 'argv': [SECRET], 'bootstrap': dict(BOOTSTRAP, path=SECRET)}
    assert runner.safe_command_diagnostic(raw) == {
        'phase': 'configure', 'exit_status': 7, 'timeout': False, 'classification': 'PROCESS_EXITED'}
    for key, value in [('phase', SECRET), ('exit_status', SECRET), ('exit_status', True),
                       ('timeout', SECRET), ('classification', SECRET), ('classification', {})]:
...
def test_provider_diagnostic_publication_revalidates_records_and_drops_extra_fields():
    raw = {'status': 'observed', 'records': [BOOTSTRAP, dict(BOOTSTRAP, address=SECRET), SECRET],
           'url': SECRET, 'stderr': SECRET}
...
def tunnel(tmp_path, monkeypatch):
    ...
    for path in (key, hosts):
        ...
        path.chmod(0o600)
    instance = dispatcher.SSHTunnel('fixture.example', user='root', identity_file=key,
                                    known_hosts_file=hosts, local_port=40000)
...
def test_tunnel_nonzero_exit_retains_status_without_subprocess_output(tunnel, monkeypatch):
    ...
    assert caught.value.diagnostic == {'phase': 'tunnel', 'exit_status': 17, 'timeout': False,
...
def test_tunnel_classifies_stderr_without_leaking_private_bytes(tunnel, monkeypatch, classification, message):
    ...
    with pytest.raises(dispatcher.TransportError) as caught:
        tunnel.start(timeout_seconds=1)
    assert caught.value.diagnostic == {'phase': 'tunnel', 'exit_status': 255, 'timeout': False,
...
def test_tunnel_drains_beyond_diagnostic_bound_without_retaining_or_classifying_late_text(tunnel, monkeypatch):
    code = ('import sys;sys.stderr.write(' + repr(SECRET) + '*5000);'
...
def test_tunnel_timeout_reaps_owned_process(tunnel, monkeypatch, tmp_path):
    ...
    started = time.monotonic()
    with pytest.raises(dispatcher.TransportError) as caught:
        tunnel.start(timeout_seconds=.3)
    assert time.monotonic()-started < 3
    assert caught.value.diagnostic == {'phase': 'tunnel', 'exit_status': None, 'timeout': True,
...
def test_stalled_provider_read_cannot_block_deletion_deadline(monkeypatch):
    ...
    try:
        result = runner.provider_bootstrap_diagnostic(None, 'owned-pod', timeout_seconds=.05)
