...
@pytest.mark.parametrize(
    "path", ["/sys/fs/cgroup", "/sys/fs/cgroup/another-service", "/sys/fs/cgroup/probe-jobs/../probe-jobs", None]
)
...
def configured_fixture(tmp_path, monkeypatch, manifest_data):
    ...
    for key, name in [
        ("BOOTSTRAP_STATE", "bootstrap.json"),
        ("CONFIGURED", "ready.json"),
        ("CONFIG_BUNDLE", "bundle.json"),
        ("BAKED_MANIFEST", "assets.json"),
    ...
    body = dict(
        model_directory="/opt/probe-assets/models/Qwen/base",
        ...
        environment_lock_path="/opt/probe-core/uv.lock",
    ...
    for path, value in [
        (launch.BOOTSTRAP_STATE, {"deadline": time.time() + 300}),
        (launch.PROVENANCE, {"source_commit": "c" * 40}),
        (launch.BAKED_MANIFEST, assets),
...
def test_private_configuration_published_once_without_inference(configured_fixture):
    ...
    launch.CONFIG_BUNDLE.write_text(canonical_json(bundle))
    launch.CONFIG_BUNDLE.chmod(0o600)
...
@pytest.mark.parametrize(
    "change", ["source", "model_path", "hash", "dataset_path", "output_path", "deadline", "permissions"]
)
def test_invalid_staging_refused_before_private_configuration(configured_fixture, change):
    ...
    if change == "source":
        body["code_git_commit"] = "e" * 40
    if change == "model_path":
        body["model_directory"] = "/etc"
    if change == "hash":
        body["assets"][0]["sha256"] = "sha256:" + "e" * 64
    if change == "dataset_path":
        body["datasets"][0]["path"] = "/opt/probe-assets/datasets/../secret"
    if change == "output_path":
        body["output_directory"] = "/opt/probe-assets"
    if change == "deadline":
        launch.BOOTSTRAP_STATE.write_text(canonical_json({"deadline": time.time() - 1}))
    launch.CONFIG_BUNDLE.write_text(canonical_json(configured_fixture))
    if change == "permissions":
        launch.CONFIG_BUNDLE.chmod(0o644)
...
def test_incompatible_host_never_opens_ssh_or_loads_model(monkeypatch):
    ...
    def incompatible(*args):
        raise ValueError("cgroup-v2 namespace required")
...
@pytest.mark.parametrize(
    ...
    [
        ("bootstrap", []),
        ("configure", ["--configure", "/run/probe-worker-bootstrap.json"]),
        (
            "check-paths",
            ["--check-paths", "/workspace/probe/config/worker.json", "/workspace/probe/config/worker-token"],
        ),
    ],
)
...
"""
    bindings = {
        "PROBE_ABSOLUTE_DEADLINE": "2027-01-01T00:00:00+00:00",
        "PROBE_WORKER_ID": "test-worker",
        "PROBE_REQUEST_ID": "test-request",
        "PROBE_CONFIGURATION_HASH": "sha256:" + "a" * 64,
        "PUBLIC_KEY": "ssh-ed25519 AAAAsynthetic",
    }
    hostile = {
        "RUNPOD_API_KEY": "synthetic-provider-secret",
        "HF_TOKEN": "synthetic-hf-secret",
        "AWS_SECRET_ACCESS_KEY": "synthetic-aws-secret",
        "HTTP_PROXY": "http://synthetic-proxy",
        "PROBE_ENV_CLEAN": "1",
        "PYTHONPATH": str(tmp_path),
        "PYTHONSTARTUP": "/synthetic/startup",
    ...
    with subprocess.Popen(
        [sys.executable, "-I", "-c", code], env=hostile, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
    ) as process:
...
@pytest.mark.parametrize(
    "arguments",
    [["--bad"], ["--configure"], ["--configure", "a", "b"], ["--check-paths", "a"], ["--check-paths", "a", "b", "c"]],
)
...
def test_resource_gates_and_every_supervisor_restart_use_clean_worker_environment(
    configured_fixture, monkeypatch, tmp_path
):
    launch.configure(launch.CONFIG_BUNDLE)
    monkeypatch.setattr(launch, "GPU_LIBRARY_DIRECTORIES", ())
...
def test_ssh_and_keygen_receive_only_fixed_root_environment(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda value: (
            tmp_path / "ssh"
            if value == "/root/.ssh"
            else tmp_path / "sshd_config"
            if value == "/etc/ssh/sshd_config.probe-worker"
...
def test_image_uses_isolated_absolute_bootstrap_interpreter():
    ...
    assert (
        'ENTRYPOINT ["/opt/probe-core/venv/bin/python", "-I", "-m", "probe_core.gpu_launch"]' in dockerfile.read_text()
    )
...
def test_clean_bootstrap_still_validates_original_deadline_and_bindings(tmp_path, monkeypatch, invalid):
    ...
    bindings = {
        "PROBE_ABSOLUTE_DEADLINE": deadline,
        "PROBE_WORKER_ID": "worker",
        "PROBE_REQUEST_ID": "request",
        "PROBE_CONFIGURATION_HASH": "sha256:" + "a" * 64,
    }
    if invalid == "expired":
        bindings["PROBE_ABSOLUTE_DEADLINE"] = datetime.fromtimestamp(now - 1, timezone.utc).isoformat()
    if invalid == "too_long":
        bindings["PROBE_ABSOLUTE_DEADLINE"] = datetime.fromtimestamp(now + 901, timezone.utc).isoformat()
    if invalid == "naive":
        bindings["PROBE_ABSOLUTE_DEADLINE"] = "2027-01-01T00:00:00"
    if invalid == "binding":
        bindings["PROBE_REQUEST_ID"] = "request\nother"
    if invalid == "missing":
        del bindings["PROBE_REQUEST_ID"]
    monkeypatch.setattr(launch, "GPU_LIBRARY_DIRECTORIES", ())
...
def test_module_failure_emits_safe_receipt_without_traceback():
    project = str(Path(launch.__file__).resolve().parents[1])
    code = (
        f"import sys, runpy; sys.path.insert(0, {project!r}); "
