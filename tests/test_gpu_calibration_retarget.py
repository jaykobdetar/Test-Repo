...
def changed(old, identity="a" * 32):
    ...
    config = dict(
        before,
        deployment=dict(before["deployment"], region="EU-RO-1"),
    ...
    for name in r.CALIBRATION:
        new[name] = old[name].replace(
            ("--config " + str(Path(before["plan_path"]).parent / "acceptance.json")).encode(),
...
def replacement(typed_inputs):
    ...
    old["acceptance.json"] = r.encoded(
        runner.RunnerConfig.model_validate_json(old["acceptance.json"]).model_dump(mode="json")
    )
...
def test_scope_expansion_refused_before_actions(replacement, fault):
    ...
    if fault.startswith("worker_"):
        ...
        key = {
            ...
            "worker_model": "model",
            "worker_source": "code_git_commit",
            "worker_image": "container_image_digest",
            "worker_assets": "assets",
        }[fault]
        if fault == "worker_region":
            worker[key] = "EU-CZ-1"
        elif fault == "worker_source":
            worker[key] = "f" * 40
        elif fault == "worker_image":
            worker[key] = "sha256:" + "f" * 64
        elif fault == "worker_model":
            worker[key]["revision_sha"] = "f" * 40
        else:
            worker[key][0]["sha256"] = "sha256:" + "f" * 64
        f.new["worker.json"] = r.encoded(worker)
        config = json.loads(f.new["acceptance.json"])
        config["worker_config_sha256"] = "sha256:" + r.digest(f.new["worker.json"])
        f.new["acceptance.json"] = r.encoded(config)
    elif fault in {"runtime", "price", "runner_region", "token", "identity", "approval_wait"}:
        config = json.loads(f.new["acceptance.json"])
        if fault == "runner_region":
            config["deployment"]["region"] = "EU-CZ-1"
        else:
            key = {
                "runtime": "max_runtime_seconds",
                "price": "expected_worker_price_usd_per_hour",
                "token": "bearer_secret_file",
                "identity": "ssh_identity_file",
                "approval_wait": "approval_wait_seconds",
            }[fault]
            config[key] = "/different-secret" if fault in {"token", "identity"} else 360 if fault != "price" else 0.79
        f.new["acceptance.json"] = r.encoded(config)
    elif fault in {"prompt", "model", "same_key", "same_label"}:
        plan = json.loads(f.new["plan.json"])
        if fault == "prompt":
            plan["cases"][0]["spec"]["inputs"]["prompt_ids"][0] = "different"
        elif fault == "model":
            plan["model"]["revision_sha"] = "f" * 40
        elif fault == "same_key":
            plan["cases"][0]["spec"]["idempotency_key"] = json.loads(f.old["plan.json"])["cases"][0]["spec"][
                "idempotency_key"
            ]
        else:
            plan["label"] = json.loads(f.old["plan.json"])["label"]
        f.new["plan.json"] = r.encoded(plan)
    else:
        name = r.CALIBRATION[1]
        f.new[name] = (
            f.new[name].replace(b"User=probe-trusted", b"User=root")
            if fault == "unit_user"
            else f.new[name] + b"\nOther=value\n"
        )
...
def pending(setup, monkeypatch):
    ...
    s.config = s.config.model_copy(
        update={
            "deployment": s.config.deployment.model_copy(update={"region": "EU-CZ-1"}),
    ...
    def snapshot(target=None):
        ...
        with redirect_stdout(out):
            exec(source, {"PENDING_JSON": json.dumps(target or identity)})
...
def test_real_never_approved_job_cancels_via_research_and_cannot_consume_approval(pending):
    ...
    nonce = ApprovalNonce(
        approval_id=f.submitted["approval_id"],
        token="s" * 48,
        pod_id=f.identity["worker_id"],
        batch_hash=f.submitted["batch_hash"],
        max_runtime_seconds=900,
        price_ceiling_usd_per_hour=0.8,
        issued_at=f.s.clock(),
        expires_at=f.s.clock() + timedelta(minutes=5),
    )
    f.s.ledger.register_approval(nonce)
    with pytest.raises(ApprovalError, match="approved jobs must be pending"):
        f.s.ledger.consume_approval(
            nonce.approval_id,
            nonce.token.get_secret_value(),
            pod_id=nonce.pod_id,
            job_ids=[f.identity["job_id"]],
            live_price_usd_per_hour=0.74,
            requested_runtime_seconds=900,
        )
...
def test_no_authority_gates_refuse_changed_or_ambiguous_state(pending, fault):
    f = pending
    snapshot = f.snapshot()
    submitted = deepcopy(f.submitted)
    if fault in {"approval", "linked_approval", "attempt", "intent"}:
        snapshot[
            {"approval": "approvals", "linked_approval": "approval_jobs", "attempt": "attempts", "intent": "intents"}[
                fault
            ]
        ] = [{"unexpected": True}]
    ...
    elif fault == "started_request":
        snapshot["requests"][0]["state"] = "PREPARING"
    elif fault == "observed_provider":
        snapshot["requests"][0]["observed_provider_id"] = "pod"
    elif fault == "deadline":
        snapshot["requests"][0]["deadline"] = 123
    elif fault == "changed_config":
        snapshot["requests"][0]["configuration_hash"] = "sha256:" + "f" * 64
    elif fault == "changed_spec":
        snapshot["jobs"][0]["spec_json"] = "{}"
    else:
        submitted["request_id"] = "different"
    with pytest.raises((r.RetargetError, ValueError)):
...
def operation(pending, monkeypatch):
    ...
    for name in ("PUBLIC", "SUBMIT", "STATE", "UNITS"):
        path = s.root / ("retarget-" + name.lower())
        path.mkdir(mode=0o700)
        monkeypatch.setattr(r, name, path)
    old = {
        "plan.json": r.encoded(f.plan),
        "acceptance.json": r.encoded(f.config),
    ...
    for name in r.CALIBRATION:
        old[name] = (
            (PROJECT / "deploy" / name).read_bytes().replace(b"/etc/probe-calibration/acceptance.json", old_path)
        )
        put(r.UNITS / name, old[name])
    new = changed(old)
    public, config = r.validate_retarget(old, new, "a" * 32, "EU-CZ-1", "EU-RO-1")
    events = []
    fault = [None]
    active = {name: False for name in r.CALIBRATION}
    a = SimpleNamespace(
        Activation=lambda: None, trusted=lambda _: None, write_file=lambda path, raw, **_: put(path, raw)
    )
    instance = r.Retarget(
        a, None, {"pending": f.identity, "installed_upgrade": {"wheel_sha256": "b" * 64}}, {}, "c" * 64
    )
    instance.work = s.root / "capsule"
    instance.work.mkdir()
    instance.old, instance.new, instance.old_config, instance.config = old, new, f.config, config
    instance.submitted = f.submitted
    instance.public = public
    instance.paths = (public, Path(config["submission_state_directory"]), Path(config["trusted_state_directory"]))
    instance.marker = public / "prepared"
    instance.guard_name = "50-probe-calibration-retarget.conf"
    ...
    instance.identities = lambda: (
        events.append("identities")
        if fault[0] != "identity"
        else (_ for _ in ()).throw(RuntimeError("identity failed"))
    )
        ...
        if args[0] == "stop":
            ...
            with runner.State(instance.paths[1]) as state:
                runner.submit(
                    ...
                    runner.AcceptancePlan.model_validate_json(new["plan.json"]),
                    s.rpc,
                    state,
                )
            ...
            put(
                instance.paths[2] / "binding.json",
                r.encoded(
                    {
                        "config_sha256": runner.digest(
                            runner.RunnerConfig.model_validate(config).model_dump(mode="json")
                        ),
    ...
    def command(argv, **_):
        ...
        if fault[0] == "lost_cancel":
            raise RuntimeError("lost response")
        return r.encoded(answer)

    instance.operation = SimpleNamespace(
        users=(os.geteuid(), os.geteuid(), os.geteuid()), ctl=ctl, command=command, idle=lambda: events.append("idle")
    )
    instance.unit_state = lambda name: {
        "ActiveState": "active" if active[name] else "inactive",
        "MainPID": "42" if active[name] else "0",
        "ControlPID": "0",
    }
...
def test_guarded_retarget_preserves_old_records_and_starts_only_unapproved_new_proposal(operation):
    ...
    assert (
        ...
        and result["approval_issued"] is False
        and result["application_reinstalled"] is False
    )
    assert (
        f.events.index("backup")
        < f.events.index(("stop", *r.CALIBRATION))
        < f.events.index("cancel-rpc")
        < f.events.index("identities")
    )
    ...
    assert all(
        "probe-controller" not in str(event) and "probe-research.service" not in str(event) for event in f.events
    )
...
def test_partial_failure_stays_closed_and_does_not_replay_mutations(operation, fault):
    ...
    if fault == "backup":
        ...
        assert all(
            (r.UNITS / (name + ".d") / f.instance.guard_name).read_bytes() == f.instance.guard for name in r.CALIBRATION
        )
        assert ("stop", *r.CALIBRATION) in f.events
        count = f.events.count("cancel-rpc")
        with pytest.raises(r.RetargetError):
            f.instance.execute()
...
def test_state_is_rechecked_after_service_stop_before_cancel(operation):
    ...
    with pytest.raises(r.RetargetError, match="AUTHORITY_OR_OTHER_WORK_PRESENT"):
        f.instance.execute()
...
def test_manifest_pins_exact_old_new_files_and_all_executed_helpers(tmp_path, monkeypatch):
    ...
    manifest = {
        ...
        "installed_upgrade": {"wheel_sha256": "b" * 64, "release_manifest_sha256": "c" * 64},
        "retarget_id": "d" * 32,
        "from_region": "EU-CZ-1",
        "to_region": "EU-RO-1",
        "pending": {"job_id": "job", "request_id": "request", "worker_id": "worker"},
    ...
    with pytest.raises(r.RetargetError, match="FILE_PIN_CHANGED"):
        r.inputs(path, pin)
    with pytest.raises(r.RetargetError, match="MANIFEST_PIN_CHANGED"):
        r.inputs(path, "f" * 64)
...
def test_existing_backup_service_must_produce_fresh_verified_archive_before_mutation(tmp_path, monkeypatch, fault):
    ...
    instance.work = tmp_path / "capsule"
    instance.work.mkdir()
    ...
    def command(args, **kwargs):
        ...
        calls.append(args)
        if len(calls) == 2 and fault != "stale":
            archive("probe-2.tar", b"fresh verified archive", fault != "unverified")
        return b""
    ...
    if fault:
        ...
        assert json.loads((instance.work / "backup.json").read_bytes())["archive_sha256"] == r.digest(
            b"fresh verified archive"
        )
