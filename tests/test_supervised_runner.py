...
def configuration(s):
    return runner.SupervisedRunnerConfig(
        **s.config.model_dump(mode="json"), helper_path="/pinned/public-job.py", helper_sha256="sha256:" + "e" * 64
    )
...
def execute(s, client):
    with runner.State(s.config.trusted_state_directory) as state:
        return runner.run(
            ...
            s.ledger,
            s.backend,
            SimpleNamespace(stop_gpu=s.controller.stop_gpu),
            state,
            endpoint=lambda *_: {},
...
def test_coordinator_reattaches_existing_attempt_without_resubmission(setup):
    ...
    client = SSHFixture(s)
    original = Dispatcher(
        s.ledger, client, worker_id=request["worker_id"], transfer_directory=s.root / "initial-transfers"
    ).dispatch_next(approval_id=request["approval_id"])
...
class CancellableSSH(SSHFixture):
    ...
    def status(self, attempt_id):
        ...
        return ExecutionReceipt(job_id=r.job_id, attempt_id=r.attempt_id, state="RUNNING", started_at=self.s.clock())

    def inspect(self, attempt_id):
        request = runner.find_request(self.s.ledger, configuration(self.s), self.s.plan)
        body = {
            "receipt": self.status(attempt_id).model_dump(mode="json"),
            "request_sha256": runner.digest(self.request.model_dump(mode="json")),
            "config_sha256": runner.digest({}),
            "child_identity": self.child,
            "original_deadline": datetime.fromtimestamp(request["deadline"], timezone.utc).isoformat(),
            "cancellation": None,
        }
        body["execution_started"] = {
            key: getattr(self.request, key) for key in ("job_id", "attempt_id", "worker_id", "approval_id")
        }
        body["execution_started"].update(
            {key: body[key] for key in ("request_sha256", "config_sha256", "child_identity")}
        )
        body["execution_started"]["deadline"] = self.request.deadline.isoformat()
        if self.cancelled and self.proof:
            body["cancellation"] = {
                key: body[key] for key in ("request_sha256", "config_sha256", "child_identity", "original_deadline")
            }
            body["cancellation"].update(
                attempt_id=attempt_id,
                process_stopped=True,
                descendants_stopped=True,
                result_present=False,
                signal="SIGKILL",
                signal_scope="fenced_monitor_descendants",
                signalled=[self.child],
            )
        return body
...
def cancel_plan(s):
    ...
    fixed = fixed_plan(s.plan.model, "cancel-proof", inputs.dataset_revision, inputs.prompt_set_hash, inputs.prompt_ids)
...
def test_suite_rejects_duplicate_or_external_paths(monkeypatch):
    for paths in [["../foreign-acceptance.json"], ["a-acceptance.json"] * 2, []]:
        monkeypatch.setattr(
            runner, "read_file", lambda *_a, **_k: json.dumps({"schema_version": 1, "configurations": paths}).encode()
        )
