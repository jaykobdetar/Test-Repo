...
def fake_runtime(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda sandbox: {
            ...
            "launchers_killed": True,
            "host_timer_excluded": True,
            "container_processes_stopped": True,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "network_allowed",
        "output_unbounded",
        "memory_startup_failure",
        "memory_other_error",
        "memory_unbounded",
        "time_startup_failure",
        "time_other_failure",
    ],
)
...
def test_stale_pass_is_invalidated_before_any_runtime_action(tmp_path, fake_runtime, monkeypatch, capsys):
    ...
    assert (
        acceptance.main(
            ["--image", IMAGE, "--workspace", str(tmp_path / "workspace"), "--output", str(tmp_path / "report.json")]
        )
        == 1
    )
    report = json.loads((tmp_path / "report.json").read_text())
...
def test_contained_logs_are_bounded_before_saving(tmp_path, fake_runtime, monkeypatch):
    ...
    def logged(sandbox, code, *, limits, inputs=None):
        ...
        return SandboxResult(
            result.returncode, result.stdout + message, message, result.artifacts, result.termination_reason
        )
...
def test_modified_profile_is_rejected_before_containers(tmp_path, fake_runtime):
    ...
    with pytest.raises(acceptance.AcceptanceError, match="reviewed seccomp"):
        acceptance.run_acceptance(
            image=IMAGE, workspace=tmp_path / "workspace", output=tmp_path / "report.json", seccomp_profile=profile
        )
    assert fake_runtime.calls == []
...
@pytest.mark.parametrize(
    ...
    [
        (IMAGE.encode(), 0, True),
        (IMAGE[7:].encode(), 0, True),
        (("sha256:" + "b" * 64).encode(), 0, False),
        (IMAGE.encode(), 1, False),
        (b"", 1, False),
    ],
)
...
def test_inspect_failure_keeps_private_stderr(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "run",
        lambda *args, **kwargs: subprocess.CompletedProcess(args[0], 125, b"", b"private store access denied"),
    )
...
def test_real_installed_acceptance_contract(tmp_path):
    ...
    report = acceptance.run_acceptance(
        ...
        podman=os.environ.get("PROBE_SANDBOX_PODMAN", "podman"),
    )
