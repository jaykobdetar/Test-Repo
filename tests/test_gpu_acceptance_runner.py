...
def setup(tmp_path, runpod, manifest_data, monkeypatch):
    ...
    monkeypatch.setattr(runner, "provider_bootstrap_diagnostic", lambda *_args: {"status": "observed", "records": []})
    ...
    deployment = DeploymentSpec.model_validate(
        dict(deployment.model_dump(), storage_mode="disposable_research", volume_id=None, volume_gb=0)
    )
    ...
    backend.config = backend.config.model_copy(
        update={"storage_rates": backend.config.storage_rates.model_copy(update={"checked_at": clock()})}
    )
    ...
    spec = JobSpec(
        idempotency_key="one-public-calibration",
        experiment_stage="calibration",
        model=data["model"],
        inputs=data["inputs"],
        operation={"kind": "backend_parity"},
        limits={"max_runtime_seconds": 240, "max_output_bytes": 1024 * 1024},
    )
    plan = AcceptancePlan(
        label="one-public",
        model=spec.model,
        cases=(AcceptanceCase(name="backend-parity", action="wait", expected_state="COMPLETED", spec=spec),),
    )
    ...
    worker_path.write_text(
        canonical_json(
            {
                "model_directory": "/opt/probe-assets/models/qwen3-base",
                "model": spec.model.model_dump(mode="json"),
                "assets": [{"path": "config.json", "sha256": "sha256:" + "e" * 64}],
                "datasets": [
                    {"path": "/opt/probe-assets/datasets/public.json", "sha256": spec.inputs.dataset_revision}
                ],
                "tensor_directory": "/workspace/probe/tensors",
                "output_directory": "/workspace/probe/attempts",
                "backend": "nnsight",
                "device": "cuda:0",
                "code_git_commit": "c" * 40,
                "container_image_digest": deployment.image_digest,
    ...
    config = runner.RunnerConfig(
        ...
        research_uid=os.geteuid() + 10,
        admin_uid=os.geteuid() + 20,
        plan_path=str(plan_path),
        plan_sha256="sha256:" + hashlib.sha256(plan_path.read_bytes()).hexdigest(),
        deployment=deployment,
        worker_config_path=str(worker_path),
        worker_config_sha256="sha256:" + hashlib.sha256(worker_path.read_bytes()).hexdigest(),
        source_commit="c" * 40,
        expected_worker_price_usd_per_hour=0.74,
        max_runtime_seconds=900,
        submission_state_directory=str(tmp_path / "submit"),
        trusted_state_directory=str(tmp_path / "runner"),
        ssh_identity_file=str(key),
        bearer_secret_file=str(token),
    )
    ...
    service = ResearchService(
        ledger,
        ResearchPolicy(discovery_datasets=(spec.inputs.dataset_revision,), allow_calibration=True),
        cloud=controller,
    )
...
def approve_fixture(s):
    ...
    nonce = ApprovalNonce(
        ...
        issued_at=s.clock(),
        expires_at=s.clock() + timedelta(minutes=5),
    )
    s.ledger.register_approval(nonce)
    grant = s.ledger.consume_approval(
        nonce.approval_id,
    ...
    s.backend.create(
        ...
        price_ceiling_usd_per_hour=0.8,
        storage_ceiling_usd_per_day=2,
        absolute_deadline=grant.deadline,
    )
    observed = s.backend.status(request["worker_id"])
    s.controller._state(
        request["request_id"], "RUNNING", deadline=grant.deadline.timestamp(), provider_id=observed.provider_id
    )
...
def test_authority_requires_exact_current_request(setup, change, code):
    ...
    assert (
        runner.validate_authority(s.ledger, s.config, s.plan, request, now=s.clock().timestamp()) == request["deadline"]
    )
...
def endpoint_fixtures(s, *, host="213.192.2.71"):
    ...
    events = [
        {"source": "container", "line": f"256 {runner.fingerprint(key)} {label} (ED25519)"}
...
def test_actual_v2_endpoint_shape_requires_authenticated_host_fingerprint(setup):
    ...
    with runner.State(s.config.trusted_state_directory) as state:
        result = runner.verified_endpoint(s.config, request, s.backend, state, logs=lambda *_: events, command=command)
...
def test_verified_known_host_matches_openssh_standard_and_nonstandard_port(setup, host, port):
    ...
    with runner.State(s.config.trusted_state_directory) as state:
        settings = runner.verified_endpoint(
            s.config, request, s.backend, state, logs=lambda *_: events, command=command
        )
        assert settings["known_hosts_file"].read_text() == f"{marker} {public_key(2)}\n"
        found = subprocess.run(
            ["/usr/bin/ssh-keygen", "-F", marker, "-f", str(settings["known_hosts_file"])],
            capture_output=True,
            text=True,
            timeout=3,
        )
...
def test_not_yet_published_endpoint_is_unavailable_without_ssh_or_log_reads(setup, shape):
    ...
    if shape == "runtime_missing":
        ...
        s.http.pods[0]["runtime"] = {
            "runtime_null": None,
            "ports_missing": {},
            "ports_null": {"ports": None},
            "ports_empty": {"ports": []},
        }[shape]
...
@pytest.mark.parametrize(
    "status,retryable", [(408, True), (429, True), (500, True), (503, True), (401, False), (403, False), (404, False)]
)
...
class Endpoint:
    ...
    def submit(self, request):
        ...
        return ExecutionReceipt(
            job_id=request.job_id, attempt_id=request.attempt_id, state="RUNNING", started_at=self.s.clock()
        )
    ...
    def status(self, attempt_id):
        ...
        data["experiment"].update(
            tool={"backend_parity": "backend_parity", "capture": "capture_activation", "steer": "steer_direction"}[
                request.spec.operation.kind
            ],
        ...
        data["software"].update(
            container_image_digest=s.config.deployment.image_digest,
        ...
        data["cost"] = {
            ...
            "bytes_persisted": sum(map(len, contents.values())),
        }
        data["artifacts"] = [
            {"path": name, "sha256": hashlib.sha256(body).hexdigest(), "retention_class": "validated"}
...
def test_startup_recovers_original_endpoint_without_new_create_or_extended_approval(
    setup, first_observation, startup_delay
):
    s = setup
    request, events, command = endpoint_fixtures(s)
...
def test_runtime_missing_forever_reaches_original_startup_deadline_then_deletes(setup):
    ...
    with runner.State(s.config.trusted_state_directory) as state:
        result = runner.run(
            ...
            state,
            clock=lambda: s.clock().timestamp(),
            sleep=lambda seconds: s.clock.advance(seconds),
            endpoint=endpoint,
        )
...
def test_failed_endpoint_binding_still_deletes_only_bound_pod(setup):
    ...
    with runner.State(s.config.trusted_state_directory) as state:
        result = runner.run(
            ...
            s.ledger,
            s.backend,
            SimpleNamespace(stop_gpu=lambda _: (_ for _ in ()).throw(ConnectionError())),
            state,
            clock=lambda: s.clock().timestamp(),
            sleep=lambda _: None,
...
def test_units_preserve_identity_boundary_and_have_no_approval_command():
    ...
    assert (
        "/etc/probe-calibration/acceptance.json" in submission and "/etc/probe-calibration/acceptance.json" in trusted
    )
...
def test_provider_log_reader_uses_authenticated_fixed_origin_and_bounds(setup, monkeypatch):
    ...
    line = {
        "source": "container",
        "line": f"256 {runner.fingerprint(public_key(2))} root@worker (ED25519)",
...
def test_unavailable_startup_retries_without_dispatch_then_stops_at_deadline(setup):
    ...
    with runner.State(s.config.trusted_state_directory) as state:
        result = runner.run(
            ...
            state,
            clock=lambda: s.clock().timestamp(),
            sleep=lambda _: s.clock.advance(30),
            endpoint=missing,
        )
    window = s.config.max_runtime_seconds - s.plan.cases[0].spec.limits.max_runtime_seconds - 120
...
def test_every_startup_stage_shares_cutoff_and_late_ready_worker_never_dispatches(setup, late_phase):
    ...
    assert (
        phases
        == ["endpoint", "configuration", "readiness"][
            : min(3, ["endpoint", "configuration", "readiness", "dispatch"].index(late_phase) + 1)
        ]
    )
...
def configure_settings(s, state):
    ...
    return {
        "host": "213.192.2.71",
        "ssh_port": 40125,
        "identity_file": Path(s.config.ssh_identity_file),
...
def configure_receipt(bundle):
    return {
        "schema_version": 1,
        "configured": True,
        "worker_config_sha256": runner.digest(bundle["worker_config"]),
...
def test_private_configuration_upload_is_fixed_pinned_and_not_replayed(setup):
    ...
    with runner.State(s.config.trusted_state_directory) as state:
        ...
        result = runner.configure_worker(
            s.config, s.plan, settings, state, deadline=time.time() + 60, command=command, owner=os.geteuid()
        )
        assert result == configure_receipt(bundles[0])
        assert (
            runner.configure_worker(
                s.config, s.plan, settings, state, deadline=time.time() + 60, command=command, owner=os.geteuid()
            )
            == result
        )
        ...
        assert calls[1][-7:] == [
            ...
            "--configure",
            "/run/probe-worker-bootstrap.json",
        ]
...
def test_configuration_transport_uncertainty_never_replays_and_removes_private_file(setup, failure_after_upload):
    ...
    with runner.State(s.config.trusted_state_directory) as state:
        ...
        first = runner.configure_worker(
            s.config, s.plan, settings, state, deadline=time.time() + 60, command=command, owner=os.geteuid()
        )
        assert first == {
            "configured": None,
            "reason": "CONFIGURATION_RESPONSE_UNCERTAIN",
            "diagnostic": {
                "phase": "configure" if failure_after_upload else "upload",
                "exit_status": None,
                "timeout": False,
...
def test_configure_failure_persists_only_strict_diagnostic_and_never_replays(setup):
    ...
    with runner.State(s.config.trusted_state_directory) as state:
        ...
        def command(argv, *, timeout):
            ...
            code = (
                "import sys;print("
                + repr(json.dumps(bootstrap))
                + ');sys.stderr.write("SYNTHETIC_PRIVATE_STDERR");sys.exit(1)'
            )
        ...
        assert result == runner.configure_worker(
            s.config, s.plan, settings, state, deadline=time.time() + 60, command=command, owner=os.geteuid()
        )
...
def test_startup_failure_observes_safe_bootstrap_before_unconditional_deletion(setup, log_failure):
    ...
    with runner.State(s.config.trusted_state_directory) as state:
        result = runner.run(
            ...
            s.ledger,
            s.backend,
            SimpleNamespace(stop_gpu=stop),
            state,
            clock=lambda: s.clock().timestamp(),
            sleep=lambda _: None,
            endpoint=lambda *_: {},
            tunnel_factory=FailedTunnel,
...
def test_worker_configuration_provenance_refuses_even_freshly_pinned_mismatch(setup, field, value):
    ...
    config = s.config.model_copy(
        update={"worker_config_sha256": "sha256:" + hashlib.sha256(path.read_bytes()).hexdigest()}
    )
...
def test_command_output_and_runtime_are_bounded_without_exposing_stderr():
    ...
    with pytest.raises(runner.RunnerError, match="^SSH_VERIFICATION_COMMAND_FAILED$"):
        runner.command_bytes([sys.executable, "-I", "-c", "import time;time.sleep(20)"], timeout=0.1)
    assert time.monotonic() - started < 3


def select_wait_case(s, name):
    inputs = s.plan.cases[0].spec.inputs
    generated = fixed_plan(
        s.plan.model, "fixed-runtime-case", inputs.dataset_revision, inputs.prompt_set_hash, inputs.prompt_ids
    )
...
class ActionEndpoint(Endpoint):
    ...
    def status(self, attempt_id):
        self.queries += 1
        if self.complete or (
            self.s.plan.cases[0].name == "supervisor-restart"
...
def run_action_case(
    s, client, state, *, action=runner.run_action, sleep=None, lifecycle_factory=None, tunnel_factory=Tunnel
):
    settings = {
        ...
        "ssh_port": 40125,
        "identity_file": Path(s.config.ssh_identity_file),
        "known_hosts_file": s.root / "known_hosts",
    }
    return runner.run(
        s.config,
        s.plan,
        ...
        state,
        clock=lambda: s.clock().timestamp(),
        sleep=sleep or (lambda _: time.sleep(0.001)),
        endpoint=lambda *_: settings,
        tunnel_factory=tunnel_factory,
        client_factory=lambda *_a, **_k: client,
        readiness=lambda *_a, **_k: None,
        configure=lambda *_a, **_k: {"configured": True, "worker_config_sha256": client.config_sha256},
        lifecycle_factory=lifecycle_factory or (lambda _: client),
        action=action,
    )
...
def test_action_wait_renews_same_lease_without_extending_execution_deadline(setup, monkeypatch):
    ...
    monkeypatch.setattr(
        dispatch_module, "datetime", SimpleNamespace(now=lambda *_: s.clock(), fromtimestamp=datetime.fromtimestamp)
    )
...
def test_lifecycle_settings_use_configured_canonical_bytes_not_public_file_hash(setup):
    ...
    s.config = s.config.model_copy(
        update={"worker_config_sha256": "sha256:" + hashlib.sha256(public.read_bytes()).hexdigest()}
    )
    approve_fixture(s)
    client = ActionEndpoint(s)
    observed = []
...
def test_running_action_gate_refuses_mutation_after_interrupted_inspection(setup, interruption):
    ...
    dispatcher = runner.Dispatcher(
        s.ledger, client, worker_id=request["worker_id"], transfer_directory=s.root / "transfers"
    )
...
def test_action_gate_closes_before_tunnel_cleanup_can_release_pending_inspection(setup, monkeypatch, interruption):
    ...
    with runner.State(s.config.trusted_state_directory) as state:
        result = run_action_case(
            s, client, state, sleep=fail_main, tunnel_factory=ClosingTunnel, action=lambda *_a, **_k: calls.append(True)
        )
...
def test_hard_deadline_still_rejects_different_recorded_timeout_reason(setup):
    ...
    select_wait_case(s, "hard-deadline")
    request = approve_fixture(s)
    client = LimitEndpoint(
        s, "timeout", "UnrelatedTimeout", observed_changes={"error_code": "ExecutionDeadlineExceeded"}
    )
    with runner.State(s.config.trusted_state_directory) as state:
        result = run_case(s, client, state)
        observation = state.read("observations.json")["cases"][0]
...
def test_changed_final_failure_code_cannot_reclassify_recorded_policy_error(setup, recorded_code):
    ...
    select_wait_case(s, "output-limit")
    request = approve_fixture(s)
    client = LimitEndpoint(s, "policy", recorded_code, observed_changes={"error_code": "OutputLimitExceeded"})
    with runner.State(s.config.trusted_state_directory) as state:
        result = run_case(s, client, state)
        observation = state.read("observations.json")["cases"][0]
