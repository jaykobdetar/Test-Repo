...
def stored(h, request):
    with closing(sqlite3.connect(h["watcher"].state_path)) as connection:
        row = connection.execute(
            "SELECT snapshot,idle_since,last_stop_reason FROM watch_schedule WHERE request_id=?",
            (request["request_id"],),
        ).fetchone()
...
def test_pull_longer_than_five_minutes_is_allowed_only_until_shared_cutoff(harness):
    ...
    h["clock"].advance(1)
    assert h["watcher"].tick() == [
        {"worker_id": request["worker_id"], "reason": "startup_deadline", "confirmed_off": True}
    ]
...
def test_crashed_runner_and_restarted_watchdog_cannot_renew_startup(harness):
    ...
    restarted = StopWatchdog(
        h["ledger"].path,
        StopOnlyBackend(h["backend"]),
        state_path=h["watcher"].state_path,
        health_path=h["health"],
        clock=h["clock"],
    )
...
def test_first_tick_after_startup_cutoff_stops_without_fresh_grace(harness):
    ...
    restarted = StopWatchdog(
        h["ledger"].path,
        StopOnlyBackend(h["backend"]),
        state_path=h["tmp_path"] / "new-watch-state.sqlite",
        health_path=h["health"],
        clock=h["clock"],
    )
    assert restarted.tick() == [
...
def test_invalid_stop_timestamp_cannot_create_startup_or_future_idle(harness, invalid_stop):
    ...
    h["ledger"]._submit(
        lambda c, _: c.execute("UPDATE attempts SET stopped_at=? WHERE attempt_id=?", (value, active.attempt_id))
    )
    h["clock"].advance(301)
    assert h["watcher"].tick()[0]["reason"] == "five_minute_idle"
    snapshot, idle, _ = stored(h, request)
...
def test_five_minute_infrastructure_probe_keeps_absolute_deadline(harness):
    ...
    request = h["controller"].request_infrastructure_preflight(
        spec, script_sha256="sha256:" + "c" * 64, max_runtime_seconds=300
    )
...
def test_durable_stop_latch_cannot_be_cleared_by_new_startup_rule(harness, reason):
    ...
    with closing(sqlite3.connect(h["watcher"].state_path)) as connection:
        connection.execute(
            "UPDATE watch_schedule SET last_stop_reason=? WHERE request_id=?", (reason, request["request_id"])
        )
        connection.commit()
    h["clock"].advance(50)
...
def test_ledger_outage_stops_cached_startup_immediately(harness, monkeypatch):
    ...
    assert h["watcher"].tick() == [
        {"worker_id": request["worker_id"], "reason": "ledger_unavailable", "confirmed_off": True}
    ]
...
def query_request(h, request):
    with h["ledger"].read_connection() as connection:
        row = connection.execute(
            "SELECT r.*, a.consumed_at,a.deadline AS absolute_deadline,a.document AS approval_document "
...
def test_only_exact_approved_unexecuted_job_set_qualifies(harness, fault):
    ...
    if fault in {"purpose", "worker", "batch", "runtime"}:
        ...
        h["ledger"]._submit(
            lambda c, _: c.execute("DELETE FROM approval_jobs WHERE approval_id=?", (started["approval_id"],))
        )
    else:
        active = h["ledger"].dispatch_next(request["worker_id"], approval_id=started["approval_id"])
        h["ledger"].cancel_job(active.job_id)
        ...
        h["ledger"]._submit(
            lambda c, _: c.execute(
                "UPDATE jobs SET state='PENDING',attempt_id=NULL,worker_id=NULL,approval_id=NULL,"
                "lease_expires_at=NULL,attempt_count=0,retry_count=0 WHERE job_id=?",
                (jobs[0].job_id,),
            )
        )
    with h["ledger"].read_connection() as connection:
...
@pytest.mark.parametrize(
    "deadline,runtime",
    [(float("nan"), 240), (float("inf"), 240), (0, 240), (True, 240), (1000, True), (1000, 0), (1000, 86401)],
)
