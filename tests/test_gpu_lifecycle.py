...
def execution(tmp_path, job_factory, monkeypatch):
    ...
    config = WorkerConfig(
        ...
        output_directory=str(tmp_path / "attempts"),
        code_git_commit="a" * 40,
        environment_lock_path=str(tmp_path / "lock"),
        provider_backend="local_cpu",
        region="test",
        live_price_usd_per_hour=0,
    )
    ...
    result = dict(
        ...
        signals=signals,
        write=write,
        config_sha256=lifecycle._sha(config_path.read_bytes()),
    )
    try:
        yield result
    finally:
...
def restart(execution, action_id="a" * 64):
    ...
    return lifecycle.restart(
        ...
        action_id,
        config_sha256=execution["config_sha256"],
        expected_before=execution["expected_before"],
        _policy=execution["policy"],
    )
...
def test_inspection_matches_metadata_without_token(execution):
    ...
    assert snapshot["execution_started_sha256"] == lifecycle._sha(
        (execution["directory"] / "execution-started.json").read_bytes()
    )
...
def cancelled(execution):
    ...
    proof = {
        ...
        **{key: getattr(request, key) for key in ("job_id", "attempt_id", "worker_id", "approval_id")},
        ...
        "config_sha256": execution["marker"]["config_sha256"],
        **execution["child"],
        "signal": "SIGKILL",
        "signal_scope": "process_group",
        "signal_sent_at": sent.isoformat(),
...
def test_fixed_remote_helper_interpreter_matches_installed_worker_image(tmp_path, monkeypatch):
    ...
    client = actions.SSHActionClient(
        {
            "host": "127.0.0.1",
            "user": "root",
            "ssh_port": 22,
