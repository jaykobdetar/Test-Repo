...
def stopped(harness):
    ...
    assert h["watcher"].tick() == [
        {"worker_id": started["worker_id"], "reason": "five_minute_idle", "confirmed_off": True}
    ]
    ...
    expected = dict(
        ...
        provider_id=started["observed_provider_id"],
        job_id=h["job"].job_id,
        ...
        with closing(sqlite3.connect(path)) as connection:
            if snapshot is not None:
                raw = connection.execute(
                    "SELECT snapshot FROM watch_schedule WHERE request_id=?", (expected["request_id"],)
                ).fetchone()[0]
                columns["snapshot"] = snapshot(json.loads(raw))
            for column, value in columns.items():
                assert column in {"snapshot", "idle_since", "last_stop_reason", "confirmed_off"}
                connection.execute(
                    "UPDATE watch_schedule SET " + column + "=? WHERE request_id=?", (value, expected["request_id"])
                )
            connection.commit()
...
def test_real_watchdog_first_idle_stop_matches_exact_pins_without_writes(stopped, monkeypatch, mode):
    ...
    assert all(
        statement.startswith(("PRAGMA query_only=ON", "PRAGMA trusted_schema=OFF", "SELECT request_id,snapshot"))
...
def test_fractional_epoch_deadline_is_exact_without_rounding_tolerance(stopped):
    ...
    f.update(
        snapshot=lambda body: json.dumps(
            dict(body, deadline=f.expected["deadline"], absolute_deadline=f.expected["deadline"], consumed_at=consumed)
        ),
        idle_since=consumed,
    )
    assert f.verify() is True
    with pytest.raises(v.VerificationError):
        f.verify(deadline=f.expected["deadline"] + 0.000001)
...
def test_stop_reason_and_never_started_snapshot_are_strict(stopped, fault):
    ...
    if fault in {"reason", "unconfirmed", "prior_execution", "later_idle"}:
        column, value = {
            "reason": ("last_stop_reason", "absolute_deadline"),
            "unconfirmed": ("confirmed_off", 0),
            "prior_execution": ("idle_since", -1),
            "later_idle": ("idle_since", f.expected["deadline"] - 899),
        }[fault]
        ...
        def changed(body):
            if fault.startswith("snapshot_"):
                key = {
                    ...
                    "snapshot_worker": "worker_id",
                    "snapshot_approval": "approval_id",
                    "snapshot_provider": "observed_provider_id",
                }[fault]
                body[key] = "different"
            elif fault in {"jobs", "job_string", "extra_job"}:
                body["job_ids"] = {
                    "jobs": '["different"]',
                    "job_string": [f.expected["job_id"]],
                    "extra_job": json.dumps([f.expected["job_id"], "extra"]),
                }[fault]
                ...
                return '{"request_id":"duplicate",' + json.dumps(body)[1:]
            elif fault == "nonfinite":
                return json.dumps(body)[:-1] + ',"unexpected":NaN}'
            elif fault == "oversize":
                body["unexpected"] = "x" * v.MAX_SNAPSHOT
            elif fault == "not_object":
                return "[]"
            return json.dumps(body)

        f.update(snapshot=changed)
    with pytest.raises((v.VerificationError, sqlite3.Error)):
        f.verify()
...
def test_unsafe_database_or_parent_or_sidecar_refuses(stopped, fault):
    ...
    if fault == "owner":
        with pytest.raises(v.VerificationError):
            v.verify(f.path, owner=os.geteuid() + 1, trusted_root=f.h["tmp_path"], **f.expected)
        return
    if fault == "group_write":
        f.path.chmod(0o660)
    elif fault == "other_read":
        f.path.chmod(0o644)
    elif fault in {"symlink", "hardlink"}:
        original = f.path.with_name("original.sqlite")
        f.path.rename(original)
        if fault == "symlink":
            f.path.symlink_to(original)
        else:
            os.link(original, f.path)
    elif fault == "parent_symlink":
        original = f.path.parent.with_name("original-watchdog")
        f.path.parent.rename(original)
        f.path.parent.symlink_to(original, target_is_directory=True)
    elif fault == "parent_write":
        f.path.parent.chmod(0o770)
    else:
        sidecar = Path(str(f.path) + "-wal")
        if fault == "sidecar_link":
            sidecar.symlink_to(f.path)
        else:
...
@pytest.mark.parametrize(
    "fault", [None, "non_root", "mismatch", "private_error", "invalid_deadline", "unknown_argument"]
)
def test_cli_is_root_only_and_prints_only_bounded_status(stopped, monkeypatch, capsys, fault):
    ...
    def check(path, **kwargs):
        ...
        if fault in {"mismatch", "private_error"}:
            raise RuntimeError("PRIVATE_SNAPSHOT_WITH_IDS")
    ...
    if fault == "invalid_deadline":
        args[-1] = "PRIVATE_INVALID_DEADLINE"
    ...
    assert json.loads(output.out) == {
        "schema_version": 1,
        "status": "passed" if fault is None else "failed",
        "verified": fault is None,
    }
    assert all(value not in output.out for value in ["PRIVATE", *map(str, f.expected.values())])
    if fault in {"non_root", "invalid_deadline", "unknown_argument"}:
        assert seen == []
