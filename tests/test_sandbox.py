...
def job():
    ...
    return JobSpec(
        ...
        limits={"max_runtime_seconds": 30, "max_output_bytes": 100000},
    )
...
@pytest.mark.parametrize(
    ...
    [
        b"{}",
        b"not json",
        b'{"kind":"start_gpu","request_id":"r1","job_key":"approved"}',
        b'{"kind":"gpu_request","request_id":"r1","job_key":"approved","code":"arbitrary"}',
        b'{"kind":"gpu_request","request_id":"r1","job_key":"unapproved"}',
    ],
)
...
def test_broker_does_not_return_controller_exception_secrets(job):
    ...
    response = GPURequestBroker({"approved": job}, fail).handle(
        b'{"kind":"gpu_request","request_id":"r1","job_key":"approved"}'
    )
...
@pytest.mark.parametrize(
    "path", ["../escape", "/etc/passwd", "a/../escape", "a//b", "a\\b", "a/%2e%2e/b", "a\x00", "."]
)
...
def test_real_container_enforces_memory_cgroup(real_sandbox):
    result = real_sandbox.run(
        "x = bytearray(1024 * 1024 * 1024); print('unexpected allocation')",
        limits=SandboxLimits(memory_bytes=128 * 1024**2),
    )
