...
def public_config():
    ...
    return {
        "model": fixture["model"],
        "assets": [{"path": "model.safetensors", "sha256": "sha256:" + "a" * 64}],
        "datasets": [
            {
                "path": "/opt/probe-assets/datasets/public-calibration-prompts.json",
...
def success(execution, artifacts):
    ...
    data["run"].update(
        run_id=execution.job_id,
        started_at=NOW.isoformat(),
        experiment_stage="calibration",
        hypothesis_id=None,
        preregistration_hash=None,
        approval_id=execution.approval_id,
        explorer_session_id=execution.science.session_id,
    ...
    data["experiment"].update(
        ...
        modules=["model.layers.14"],
        positions=["last"],
        intervention_hash=Ledger.operation_hash(execution.spec),
        predicted_direction=execution.science.predicted_direction,
        primary_metric=execution.science.primary_metric,
        falsifier=execution.science.falsifier,
    ...
    data["results"].update(
        effect_size=None, confidence_interval=None, heldout=False, replication_status="not_applicable"
    )
    data["cost"]["bytes_persisted"] = sum(len(body) for body in artifacts.values())
    data["artifacts"] = [
        {"path": name, "sha256": hashlib.sha256(body).hexdigest(), "retention_class": "derived"}
...
class FakeTransport:
    def __init__(self, execution):
        ...
        self.artifacts = {
            "summary.json": encoded({"observation": "fixed public fixture"}),
            "nested/tensors.safetensors": tensor_bytes(),
        }
        self.receipt = ExecutionReceipt(
            job_id=execution.job_id, attempt_id=execution.attempt_id, state="RUNNING", started_at=NOW
        )
        self.tensors = {}
    ...
    def __call__(self, command, payload, *, timeout, max_output_bytes):
        ...
        if operation in {"stage", "upload"}:
            ...
            self.upload_receipt = {
                "path": sha(body)[7:] + "/tensor.safetensors",
                "sha256": sha(body),
        ...
        if operation == "start":
            ...
            self.receipt = ExecutionReceipt(
                job_id=self.execution.job_id, attempt_id=self.execution.attempt_id, state="RUNNING", started_at=NOW
            )
            if self.lose_start:
...
def setup(tmp_path, execution, public_config):
    ...
    known.write_bytes(b"synthetic-pinned-host-key")
    known.chmod(0o600)
    settings = {
        "host": "203.0.113.10",
        "user": "root",
    ...
    client = SSHJobClient(
        settings, public_config, NOW + timedelta(seconds=900), transport=transport, clock=lambda: NOW.timestamp()
    )
...
def test_lost_start_preserves_request_and_never_automatically_retries(setup, execution):
    ...
    with pytest.raises(TransportError, match="cannot be rebound"):
        client.submit(different)
...
def test_same_client_refuses_new_attempt_after_uncertain_start(setup, execution):
    ...
    with pytest.raises(TransportError, match="fresh attempt"):
        client.submit(changed)
    with pytest.raises(TransportError, match="fresh attempt"):
        client.status("fresh-attempt")
    assert [row[0] for row in transport.calls] == ["start"]
...
def test_start_identity_mismatch_refused(setup, execution, field, value):
    ...
    with pytest.raises(TransportError, match="different execution identity"):
        client.submit(execution)
...
def test_malformed_receipts_redacted(setup, execution, raw):
    client, _, _ = setup
    client._transport = lambda *args, **kwargs: raw
    with pytest.raises(TransportError) as error:
        client.submit(execution)
    assert "synthetic-secret" not in str(error.value)
...
def test_cancellation_preserves_completed_outcome(setup, execution):
    client, transport, _ = setup
    transport.receipt = success(execution, transport.artifacts)
...
def test_pinned_helper_staging_checks_source_and_remote_readback(setup, tmp_path):
    ...
    script.write_bytes(b'print("fixed")\n')
    ...
    with pytest.raises(TransportError):
        client.stage_helper(script, "sha256:" + "0" * 64)
    assert len(transport.calls) == 1
    client._transport = lambda *args, **kwargs: encoded(
        dict(result, sha256="sha256:" + "0" * 64, bytes=script.stat().st_size, uid=0, mode=0o444)
    )
    with pytest.raises(TransportError, match="readback"):
        client.stage_helper(script, sha(script.read_bytes()))
...
def test_changed_trust_file_prevents_any_remote_command(setup, execution):
    ...
    with pytest.raises(TransportError, match="trust file changed"):
        client.submit(execution)
    assert not transport.calls
...
def test_corrupt_tensor_transfer_never_succeeds(setup, tmp_path, fault):
    ...
    if fault == "upload_receipt":
        client._transport = lambda *args, **kwargs: encoded({"path": "wrong"})
    with pytest.raises(TransportError):
        client.upload_tensor(path, expected_sha256=expected)
...
def test_download_refuses_bad_evidence_or_destination(setup, execution, tmp_path, fault):
    client, transport, _ = setup
    receipt = success(execution, transport.artifacts)
    target = tmp_path / "copied"
    max_bytes = 1024 * 1024
    if fault == "hash":
        transport.artifacts["summary.json"] += b"tampered"
    if fault == "total":
        data = receipt.model_dump(mode="json")
        data["manifest"]["cost"]["bytes_persisted"] += 1
    ...
    if fault in {"symlink", "hardlink"}:
        ...
        other.write_bytes(transport.artifacts["summary.json"])
        if fault == "symlink":
            (target / "summary.json").symlink_to(other)
        else:
            os.link(other, target / "summary.json")
    if fault == "oversized_stream":
        client._transport = lambda *args, **kwargs: b"x" * (kwargs["max_output_bytes"] + 1)
    with pytest.raises(TransportError):
        client.download(receipt, target, max_bytes=max_bytes)
...
def test_dispatcher_can_seal_real_bytes_via_ssh_adapter(setup, execution, tmp_path):
    ...
    with Ledger(tmp_path / "ledger.sqlite", clock=lambda: NOW) as ledger:
        ...
        approval = ApprovalNonce(
            ...
            token="x" * 40,
            pod_id="worker",
            batch_hash=ledger.batch_hash([job.job_id]),
...
def test_actual_byte_transport_bounds_output_and_redacts_stderr():
    command = [
        sys.executable,
        "-c",
        'import sys;sys.stderr.write("synthetic-secret");sys.stdout.buffer.write(sys.stdin.buffer.read())',
    ]
...
def test_actual_timeout_kills_group_after_leader_exits(tmp_path):
    ...
    with pytest.raises(TransportError, match="timed out"):
        bounded_command([sys.executable, "-c", program, str(pidfile)], b"", timeout=0.25, max_output_bytes=100)
...
def test_inspect_binds_request_config_deadline_and_process_shapes(setup, execution, fault):
    ...
    value = {
        "receipt": transport.receipt.model_dump(mode="json"),
        ...
        "monitor_identity": dict(identity, pid=122),
    ...
    if fault == "config":
        value["config_sha256"] = "sha256:" + "0" * 64
    if fault == "request":
        value["request_sha256"] = "sha256:" + "0" * 64
    if fault == "attempt":
        value["receipt"]["attempt_id"] = "other"
    if fault == "deadline":
        value["original_deadline"] = (client.absolute_deadline + timedelta(seconds=1)).isoformat()
    if fault == "process":
        value["child_identity"]["pid"] = True
    if fault == "fields":
        value["extra"] = "unexpected"
    client._transport = lambda *args, **kwargs: encoded(value)
    if fault:
        with pytest.raises(TransportError):
            client.inspect(execution.attempt_id)
    else:
...
def test_actual_remote_helper_transfer_and_inspect_protocol(setup, execution, tmp_path, monkeypatch):
    ...
    def transport(command, payload, **_):
        ...
        if operation == "read-tensor":
            return helper.read_tensor(body, workspace)
        if operation == "artifact":
            return helper.artifact(body, state, workspace)
        if operation == "inspect":
            return encoded(helper.inspect_job(body, state, workspace))
        raise AssertionError(operation)
    ...
    output = workspace / "job-output"
    output.mkdir(mode=0o700)
    for name, body in outputs.items():
        (output / name).write_bytes(body)
    claim = {
        "request": execution.model_dump(mode="json"),
        "request_sha256": sha(canonical_json(execution.model_dump(mode="json")).encode()),
        "config_sha256": sha(canonical_json(client.config).encode()),
        "original_deadline": client.absolute_deadline.isoformat(),
        "effective_job_deadline": execution.deadline.isoformat(),
