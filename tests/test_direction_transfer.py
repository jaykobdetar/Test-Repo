...
from probe_core.direction_transfer import (DIRECTION_BYTES, DIRECTION_NAME, DIRECTION_SHA256,
    READBACK_PROGRAM, controller_direction, direction_reference)
...
def prepared(setup, monkeypatch):
    ...
    s.plan = fixed_direction_plan(s.plan.model, 'fixed-public-direction-v1', inputs.dataset_revision,
...
class Wire:
    ...
    def open(self, request, timeout):
        ...
        if request.method == 'PUT':
            ...
            receipt = Supervisor.upload_tensor(SimpleNamespace(config=SimpleNamespace(
                tensor_directory=str(self.s.remote), max_tensor_bytes=DIRECTION_BYTES)),
                path.rsplit('/', 1)[1], request.data, int(request.get_header('Content-length')))
    ...
    def readback(self, settings, phase, *, deadline, clock):
        ...
        completed = subprocess.run([sys.executable, '-I', '-c', READBACK_PROGRAM,
            str(self.s.remote), DIRECTION_SHA256.removeprefix('sha256:'), str(os.geteuid()), phase],
...
def execute(s, wire, state, *, readback=None):
    return runner.run(s.config, s.plan, s.ledger, s.backend,
        ...
        tunnel_factory=Tunnel, client_factory=lambda *_a, **_k: wire.client,
...
def test_real_upload_hash_readback_and_sealed_manifest_precede_pass(prepared):
    ...
    assert (s.remote/direction_reference()['path']).read_bytes() == (s.registry/direction_reference()['path']).read_bytes()
...
@pytest.mark.parametrize('fault', ['controller_corrupt', 'worker_preexisting', 'upload_digest', 'upload_shape',
                                 'upload_shape_float', 'worker_corrupt', 'readback_missing', 'readback_timeout'])
    ...
    def readback(*args, **kwargs):
        if args[1] == 'after':
            if fault == 'worker_corrupt':
                ...
                runner.command_bytes([sys.executable, '-I', '-c', 'import time; time.sleep(5)'], timeout=.05)
...
@pytest.mark.parametrize('fault', ['missing', 'attempt', 'deadline', 'approval', 'operation',
                                 'upload', 'readback', 'receipt', 'controller_after'])
def test_collector_requires_exact_transfer_proof_and_unchanged_controller_bytes(prepared, fault):
    ...
    if fault == 'missing':
        ...
        def altered(attempt_id):
            ...
            return receipt.model_copy(update={'manifest': receipt.manifest.model_copy(update={'experiment': experiment})})
...
@pytest.mark.parametrize('field,value', [('strength', 0.0), ('positions', [0]),
...
def test_fixed_ssh_command_has_only_the_pinned_path_and_bounded_time(prepared):
    s = prepared
    settings = {'host': 'fixture.example', 'ssh_port': 40103,
    ...
    def command(argv, *, timeout):
        ...
        return '{"state":"absent"}'
    assert runner.read_worker_direction(settings, 'before', deadline=s.clock().timestamp()+10,
                                       clock=lambda: s.clock().timestamp(), command=command) == {'state': 'absent'}
    ...
    with pytest.raises(runner.RunnerError, match='DIRECTION_READBACK_DEADLINE'):
        runner.read_worker_direction(settings, 'after', deadline=s.clock().timestamp(),
                                     clock=lambda: s.clock().timestamp(), command=command)
    assert len(commands) == 1


@pytest.mark.parametrize('fault', ['symlink_file', 'symlink_directory', 'hardlink_file', 'public_directory',
                                 'writable_file', 'oversized_file', 'extra_file'])
def test_actual_readback_program_refuses_unsafe_or_unbounded_remote_input(prepared, fault):
    ...
    completed = subprocess.run([sys.executable, '-I', '-c', READBACK_PROGRAM, str(s.remote),
        DIRECTION_SHA256.removeprefix('sha256:'), str(os.geteuid()), 'after'], capture_output=True, timeout=3)
