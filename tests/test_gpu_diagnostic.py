...
def test_diagnostic_rejects_incompatible_driver_and_never_claims_stop(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        implementation,
        "_command",
        lambda _: {"returncode": 0, "stdout": "NVIDIA GeForce RTX 4090, GPU-test, 570.1, 24564, 8.9\n", "stderr": ""},
    )
...
def test_diagnostic_accepts_one_compatible_gpu_only_after_cgroup_probe(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda _: {
            "returncode": 0,
            "stdout": "NVIDIA GeForce RTX 4090, GPU-test, 580.65.06, 24564, 8.9\n",
            "stderr": "",
        },
    )
...
def test_write_open_reports_kernel_denial_not_mode_bits(tmp_path, monkeypatch, code):
    ...
    result = implementation.inspect_cgroup(tmp_path)
    assert all(not item["opened"] and item["errno"] == code for item in result["open_for_write_without_write"].values())
    assert result["exclusive_ownership_verified"] is False
...
def test_inspection_refuses_control_symlink_and_fifo_without_hanging(tmp_path, monkeypatch):
    ...
    (tmp_path / "cgroup.procs").unlink()
    (tmp_path / "cgroup.procs").symlink_to(target)
    (tmp_path / "cgroup.threads").unlink()
    os.mkfifo(tmp_path / "cgroup.threads")
    monkeypatch.setattr(implementation, "_filesystem_type", lambda _: 0x63677270)
    result = implementation.inspect_cgroup(tmp_path)
...
def test_mount_parser_selects_containing_mount_and_decodes_escapes():
    ...
    assert implementation._mounts(text, "/sys/fs/cgroup/worker") == [
        {
            "mount_id": "1",
            "root": "/",
...
def test_worker_inspection_failure_never_falls_back_to_root(tmp_path, monkeypatch, outcome):
    ...
    def fail(*args, **kwargs):
        if outcome == "timeout":
            raise subprocess.TimeoutExpired("inspect", 10)
        if outcome == "permission":
            raise PermissionError(errno.EPERM, "cannot set identity")
        ...
        data = {
            "malformed": "{",
            "oversized": "x" * (implementation.MAX_RESULT_BYTES + 1),
            "wrong_identity": json.dumps(value),
            "failed": "{}",
        }[outcome]
...
def test_existing_worker_identity_is_not_switched_again(tmp_path, monkeypatch):
    implementation = module()
    current = {"worker_identity_verified": True}
...
def test_successful_root_probe_cannot_certify_worker_prerequisites(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        implementation,
        "_command",
        lambda _: {"returncode": 0, "stdout": "NVIDIA GeForce RTX 4090, GPU-test, 580.65.06, 24564, 8.9\n"},
    )
    monkeypatch.setattr(implementation, "inspect_cgroup", lambda _: {"worker_identity_verified": False})
    monkeypatch.setattr(
        implementation, "worker_inspection", lambda *_: {"worker_identity_verified": True, "inspection_only": True}
    )
    monkeypatch.setattr(implementation, "cgroup_probe", lambda _: {"passed": True})
...
@pytest.mark.parametrize(
    "row",
    [
        "NVIDIA RTX 3090, GPU-test, 580.65.06, 24564, 8.9",
        "NVIDIA GeForce RTX 4090, GPU-test, 580.65.06, 123, 8.9",
        "NVIDIA GeForce RTX 4090, GPU-test, 580.65.06, 22999, 8.9",
        "NVIDIA GeForce RTX 4090, GPU-test, 580.65.06, 24564, inf",
        "NVIDIA GeForce RTX 4090, GPU-test, 580.65.06, 24564, nan",
        "NVIDIA GeForce RTX 4090, GPU-test, 580.65.06, 24564, invalid",
        "NVIDIA GeForce RTX 4090, GPU-test, 570.1, 24564, 8.9",
    ],
)
...
def test_inventory_includes_legacy_and_hybrid_mounts_below_tmpfs_root():
    implementation = module()
    data = (
        "10 1 0:1 / /sys/fs/cgroup ro - tmpfs tmpfs ro\n"
...
def fixture_mount(path, controllers="memory", root="/"):
    return {
        ...
        "mount_options": ["rw"],
        "super_options": ["rw", *controllers.split(",")],
        "optional_fields": [],
    }
...
def test_v1_own_subtree_and_outer_limits_are_read_without_changes(tmp_path, monkeypatch):
    ...
    result = implementation._inventory(
        [fixture_mount(tmp_path)], {"text": "9:memory:/docker/pod\n", "truncated": False}
    )
...
@pytest.mark.parametrize(
    "membership,expected_method",
    [("/docker/pod", "mount_root_relative"), ("/", "namespace_relative_requires_pid_proof")],
)
def test_bind_mount_and_namespace_relative_resolution_require_own_pid(
    tmp_path, monkeypatch, membership, expected_method
):
    implementation = module()
    mount = fixture_mount(tmp_path, "", "/docker/pod")
...
@pytest.mark.parametrize(
    "unsafe", ["/../pod", "/docker/../../pod", "/docker/./pod", "/docker//pod", "relative", "/pod\x00"]
)
...
def test_readonly_inventory_cli_never_runs_active_probe_or_gpu_query(tmp_path, monkeypatch, capsys):
    implementation = module()
    inspection = {"worker_identity_verified": False, "inspection_only": True}
