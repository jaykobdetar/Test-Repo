...
def public_data():
    ...
    return {'model': {'repo': lock.repo, 'revision_sha': lock.revision, 'tokenizer_revision': lock.revision,
                     'local_weight_hashes': [item.sha256 for item in lock.files if item.path.endswith('.safetensors')],
                     'dtype': 'bfloat16', 'quantized': False, 'chat_template_hash': None, 'thinking_mode': None},
            ...
            'datasets': [{'path': '/opt/probe-assets/datasets/public-calibration-prompts.json',
                          'sha256': calibration.DATASET_SHA}],
...
@pytest.mark.parametrize('change', [
    {'cgroup_directory': '/sys/fs/cgroup/false-claim'}, {'operation': {'kind': 'generate'}},
    {'model_directory': '/arbitrary'}, {'callback': 'anything'}, {'live_price_usd_per_hour': 1.50},
    {'live_price_usd_per_hour': float('nan')}, {'container_image_digest': 'latest'},
])
...
@pytest.mark.parametrize('field,value', [
    ('repo', 'probe/testing-tiny-qwen3'), ('revision_sha', 'b' * 40),
    ('tokenizer_revision', 'b' * 40), ('local_weight_hashes', ['sha256:' + 'b' * 64]),
    ('dtype', 'float32'), ('quantized', True), ('thinking_mode', False),
])
...
def test_asset_inventory_cannot_expand(public_data, mutation):
    if mutation == 'extra_asset': public_data['assets'].append({'path': 'model.py', 'sha256': 'sha256:' + 'b' * 64})
    if mutation == 'duplicate_asset': public_data['assets'].append(public_data['assets'][0])
    if mutation == 'missing_asset': public_data['assets'].pop()
    if mutation == 'other_dataset': public_data['datasets'][0]['sha256'] = 'sha256:' + 'b' * 64
    if mutation == 'other_path': public_data['datasets'][0]['path'] = '/private/heldout.json'
    with pytest.raises(ValueError):
        calibration.PublicCalibrationConfig.model_validate(public_data)
...
def asset_environment(config, monkeypatch):
    ...
    documents = {str(calibration.BAKED_MANIFEST): json.dumps(baked).encode(),
...
def test_runtime_asset_and_source_tampering_refused(config, asset_environment, fault):
    s = asset_environment
    if fault == 'source': s.provenance['source_commit'] = 'b' * 40
    if fault == 'lock': s.provenance['environment_lock_hash'] = 'sha256:' + 'b' * 64
    if fault == 'asset_hash': s.baked['assets'][0]['sha256'] = 'sha256:' + 'b' * 64
    if fault == 'extra_asset': s.baked['assets'].append({'path': 'private/heldout.json', 'sha256': 'sha256:' + 'b' * 64})
    if fault == 'duplicate_asset': s.baked['assets'].append(s.baked['assets'][0])
    if fault == 'dataset': s.documents[config.datasets[0].path] += b' '
...
def test_mutable_or_symlinked_asset_refused(config, asset_environment, monkeypatch, mode):
    ...
    monkeypatch.setattr(calibration.Path, 'lstat', lambda path: SimpleNamespace(st_uid=0, st_mode=mode, st_nlink=1)
                        if path == filename else original(path))
...
def test_posttrained_mode_requires_explicit_template_identity(public_data, thinking):
    ...
    prefix = 'models/' + lock.repo.split('/')[-1] + '/' + lock.revision + '/'
    public_data['assets'] = [{'path': item['path'][len(prefix):], 'sha256': item['sha256']}
                            for item in baked['assets'] if item['path'].startswith(prefix)]
    public_data['model'].update(repo=lock.repo, revision_sha=lock.revision, tokenizer_revision=lock.revision,
        local_weight_hashes=[item.sha256 for item in lock.files if item.path.endswith('.safetensors')],
        thinking_mode=thinking, chat_template_hash='sha256:' + 'b' * 64)
...
def fake_run(config, dataset, monkeypatch, tmp_path):
    ...
    def execute(self, request, output):
        ...
        class Value:
            def model_dump(self, **_): return {'synthetic': True}
        return SimpleNamespace(started_at=datetime.now(timezone.utc), finished_at=datetime.now(timezone.utc),
    ...
    return SimpleNamespace(config=config, output=tmp_path / 'result', calls=calls,
...
def test_one_shot_result_is_honest_standalone_evidence(fake_run):
    ...
    for field in ('scientific_evidence', 'heldout_data_used', 'installed_ledger_used',
                  'lifecycle_acceptance', 'nested_cgroup_limits_enforced', 'process_exit_and_pod_deletion_verified'):
...
def test_failed_attempt_is_irreversibly_claimed(fake_run, monkeypatch):
    def failed(*_): raise RuntimeError('synthetic-secret-must-not-be-published')
...
def test_cli_error_does_not_publish_exception_text(monkeypatch, capsys):
    def fail(*_): raise RuntimeError('synthetic-secret-must-not-be-published')
    monkeypatch.setattr(calibration, 'immutable_file', fail)
    assert calibration.main(['--config', '/run/example.json', '--run-id', 'run',
        '--absolute-deadline', '2026-09-21T00:00:00+00:00', '--output', '/tmp/public-calibration/result']) == 1
