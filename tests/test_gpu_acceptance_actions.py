...
from probe_core.gpu_acceptance_actions import (
    ActionError,
    ActionStore,
    _bounded_command,
...
class ObservedWorker:
    ...
    def __init__(self, request, clock):
        ...
        self.receipt = ExecutionReceipt(
            job_id=request.job_id, attempt_id=request.attempt_id, state="RUNNING", started_at=clock()
        )
        self.snapshot = {
...
def scenario(tmp_path, clock, job_factory):
    ...
    def make(action="restart_supervisor_after_running"):
        spec = JobSpec.model_validate(
            {**job_factory("case-" + str(len(created))).model_dump(mode="json"), "experiment_stage": "calibration"}
        )
        case = AcceptanceCase(
            name="lifecycle",
            action=action,
            expected_state="FAILED" if action == "cancel_after_running" else "COMPLETED",
            expected_failure_kind="cancelled" if action == "cancel_after_running" else None,
            spec=spec,
        )
        ...
        kwargs = dict(
            ...
            client=worker,
            lifecycle=worker,
            action_directory=tmp_path / (str(len(created)) + "-actions"),
            clock=clock,
...
def test_late_cancellation_cannot_accept_a_successful_receipt(scenario, manifest_data):
    ...
    def finish(item):
        ...
        item.receipt = ExecutionReceipt(
            job_id=item.request.job_id,
            attempt_id=item.request.attempt_id,
            state="SUCCEEDED",
            started_at=item.clock(),
            finished_at=item.clock(),
            process_stopped=True,
...
def add_cancel_proof(item):
    ...
    proof = {
        ...
        **{key: getattr(item.request, key) for key in ("job_id", "attempt_id", "worker_id", "approval_id")},
        ...
        "config_sha256": item.snapshot["loaded_config_sha256"],
        **item.snapshot["child"],
        "signal": "SIGKILL",
        "signal_scope": "process_group",
        "signal_sent_at": item.clock().isoformat(),
        "stopped_at": item.clock().isoformat(),
        "process_stopped": True,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("job_scope_stopped", False),
        ("result_present", True),
        ("signal_scope", "child_only"),
    ],
)
