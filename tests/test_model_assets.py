...
@pytest.fixture
def source():
    config = {"model_type":"qwen3", "num_hidden_layers":28, "num_attention_heads":16,
              "num_key_value_heads":8, "hidden_size":2048, "head_dim":128, "vocab_size":151936}
    contents = {"config.json":json.dumps(config).encode(), "tokenizer.json":b"{}",
    ...
    for name, body in contents.items():
        entries.append(LockedFile(path=name,size_bytes=len(body),
    ...
    lock = ModelLock(repo="Qwen/Qwen3-1.7B-Base",revision="a"*40,files=entries,source_url="https://huggingface.co/test-fixture")
...
def test_canonical_lock_has_exact_two_public_revisions_and_sizes():
    ...
    assert [lock.revision for lock in locks] == ["ea980cb0a6c2ae4b936e82123acc929f1cec04c1", "70d244cc86ccca08cf5af4e1e306ecf908b1ad5e"]
    assert sum(lock.size_bytes for lock in locks) == 7532122402
    assert all(entry.sha256 is not None for lock in locks for entry in lock.files if entry.path.endswith(".safetensors"))
...
def test_plan_is_offline_and_download_requires_exact_allowance(source,tmp_path):
    ...
    prepare(tmp_path,(lock,),max_download_bytes=0,opener=lambda *_: pytest.fail("already downloaded asset fetched again"))
