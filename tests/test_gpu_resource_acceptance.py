...
def test_private_scope_limits_cleanup_and_unrelated_files_are_preserved(tmp_path, monkeypatch):
    implementation = module()
    unrelated = tmp_path / "other-job"
    unrelated.mkdir()
    (unrelated / "data").write_text("unchanged")
...
def fake_scope(kind, status=0, message=None):
    return SimpleNamespace(
        ...
        launch=lambda: None,
        exited=lambda: True,
        message=lambda _deadline: message,
    )


@pytest.mark.parametrize(
    "usage,throttled,passed", [(1400000, 20, True), (1400000, 0, False), (2900000, 20, False), (0, 1, False)]
)
def test_cpu_requires_actual_throttling_and_bounded_accounted_usage(monkeypatch, usage, throttled, passed):
    implementation = module()
    values = iter([{"usage_usec": 0, "nr_throttled": 0}, {"usage_usec": usage, "nr_throttled": throttled}])
...
@pytest.mark.parametrize(
    "status,oom_kills,passed", [(signal.SIGKILL, 1, True), (signal.SIGKILL, 0, False), (0, 1, False)]
)
def test_memory_requires_both_kernel_oom_counter_and_sigkill(monkeypatch, status, oom_kills, passed):
    implementation = module()
    values = iter([{"oom_kill": 0}, {"oom_kill": oom_kills}])
    monkeypatch.setattr(implementation, "counters", lambda *_: next(values))
...
@pytest.mark.parametrize(
    "count,current,events,error",
    [(15, 16, 1, errno.EAGAIN), (12, 13, 1, errno.EAGAIN), (15, 16, 0, errno.EAGAIN), (15, 16, 1, errno.ENOMEM)],
)
def test_pids_rejects_outer_limit_and_unrelated_fork_failures(monkeypatch, count, current, events, error):
    implementation = module()
    values = iter([{"max": 0}, {"max": events}])
    monkeypatch.setattr(implementation, "counters", lambda *_: next(values))
...
def test_detached_descendants_require_both_pidfd_exit_and_empty_scope(monkeypatch):
    ...
    scope = fake_scope(
        "descendants",
        message={"leader": 12345, "descendant": 23456, "descendant_session": 23456, "descendant_group": 23456},
    )
    ...
    monkeypatch.setattr(
        implementation.select, "select", lambda descriptors, *_: (descriptors if writes else [], [], [])
    )
...
def test_child_guard_disables_parent_death_for_detached_proof_but_keeps_deadline(monkeypatch):
    ...
    monkeypatch.setattr(
        implementation.ctypes, "CDLL", lambda *_a, **_k: SimpleNamespace(prctl=lambda *args: calls.append(args) or 0)
    )
...
def test_cleanup_failure_prevents_following_tests_and_overall_success(monkeypatch):
    ...
    class FailedScope:
        def __init__(self, _root, kind, _deadline):
            self.name = "private-test"
            self.limits = {}
            instances.append(kind)

        def prepare(self):
            pass

        def cleanup(self, _deadline):
            raise implementation.AcceptanceError("ScopeStillPopulated")
...
@pytest.mark.parametrize(
    "maximum,current,passed", [(256 * 1024 * 1024, 32 * 1024 * 1024, True), (128 * 1024 * 1024, 0, False)]
)
def test_known_outer_memory_headroom_is_checked_without_changing_outer_limit(
    tmp_path, monkeypatch, maximum, current, passed
):
    implementation = module()
    (tmp_path / "memory.max").write_text(str(maximum))
    (tmp_path / "memory.current").write_text(str(current))
    monkeypatch.setattr(
        implementation, "filesystem", lambda fd: 0x63677270 if os.fstat(fd).st_ino == tmp_path.stat().st_ino else 0
    )
    if passed:
...
def test_adopted_descendants_are_reaped_until_no_children_remain(monkeypatch):
    implementation = module()
    outcomes = iter([(123, 9), (456, 9), None])
    seen = []

    def waiting(pid, options):
        seen.append((pid, options))
        value = next(outcomes)
        if value is None:
