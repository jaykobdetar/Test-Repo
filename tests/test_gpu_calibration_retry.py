...
def replacement(typed_inputs):
    ...
    for name in r.CALIBRATION:
        old[name] = (PROJECT / "deploy" / name).read_bytes()
    ...
    config.update(
        plan_path=str(r.PUBLIC / ("retry-" + retry_id) / "plan.json"),
        plan_sha256="sha256:" + r.digest(new["plan.json"]),
...
def test_scope_changes_rejected_before_any_action(replacement, change):
    ...
    if change in {"prompt", "model", "same_key"}:
        ...
        if change == "prompt":
            body["cases"][0]["spec"]["inputs"]["prompt_ids"][0] = "different"
        elif change == "model":
            body["model"]["revision_sha"] = "f" * 40
        else:
            body["cases"][0]["spec"]["idempotency_key"] = json.loads(s.old["plan.json"])["cases"][0]["spec"][
                "idempotency_key"
            ]
        s.new["plan.json"] = r.encoded(body)
    elif change in {"price", "runtime"}:
        body = json.loads(s.new["acceptance.json"])
        body["expected_worker_price_usd_per_hour" if change == "price" else "max_runtime_seconds"] = (
            0.79 if change == "price" else 800
        )
        s.new["acceptance.json"] = r.encoded(body)
    else:
        name = r.CALIBRATION[1]
        s.new[name] = (
            s.new[name].replace(b"User=probe-trusted", b"User=root")
            if change == "unit_user"
            else s.new[name] + b"\nOther=setting\n"
        )
    with pytest.raises(r.RetryError):
        r.validate_replacement(s.old, s.new, s.retry_id)
...
def real_failed(setup, monkeypatch):
    ...
    result = {
        "status": "failed",
        "stage": "verified_worker_startup",
        "reason": "ACCEPTANCE_RUNTIME_UNAVAILABLE",
    ...
    bound = {
        key: request[key]
        for key in ("request_id", "worker_id", "approval_id", "batch_hash", "deadline", "observed_provider_id")
    }
...
@pytest.mark.parametrize(
    "change", ["attempt", "not_absent", "other_pod", "open_approval", "wrong_result", "wrong_binding", "other_job"]
)
def test_known_failure_gate_rejects_any_execution_or_uncertain_scope(real_failed, change):
    ...
    if change == "attempt":
        snapshot["attempts"] = [{"attempt_id": "unexpected"}]
    elif change == "not_absent":
        snapshot["provider_absent"] = False
    elif change == "other_pod":
        snapshot["pods"] = 1
    elif change == "open_approval":
        snapshot["approvals"][0]["ended_at"] = None
    elif change == "wrong_result":
        result["stage"] = "approved_dispatch"
    elif change == "wrong_binding":
        bound["approval_id"] = "different"
    else:
        snapshot["other_unfinished_jobs"] = 1
    with pytest.raises(r.RetryError):
        r.validate_failed(snapshot, f.failed, f.plan, result, bound, f.submitted)
...
def test_close_phase_is_audited_once_and_reuses_durable_receipt(real_failed, tmp_path, monkeypatch):
    ...
    operation.work = tmp_path / "recovery"
    operation.work.mkdir()
    ...
    operation.operation = SimpleNamespace(
...
def prepare_case(tmp_path, replacement, monkeypatch):
    ...
    for name in r.CALIBRATION:
        put(units / name, replacement.old[name])
    config = json.loads(replacement.new["acceptance.json"])
    config.update(
        plan_path=str(public / "plan.json"), submission_state_directory=str(submit), trusted_state_directory=str(state)
    )
    files = dict(replacement.new, **{"acceptance.json": r.encoded(config)})
    events, fault = [], [None]
    a = SimpleNamespace(
        Activation=lambda: None, trusted=lambda _: None, write_file=lambda path, raw, **_: put(path, raw)
    )
    manifest = {
        "target_upgrade": {"wheel_sha256": "a" * 64},
        "failed": {"job_id": "old-job", "request_id": "old-request", "worker_id": "old-worker"},
    }
    operation = r.Recovery(a, None, manifest, files, "b" * 64)
    operation.work = tmp_path / "capsule"
    operation.work.mkdir()
    ...
    operation.record(
        ...
        {
            "status": "closed",
            "manifest_sha256": operation.pin,
            "failed_evidence_sha256": operation.failure_hash,
            "history": {"old": "retained"},
        },
    )
    ...
    def ctl(*args):
        ...
        if args == ("start", r.CALIBRATION[1]):
            ...
            expected = {
                "config_sha256": runner.digest(runner.RunnerConfig.model_validate(config).model_dump(mode="json")),
                "plan_sha256": config["plan_sha256"],
            }
            put(state / "binding.json", r.encoded(expected))

    operation.operation = SimpleNamespace(
        users=(994, 993, 1000), baseline=None, idle=lambda: events.append("idle") or {"idle": True}, ctl=ctl
    )
    ...
    def record(name, value):
        if name == "prepare-report.json" and fault[0] == "receipt":
            raise OSError("synthetic receipt failure")
        return original_record(name, value)

    operation.record = record
    return SimpleNamespace(
        operation=operation, units=units, public=public, submit=submit, state=state, events=events, fault=fault
    )
...
def test_prepare_stages_new_private_state_then_waits_for_bound_runner_without_approval(prepare_case):
    ...
    assert (
        s.events.index("idle")
        < s.events.index(("start", r.CALIBRATION[0]))
        < s.events.index(("start", r.CALIBRATION[1]))
    )
    ...
    assert not any(
        "approve" in str(event) or "probe-controller" in str(event) or "probe-research.service" in str(event)
...
def test_package_pins_are_checked_before_helpers_can_execute(tmp_path, monkeypatch):
    ...
    manifest = {
        ...
        "previous_activation_manifest_sha256": "f" * 64,
        "retry_id": "1" * 32,
        "failed": {
            "job_id": "old-job",
    ...
    put(path, r.encoded(manifest))
    pin = r.digest(path.read_bytes())
    monkeypatch.setattr(r, "read", lambda path, **_: Path(path).read_bytes())
    assert r.inputs(path, pin)[2] == files
    put(tmp_path / "upgrade-controller.py", b"changed helper")
    with pytest.raises(r.RetryError, match="FILE_PIN_CHANGED"):
        r.inputs(path, pin)
    with pytest.raises(r.RetryError, match="MANIFEST_PIN_CHANGED"):
        r.inputs(path, "0" * 64)
...
def test_generic_error_reports_only_type_and_helper_line(tmp_path, monkeypatch, capsys):
    ...
    assert (
        r.main(["prepare", "--manifest", str(tmp_path / "manifest"), "--manifest-sha256", "a" * 64, "--human", "human"])
        == 1
    )
...
def prior_retry(tmp_path, replacement, monkeypatch):
    ...
    prior = dict(
        schema_version=1,
        original_manifest_sha256="3" * 64,
        previous_activation_manifest_sha256="4" * 64,
    ...
    put(
        capsule / "close-report.json",
        r.encoded(
            dict(
                status="closed",
                manifest_sha256=previous_pin,
                original_job_cancelled=True,
    ...
    for name in prior["files"]:
        put(capsule / name, replacement.new[name])
    manifest = dict(
        schema_version=2,
        original_manifest_sha256=prior["original_manifest_sha256"],
        previous_activation_manifest_sha256=prior["previous_activation_manifest_sha256"],
        previous_upgrade=prior["target_upgrade"],
        target_upgrade={"wheel_sha256": "7" * 64, "release_manifest_sha256": "8" * 64},
        previous_retry_manifest_sha256=previous_pin,
        previous_retry_record_sha256=r.digest(raw),
...
def test_second_retry_loads_pinned_nested_inputs_and_changes_only_fresh_identity(prior_retry):
    ...
    plan = json.loads(old["plan.json"])
    plan["label"] += "-second"
    plan["cases"][0]["spec"]["idempotency_key"] += "-second"
    new["plan.json"] = r.encoded(plan)
    config = json.loads(old["acceptance.json"])
    new_public = r.PUBLIC / ("retry-" + s.manifest["retry_id"])
    old_config_path = Path(config["plan_path"]).parent / "acceptance.json"
    config.update(
        plan_path=str(new_public / "plan.json"),
        plan_sha256="sha256:" + r.digest(new["plan.json"]),
    ...
    for name in r.CALIBRATION:
        new[name] = old[name].replace(
            ("--config " + str(old_config_path)).encode(), ("--config " + str(new_public / "acceptance.json")).encode()
        )
...
@pytest.mark.parametrize(
    "fault", ["record", "incomplete", "wrong_wheel", "wrong_job", "file", "missing_close", "wrong_chain"]
)
def test_second_retry_refuses_unpinned_incomplete_or_mismatched_history(prior_retry, fault):
    s = prior_retry
    if fault == "record":
        s.manifest["previous_retry_record_sha256"] = "f" * 64
    elif fault == "file":
        put(s.capsule / "plan.json", b"changed")
    elif fault == "missing_close":
        (s.capsule / "close-report.json").unlink()
    elif fault == "wrong_chain":
        s.manifest["previous_upgrade"]["wheel_sha256"] = "f" * 64
    else:
        report = deepcopy(s.report)
        if fault == "incomplete":
            report["status"] = "failed"
        elif fault == "wrong_wheel":
            report["target_wheel_sha256"] = "f" * 64
        else:
...
def test_real_snapshot_deadline_failure_requires_exact_manifest_reason(real_failed):
    ...
    r.validate_failed(
        f.snapshot(), f.failed, f.plan, result, f.bound, f.submitted, expected_failure_reason="WORKER_STARTUP_DEADLINE"
    )
...
def test_schema2_manifest_requires_canonical_record_pin_and_allowlisted_reason(tmp_path, monkeypatch, fault):
    ...
    manifest = dict(
        ...
        target_upgrade={"wheel_sha256": "d" * 64, "release_manifest_sha256": "e" * 64},
        previous_activation_manifest_sha256="f" * 64,
        previous_retry_manifest_sha256="1" * 64,
        previous_retry_record_sha256="2" * 64,
        expected_failure_reason="WORKER_STARTUP_DEADLINE",
        retry_id="3" * 32,
        failed={"job_id": "job", "request_id": "request", "worker_id": "worker", "provider_id": "pod"},
        files={name: r.digest(raw) for name, raw in files.items()},
    )
    if fault == "missing_record_pin":
        del manifest["previous_retry_record_sha256"]
    if fault == "unknown_reason":
        manifest["expected_failure_reason"] = "UNRELATED_FAILURE"
    path = tmp_path / "manifest.json"
    put(path, r.encoded(manifest))
    monkeypatch.setattr(r, "read", lambda path, **_: Path(path).read_bytes())
    if fault:
        with pytest.raises(r.RetryError):
            r.inputs(path, r.digest(path.read_bytes()))
    else:
        assert r.inputs(path, r.digest(path.read_bytes()))[1] == manifest
...
def next_inputs(old, retry_id, *, new_worker=False):
    ...
    config.update(
        plan_path=str(public / "plan.json"),
        plan_sha256="sha256:" + r.digest(new["plan.json"]),
        submission_state_directory=str(r.SUBMIT / ("retry-" + retry_id)),
        trusted_state_directory=str(r.STATE / ("retry-" + retry_id)),
    )
    if new_worker:
        ...
        config.update(
            source_commit=worker["code_git_commit"],
            worker_config_path=str(public / "worker.json"),
    ...
    for name in r.CALIBRATION:
        new[name] = old[name].replace(
            ("--config " + old_path).encode(), ("--config " + str(public / "acceptance.json")).encode()
        )
    return new
...
def third_retry(prior_retry):
    ...
    report = dict(
        ...
        status="prepared",
        manifest_sha256=previous_pin,
        target_wheel_sha256=previous["target_upgrade"]["wheel_sha256"],
        old_history_preserved=True,
        approval_issued=False,
        cloud_mutations_performed=False,
        submission={**failed, "approval_consumed_by_runner": False},
    ...
    for name in previous["files"]:
        put(capsule / name, old[name])
    manifest = dict(
        previous,
        schema_version=3,
        previous_upgrade=previous["target_upgrade"],
        target_upgrade={"wheel_sha256": "c" * 64, "release_manifest_sha256": "d" * 64},
        previous_retry_manifest_sha256=previous_pin,
        previous_retry_record_sha256=r.digest(r.encoded(previous)),
        expected_failure_reason="REQUEST_CHANGED",
        failed_result_canonical_sha256="e" * 64,
        retry_id="c" * 32,
        failed=failed,
    )
...
@pytest.mark.parametrize(
    "fault",
    ["original_missing", "original_hash", "original_content", "earlier_file", "earlier_report", "previous_report"],
)
def test_third_retry_refuses_broken_original_or_intermediate_chain(third_retry, fault):
    s = third_retry
    if fault == "original_missing":
        (s.capsule / "manifest-original.json").unlink()
    elif fault == "original_hash":
        put(s.capsule / "manifest-original.json", b"{}\n")
    elif fault == "original_content":
        changed = deepcopy(s.previous)
        changed["retry_id"] = "9" * 32
        raw = r.encoded(changed)
        put(s.capsule / "manifest.json", raw)
        s.manifest["previous_retry_record_sha256"] = r.digest(raw)
    elif fault == "earlier_file":
        put(s.first / "plan.json", b"{}\n")
    else:
        folder = s.first if fault == "earlier_report" else s.capsule
        body = json.loads((folder / "prepare-report.json").read_bytes())
        body["submission"]["job_id"] = "unrelated"
...
def test_schema3_allows_software_provenance_only(third_retry, fault):
    ...
    worker = json.loads(s.new["worker.json"])
    plan = json.loads(s.new["plan.json"])
    if fault == "model":
        worker["model"]["revision_sha"] = "1" * 40
    elif fault == "dataset":
        worker["datasets"][0]["sha256"] = "sha256:" + "1" * 64
    elif fault == "ram":
        plan["cases"][0]["spec"]["limits"]["max_ram_bytes"] = (
            JobSpec.model_validate(plan["cases"][0]["spec"]).limits.max_ram_bytes + 1
        )
    elif fault == "price":
        config["expected_worker_price_usd_per_hour"] = 0.79
    elif fault == "region":
        config["deployment"]["region"] = "EU-RO-1"
    elif fault == "launch":
        config["deployment"]["launch_config_hash"] = "sha256:" + "1" * 64
    elif fault == "worker_price":
        worker["live_price_usd_per_hour"] = 0.79
    elif fault == "worker_path":
        config["worker_config_path"] = "/etc/probe-calibration/worker.json"
    elif fault == "same_image":
        worker["container_image_digest"] = json.loads(s.old["worker.json"])["container_image_digest"]
    elif fault == "same_source":
        worker["code_git_commit"] = json.loads(s.old["worker.json"])["code_git_commit"]
    ...
    config["worker_config_sha256"] = "sha256:" + r.digest(s.new["worker.json"])
    if fault == "worker_hash":
        config["worker_config_sha256"] = "sha256:" + "1" * 64
...
def test_schema3_operator_stop_requires_exact_complete_result_and_real_closed_zero_attempt_case(real_failed):
    ...
    altered = f.snapshot()
    altered["attempts"] = [{"attempt_id": "unexpected"}]
    with pytest.raises(r.RetryError, match="FAILED_CASE_EXECUTED_OR_CHANGED"):
        operation.verify_failure(altered)
    altered = f.snapshot()
    altered["provider_absent"] = False
    with pytest.raises(r.RetryError, match="PROVIDER_DELETION_UNCONFIRMED"):
        operation.verify_failure(altered)
    f.s.rpc.call("cancel_job", {"job_id": f.failed["job_id"]})
...
def test_schema3_manifest_requires_seven_pins_and_exact_reason(tmp_path, monkeypatch, fault):
    ...
    if fault == "missing_worker":
        del m["files"]["worker.json"]
    elif fault == "bad_worker_pin":
        m["files"]["worker.json"] = "0" * 64
    elif fault == "missing_result_pin":
        del m["failed_result_canonical_sha256"]
    elif fault == "old_reason":
        m["expected_failure_reason"] = "WORKER_STARTUP_DEADLINE"
    path = tmp_path / "manifest.json"
    put(path, r.encoded(m))
    monkeypatch.setattr(r, "read", lambda path, **_: Path(path).read_bytes())
    if fault:
        with pytest.raises(r.RetryError):
            r.inputs(path, r.digest(path.read_bytes()))
...
def test_schema3_prepare_publishes_separate_public_worker_before_submission(prepare_case):
    ...
    def ctl(*args):
        if args == ("start", r.CALIBRATION[0]):
            assert (s.public / "worker.json").read_bytes() == expected
...
@pytest.mark.parametrize(
    "fault",
    ["worker_missing", "worker_bytes", "original_missing", "original_bytes", "report", "earlier_worker_binding"],
)
def test_fourth_retry_refuses_damaged_schema3_capsule_or_chain(fourth_retry, fault):
    s = fourth_retry
    if fault == "worker_missing":
        (s.capsule / "worker.json").unlink()
    elif fault == "worker_bytes":
        put(s.capsule / "worker.json", s.original["worker.json"])
    elif fault == "original_missing":
        (s.capsule / "manifest-original.json").unlink()
...
@pytest.mark.parametrize(
    "field",
    ["worker_config_path", "worker_config_sha256", "source_commit", "image", "price", "region", "runtime", "model"],
)
def test_schema4_cannot_change_worker_or_scientific_scope(fourth_retry, field):
    s = fourth_retry
    old = s.operation.previous_inputs(s.original)
    config = json.loads(s.new["acceptance.json"])
    if field == "worker_config_path":
        config[field] = str(r.PUBLIC / "worker.json")
    elif field == "worker_config_sha256":
        config[field] = "sha256:" + "a" * 64
    elif field == "source_commit":
        config[field] = "a" * 40
    elif field == "image":
        config["deployment"]["image_digest"] = "sha256:" + "a" * 64
    elif field == "price":
        config["expected_worker_price_usd_per_hour"] = 0.79
    elif field == "region":
        config["deployment"]["region"] = "unapproved-region"
    elif field == "runtime":
...
def test_schema4_real_closed_zero_attempt_configuration_failure_remains_audited(real_failed):
    ...
    operation.result = result
    changed = f.snapshot()
    changed["attempts"] = [{"attempt_id": "unexpected"}]
    with pytest.raises(r.RetryError, match="FAILED_CASE_EXECUTED_OR_CHANGED"):
        operation.verify_failure(changed)
    changed = f.snapshot()
    changed["approvals"][0]["ended_at"] = None
    with pytest.raises(r.RetryError, match="FAILED_APPROVAL_NOT_CLOSED"):
        operation.verify_failure(changed)
    changed = f.snapshot()
    changed["provider_absent"] = False
    with pytest.raises(r.RetryError, match="PROVIDER_DELETION_UNCONFIRMED"):
        operation.verify_failure(changed)
    f.s.rpc.call("cancel_job", {"job_id": f.failed["job_id"]})
    operation.verify_failure(f.snapshot(), cancelled=True)
...
def test_schema4_rejects_other_configuration_failures_even_with_a_new_canonical_pin(real_failed, fault):
    ...
    elif fault == "configured":
        result["configuration"]["configured"] = False
    elif fault == "configuration_reason":
        result["configuration"]["reason"] = "UNRELATED"
    elif fault == "exception":
        result["diagnostic"]["exception_type"] = "ValueError"
    else:
        result["diagnostic"]["location"] = "gpu_acceptance_runner.py:1"
    manifest = {
        "schema_version": 4,
        "failed": f.failed,
        "expected_failure_reason": "ACCEPTANCE_RUNTIME_UNAVAILABLE",
...
def test_schema4_manifest_has_six_pins_and_exact_configuration_reason(tmp_path, monkeypatch, fault):
    ...
    manifest = dict(
        ...
        previous_retry_record_sha256="2" * 64,
        expected_failure_reason="ACCEPTANCE_RUNTIME_UNAVAILABLE",
        failed_result_canonical_sha256="3" * 64,
        retry_id="4" * 32,
        failed={"job_id": "job", "request_id": "request", "worker_id": "worker", "provider_id": "pod"},
        files={name: r.digest(raw) for name, raw in files.items()},
    )
    if fault == "extra_worker":
        manifest["files"]["worker.json"] = "5" * 64
    elif fault == "missing_result_pin":
        del manifest["failed_result_canonical_sha256"]
    elif fault == "missing_record_pin":
        del manifest["previous_retry_record_sha256"]
    elif fault == "old_reason":
        manifest["expected_failure_reason"] = "REQUEST_CHANGED"
    path = tmp_path / "manifest.json"
    put(path, r.encoded(manifest))
    monkeypatch.setattr(r, "read", lambda path, **_: Path(path).read_bytes())
    if fault:
...
def completed_retry_then_next(previous_case, retry_id):
    ...
    manifest = dict(
        ...
        previous_retry_manifest_sha256=pin,
        previous_retry_record_sha256=r.digest(r.encoded(previous)),
        expected_failure_reason="PROVIDER_ENDPOINT_REFUSED",
        expected_failure_stage="verified_worker_startup",
        failed_result_canonical_sha256="9" * 64,
        retry_id=retry_id,
        failed=failed,
    )
    ...
    return SimpleNamespace(
        ...
        previous=previous,
        capsule=capsule,
        prior_capsules=(*s.prior_capsules, capsule),
    )
...
def test_schema5_chain_refuses_repeated_excessive_or_broken_history(fifth_retry, monkeypatch, fault):
    ...
    if fault in {"repeat", "limit"}:
        kwargs["_seen"] = (
            (s.manifest["previous_retry_manifest_sha256"],)
            if fault == "repeat"
            else tuple(f"{index:064x}" for index in range(32))
        )
        monkeypatch.setattr(
            r, "read", lambda *_a, **_k: pytest.fail("refuse before reading a repeated/excessive capsule")
        )
...
def startup_failure(f, *, stage="verified_worker_startup", reason="PROVIDER_ENDPOINT_REFUSED"):
    return dict(
        f.result,
        schema_version=1,
        kind="single_public_gpu_calibration",
        scientific_evidence=False,
        stage=stage,
        reason=reason,
        startup={"endpoint_attempts": 201},
...
def startup_recovery(f, result):
    manifest = dict(
        schema_version=5,
        failed=f.failed,
        expected_failure_reason=result["reason"],
        expected_failure_stage=result["stage"],
        failed_result_canonical_sha256=r.digest(r.encoded(result)),
    )
    operation = r.Recovery(SimpleNamespace(Activation=lambda: None), None, manifest, {}, "a" * 64)
    operation.old = {"plan.json": r.encoded(f.plan)}
...
def test_schema5_cannot_recover_executed_unclosed_or_reclassified_cases(real_failed, fault):
    ...
    snapshot = f.snapshot()
    if fault == "attempt":
        snapshot["attempts"] = [{"attempt_id": "executed"}]
    elif fault == "attempt_count":
        snapshot["jobs"][0]["attempt_count"] = 1
    elif fault == "attempt_identity":
        snapshot["jobs"][0]["attempt_id"] = "executed"
    elif fault == "spec":
        value = json.loads(snapshot["jobs"][0]["spec_json"])
        value["limits"]["max_output_bytes"] += 1
        snapshot["jobs"][0]["spec_json"] = json.dumps(value)
    elif fault == "approval":
        snapshot["approvals"][0]["ended_at"] = None
    elif fault == "request":
        snapshot["requests"][0]["state"] = "RUNNING"
    elif fault == "absence":
        snapshot["provider_absent"] = False
    elif fault == "scientific":
        operation.result["scientific_evidence"] = True
    elif fault == "success":
        operation.result["status"] = "passed"
    elif fault == "result_schema":
        operation.result["schema_version"] = True
    elif fault == "result_kind":
        operation.result["kind"] = "different"
    elif fault == "result_job":
        operation.result["request_id"] = "different"
    elif fault == "result_stage":
        operation.result["stage"] = "approved_dispatch"
    else:
        operation.result["reason"] = "ANOTHER_REASON"
    ...
    with pytest.raises(r.RetryError):
        operation.verify_failure(snapshot)
...
@pytest.mark.parametrize(
    "fault", [None, "stage_missing", "stage_execution", "reason_private", "reason_empty", "extra_worker", "pin_missing"]
)
def test_schema5_manifest_requires_six_files_exact_stage_reason_and_result_pin(tmp_path, monkeypatch, fault):
    ...
    if fault == "stage_missing":
        del manifest["expected_failure_stage"]
    elif fault == "stage_execution":
        manifest["expected_failure_stage"] = "approved_dispatch"
    elif fault == "reason_private":
        manifest["expected_failure_reason"] = "Untrusted message /private/path"
    elif fault == "reason_empty":
        manifest["expected_failure_reason"] = ""
    elif fault == "extra_worker":
        manifest["files"]["worker.json"] = "5" * 64
    elif fault == "pin_missing":
        del manifest["failed_result_canonical_sha256"]
    path = tmp_path / "manifest.json"
    put(path, r.encoded(manifest))
    monkeypatch.setattr(r, "read", lambda path, **_: Path(path).read_bytes())
...
def retarget_retry(fifth_retry, monkeypatch):
    ...
    submitted = dict(
        ...
        configuration_hash=config.deployment.digest,
        approval_consumed_by_runner=False,
    )
    report = json.loads((s.capsule / "prepare-report.json").read_bytes())
    ...
    cancelled = dict(
        ...
        jobs=[
            dict(
                ...
                failure_reason="research client cancellation",
            )
        ],
        requests=[
            dict(
                ...
                state="PENDING",
                action="CREATE",
                job_ids=json.dumps([pending["job_id"]]),
                infrastructure=None,
                replaces_worker_id=None,
                deadline=None,
                observed_provider_id=None,
                last_error_code=None,
                configuration=config.deployment.model_dump_json(),
                configuration_hash=config.deployment.digest,
                max_runtime_seconds=config.max_runtime_seconds,
                approval_id=submitted["approval_id"],
                batch_hash=submitted["batch_hash"],
            )
        ],
    )
    files = {
        name: (PROJECT / "deploy" / name).read_bytes()
        for name in ("activate-gpu-calibration.py", "upgrade-controller.py", "retarget-gpu-calibration.py")
    }
    files.update({"old/" + name: raw for name, raw in old.items()})
    files.update({"new/" + name: raw for name, raw in new.items()})
    manifest = dict(
        schema_version=1,
        original_manifest_sha256=s.manifest["original_manifest_sha256"],
        installed_upgrade=s.manifest["previous_upgrade"],
        retarget_id="1" * 32,
        from_region="EU-CZ-1",
        to_region="EU-RO-1",
        pending=pending,
    ...
    return SimpleNamespace(
        **vars(s),
        retarget_old=old,
        retarget_new=new,
        retarget_manifest=manifest,
        retarget_capsule=capsule,
        retarget_report=prepared,
        retarget_cancelled=cancelled,
        marker=marker,
...
def test_retarget_predecessor_refuses_changed_pins_receipts_or_prior_authority(retarget_retry, monkeypatch, fault):
    s = retarget_retry
    if fault == "manifest":
        put(s.retarget_capsule / "manifest.json", b"{}\n")
    elif fault == "file":
        put(s.retarget_capsule / "new/plan.json", b"{}\n")
    elif fault == "helper":
        put(s.retarget_capsule / "retarget-gpu-calibration.py", b'raise AssertionError("must not execute")')
    elif fault == "receipt_missing":
        (s.retarget_capsule / "prepare-report.json").unlink()
    elif fault.startswith("receipt_"):
        report = deepcopy(s.retarget_report)
        if fault == "receipt_status":
            report["status"] = "failed"
        elif fault == "receipt_wheel":
            report["installed_wheel_sha256"] = "0" * 64
        elif fault == "receipt_identity":
            report["submission"]["job_id"] = "unrelated"
        elif fault == "receipt_approval":
            report["approval_issued"] = True
        else:
            report["old_history_preserved"] = False
        put(s.retarget_capsule / "prepare-report.json", r.encoded(report))
    elif fault == "cancel_intent":
        put(s.retarget_capsule / "cancel-intent.json", b"{}\n")
    elif fault == "cancelled_authority":
        snapshot = deepcopy(s.retarget_cancelled)
        snapshot["approvals"] = [{"approval_id": "unexpected"}]
        ...
        monkeypatch.setattr(
            r, "read", lambda path, **_: b"changed" if Path(path) == s.marker else Path(path).read_bytes()
        )
...
def test_even_repinned_retarget_cannot_change_scientific_scope_or_previous_retry_history(retarget_retry, fault):
    ...
    name = (
        "old/plan.json"
        if fault == "old_history"
        else "new/worker.json"
        if fault in {"model", "region"}
        else "new/acceptance.json"
    )
    body = json.loads((s.retarget_capsule / name).read_bytes())
    if fault == "old_history":
        body["label"] = "different old history"
    elif fault == "model":
        body["model"]["revision_sha"] = "0" * 40
    elif fault == "region":
        body["region"] = "another-region"
    elif fault == "deadline":
        body["max_runtime_seconds"] -= 1
    else:
        body["deployment"]["image_digest"] = "sha256:" + "0" * 64
    raw = r.encoded(body)
    put(s.retarget_capsule / name, raw)
    manifest = deepcopy(s.retarget_manifest)
    manifest["files"][name] = r.digest(raw)
    raw = r.encoded(manifest)
    new_pin = r.digest(raw)
    new_capsule = s.retarget_capsule.with_name(new_pin)
    s.retarget_capsule.rename(new_capsule)
    put(new_capsule / "manifest.json", raw)
...
def test_optional_retarget_pin_is_strict_and_keeps_six_existing_inputs(tmp_path, monkeypatch, pin):
    ...
    manifest = dict(
        ...
        failed={"job_id": "job", "request_id": "request", "worker_id": "worker", "provider_id": "pod"},
        files={name: r.digest(raw) for name, raw in files.items()},
        previous_retarget_manifest_sha256=pin,
    )
    path = tmp_path / "manifest.json"
    put(path, r.encoded(manifest))
    monkeypatch.setattr(r, "read", lambda path, **_: Path(path).read_bytes())
    if pin == "1" * 64:
        ...
        with pytest.raises(r.RetryError, match="PREVIOUS_RETARGET_PIN_INVALID"):
            r.inputs(path, r.digest(path.read_bytes()))
...
def guarded_retarget_prepare(prepare_case):
    ...
    for name in r.CALIBRATION:
        put(s.units / (name + ".d") / r.RETARGET_GUARD, old_guard)
        ...
        if args == ("daemon-reload",):
            s.events.append(
                (
                    "guard-files",
                    tuple(
                        tuple(sorted(p.name for p in (s.units / (n + ".d")).iterdir()))
                        if (s.units / (n + ".d")).exists()
                        else ()
                        for n in r.CALIBRATION
                    ),
                )
            )
        prior_ctl(*args)
    ...
    def unit(name):
        ...
        return dict(
            ActiveState="active" if name in started else "inactive",
            MainPID="42" if name in started else "0",
            ControlPID="0",
            DropInPaths=" ".join(sorted(str(p) for p in directory.iterdir())) if directory.exists() else "",
        )
...
def test_retarget_guard_or_inactivity_mismatch_never_replaces_units_or_submits(guarded_retarget_prepare, fault):
    ...
    if fault == "changed":
        put(directory / r.RETARGET_GUARD, b"[Unit]\nConditionPathExists=/unrelated\n")
    elif fault == "missing":
        (directory / r.RETARGET_GUARD).unlink()
    elif fault == "extra":
        put(directory / "99-unrelated.conf", b"[Service]\n")
    else:
        old = s.operation.unit_state
        s.operation.unit_state = lambda name: dict(
            old(name), **({"ActiveState": "active", "MainPID": "123"} if fault == "active" else {"DropInPaths": ""})
        )
    with pytest.raises(r.RetryError):
        s.operation.prepare()
...
def test_partial_retarget_guard_removal_preserves_new_blocking_guards(guarded_retarget_prepare, monkeypatch):
    ...
    def unlink(path, *args, **kwargs):
        if path == s.units / (r.CALIBRATION[1] + ".d") / r.RETARGET_GUARD:
