...
def bundle(tmp_path):
    ...
    installer.write_text(
        'echo "earlier install must not run"\nexit 99\n'
...
@pytest.mark.parametrize(
    "damage", ["manifest", "member", "extra_file", "later_directory", "escaping_link", "writable", "hardlink"]
)
...
def partial_state(tmp_path):
    ...
    installer = (
        SCRIPT.with_name("install-controller.sh")
        .read_text()
        .split("/opt/probe-core/python/bin/python3.13 -I -m venv /opt/probe-core/venv\n", 1)[0]
    )
...
@pytest.mark.parametrize(
    "relative",
    [
        "etc/probe-core/runpod.json",
        "var/lib/probe-core/research.sqlite",
        "var/lib/probe-provider/runpod.sqlite",
        "var/lib/probe-sandbox/.runtime",
        "var/lib/probe-backup/rclone.conf",
        "var/lib/probe-backups/outbox/existing.tar",
        "var/lib/probe-backups/receipts/existing.json",
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        "",
        "probe-trusted:165536:65535",
        "probe-trusted:0:65536",
        "probe-trusted:165536:65536\nother:200000:65536",
        "probe-trusted:165536:65536\nprobe-trusted:165537:65536",
        "probe-trusted:165536:65536\nmalformed",
        "probe-trusted:4294967290:65536",
    ],
)
...
def test_loaded_unit_is_rejected_even_if_unit_file_is_absent(tmp_path):
    ...
    def run(command, **kwargs):
        ...
        return subprocess.CompletedProcess(
            command, 0, b"probe-controller.service loaded active running\n" if command[1] == "list-units" else b"", b""
        )
...
def test_unit_symlink_is_rejected_without_querying_or_removing_it(tmp_path):
    ...
    with pytest.raises(recovery.RecoveryError, match="unit files"):
        recovery.verify_no_units(
            run=lambda *args, **kwargs: pytest.fail("must fail before query"), filesystem_root=tmp_path
        )
...
def test_failed_systemd_inspection_is_not_mistaken_for_no_matches(tmp_path, operation, stderr):
    def run(command, **kwargs):
        return subprocess.CompletedProcess(
            command, 1 if command[1] == operation else 0, b"", stderr if command[1] == operation else b""
        )
...
def test_continuation_executes_only_verified_tail_and_restores_only_interpreter(bundle, tmp_path):
    ...
    script = recovery.continuation_script(
        old,
        human="human",
        rclone_config=hostile_looking_path,
        provider_directory=tmp_path / "private provider",
        installed_root=root,
    )
    ...
    assert (root / "continued").read_text().splitlines() == [
        "human",
        str(hostile_looking_path),
        str(tmp_path / "private provider"),
    ]
...
def test_continuation_requires_unique_exact_boundary(count):
    ...
    with pytest.raises(recovery.RecoveryError, match="unique exact"):
        recovery.continuation_script(
            (marker * count).encode(),
            human="human",
            rclone_config=Path("/private/drive"),
