...
def tiny_bundle(tmp_path_factory):
    ...
    model = Qwen3ForCausalLM(Qwen3Config(vocab_size=64,hidden_size=32,intermediate_size=64,num_hidden_layers=2,num_attention_heads=4,num_key_value_heads=2,head_dim=8,max_position_embeddings=128,pad_token_id=0,eos_token_id=63)).eval()
    model.save_pretrained(model_directory, safe_serialization=True)
    dataset_path = root / "prompts.json"
    dataset = PromptDataset.model_validate({"prompts": [{"prompt_id":"short","token_ids":[1,2,3]},{"prompt_id":"long","token_ids":[4,5,6,7,8]}]})
    dataset_path.write_text(dataset.model_dump_json())
    tensor_root = root / "tensor-inputs"
    tensor_root.mkdir()
    identity = {"repo":"probe/testing-tiny-qwen3","revision_sha":sha256_file(model_directory / "config.json").removeprefix("sha256:"),"local_weight_hashes":[sha256_file(path) for path in sorted(model_directory.glob("*.safetensors"))],"tokenizer_revision":hashlib.sha256(b"explicit-token-id-fixture-no-tokenizer").hexdigest(),"dtype":"float32","quantized":False,"chat_template_hash":None,"thinking_mode":None}
    ...
    config = WorkerConfig(model_directory=str(model_directory), model=identity, assets=[{"path":path.name,"sha256":sha256_file(path)} for path in model_directory.iterdir() if path.is_file()], datasets=[{"path":str(dataset_path),"sha256":sha256_file(dataset_path)}],tensor_directory=str(tensor_root),output_directory=str(root / "outputs"),backend="nnsight",device="cpu",code_git_commit=source_commit,environment_lock_path=str(project / "uv.lock"),provider_backend="local_cpu",region="cpu-test",live_price_usd_per_hour=0.0)
...
def make_request(tiny_bundle):
    ...
    def make(operation=None, *, key="job-1"):
        spec=JobSpec.model_validate({"idempotency_key":key,"experiment_stage":"calibration","model":config.model.model_dump(mode="json"),"inputs":{"dataset_revision":config.datasets[0].sha256,"prompt_set_hash":prompt_set_hash(dataset,("short","long")),"prompt_ids":["short","long"],"random_seed":123,"generation":{"temperature":0.0,"max_new_tokens":3}},"operation":operation or {"kind":"capture","modules":[{"layer":0,"component":"residual"}],"positions":["last"],"reduction":"none"},"limits":{"max_runtime_seconds":60,"max_output_bytes":1000000,"max_cpu_cores":1}})
        return ExecutionRequest(job_id=key,attempt_id=key+"-attempt",worker_id="worker-1",approval_id="wake-1",deadline=datetime.now(timezone.utc)+timedelta(seconds=60),spec=spec)
    return make
...
def test_actual_hf_nnsight_noop_and_capture_parity(engine,make_request):
    ...
    reference,captures=engine.forward(inputs,lengths,request.spec.operation,backend="reference",limits=request.spec.limits)
    traced,saved=engine.forward(inputs,lengths,request.spec.operation,backend="nnsight",limits=request.spec.limits)
...
@pytest.mark.parametrize("component,head",[("residual",None),("attention_output",None),("mlp_output",None),("attention_head",1)])
def test_ablation_matches_raw_hooks_for_all_supported_modules(engine,make_request,component,head):
    ...
    reference,_=engine.forward(inputs,lengths,request.spec.operation,backend="reference",limits=request.spec.limits)
...
def test_patch_replays_captured_state_and_steering_zero_is_noop(engine,make_request,tiny_bundle):
    request=make_request()
    inputs,lengths=engine._inputs(request)
    baseline,captured=engine.forward(inputs,lengths,request.spec.operation,backend="reference",limits=request.spec.limits)
    source=tensor_reference(tiny_bundle,"captured",{"states":captured["layer_0_residual"].cpu()})
    patch=make_request({"kind":"patch","target":{"layer":0,"component":"residual"},"positions":["last"],"source":source})
    ...
    steer=make_request({"kind":"steer","target":{"layer":0,"component":"residual"},"positions":["last"],"direction":direction,"strength":0.0})
...
def test_nonzero_steering_matches_reference(engine,make_request,tiny_bundle):
    direction=tensor_reference(tiny_bundle,"nonzero-direction",{"direction":torch.arange(32,dtype=torch.float32)/32})
    request=make_request({"kind":"steer","target":{"layer":0,"component":"mlp_output"},"positions":[0,"last"],"direction":direction,"strength":0.3})
    inputs,lengths=engine._inputs(request)
    actual,_=engine.forward(inputs,lengths,request.spec.operation,limits=request.spec.limits)
    expected,_=engine.forward(inputs,lengths,request.spec.operation,backend="reference",limits=request.spec.limits)
...
def test_tensor_checksum_and_shape_are_enforced(engine,make_request,tiny_bundle):
    ...
    operation={"kind":"patch","target":{"layer":0,"component":"residual"},"positions":["last"],"source":source}
...
def test_padding_positions_never_select_padding(engine,make_request):
    request=make_request({"kind":"capture","modules":[{"layer":0,"component":"residual"}],"positions":[3],"reduction":"none"})
    inputs,lengths=engine._inputs(request)
...
def test_generation_and_readonly_inspection_are_real_bounded_operations(engine,make_request,tmp_path,kind):
    operation={"kind":kind}
    if kind=="weight_stats":operation["modules"]=["model.layers.0"]
    if kind=="tensor_slice":operation.update(parameter="model.embed_tokens.weight",starts=[0,0],sizes=[2,3])
    ...
    if kind=="tensor_slice":assert load_file(str(tmp_path/kind/"tensors.safetensors"))["weight_slice"].shape==(2,3)
    if kind=="weight_stats":assert summary["weights"]
    if kind=="module_manifest":assert any(item["name"]=="model.layers.0" for item in summary["modules"])


@pytest.mark.parametrize("algorithm",["ridge","logistic_regression"])
def test_probe_fitting_uses_safetensors_and_reproducible_disjoint_split(engine,make_request,tiny_bundle,tmp_path,algorithm):
    torch.manual_seed(7)
    x=torch.randn(100,3)
    ...
    request=make_request({"kind":"fit_probe","activations":features,"labels":labels,"algorithm":algorithm,"l2_penalty":0.01,"max_iterations":20},key=algorithm)
...
def test_spawned_supervisor_executes_and_positive_stop_receipt(tiny_bundle,make_request,tmp_path):
    ...
    try:
        ...
        while time.monotonic()<deadline:
            ...
            if status.process_stopped:break
            time.sleep(0.05)
...
def test_restart_missing_process_identity_cannot_invent_stop_receipt(tiny_bundle,make_request,tmp_path):
    ...
    receipt = ExecutionReceipt(job_id=request.job_id, attempt_id=request.attempt_id,
                               state=WorkerState.ACCEPTED, started_at=datetime.now(timezone.utc))
...
def test_text_inputs_apply_the_verified_thinking_flag(tiny_bundle,make_request,tmp_path):
    ...
    for mode in (True,False):
        ...
        body["assets"]=[{"path":path.name,"sha256":sha256_file(path)} for path in model_directory.iterdir() if path.is_file()]
        ...
        request["spec"]["inputs"].update(dataset_revision=sha256_file(dataset_path),prompt_set_hash=prompt_set_hash(dataset,("text",)),prompt_ids=["text"])
...
def test_multi_capture_is_ordered_and_includes_head_preprojection_tensors(engine,make_request):
    operation={"kind":"capture","modules":[{"layer":1,"component":"residual"},{"layer":0,"component":"residual"},{"layer":0,"component":"attention_head","head":1},{"layer":0,"component":"attention_head","head":0},{"layer":0,"component":"attention_output"},{"layer":0,"component":"mlp_output"}],"positions":[0,"last"],"reduction":"none"}
    request=make_request(operation)
    inputs,lengths=engine._inputs(request)
    a,reference=engine.forward(inputs,lengths,request.spec.operation,backend="reference",limits=request.spec.limits)
